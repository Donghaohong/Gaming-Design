...
/** The default output buffer size for each audio node */
...
/**
 * Returns the collection of available audio devices
 *
 ...
 *
 * @param output    Whether to show output (instead of input) devices
 *
 * @return the collection of available audio devices
 */
std::unordered_map<std::string, SDL_AudioDeviceID> AudioDevices::devices(bool output) {
    std::unordered_map<std::string, SDL_AudioDeviceID> result;
    int count = 0;
    ...
    if (count > 0) {
        for(int ii = 0; ii < count; ii++) {
            std::string name = SDL_GetAudioDeviceName(devices[count]);
            result[name] = devices[count];
        }
        SDL_free(devices);
    }
    return result;
...
std::vector<std::string> AudioDevices::occupied(bool output) {
    ...
    if (output) {
        for(auto it = _gManager->_outputs.begin(); it != _gManager->_outputs.end(); ++it) {
            result.push_back(it->second->getDeviceName());
        }
    }
    return result;
}
...
std::shared_ptr<cugl::audio::AudioOutput> AudioDevices::openOutput() {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_outputs.find(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK) != _outputs.end()) {
    ...
    if (result && result->init()) {
        _outputs[SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK] = result;
        if (_active) {
 ...
 *
 * @param device    The id of the output device
 *
 * @return the given output device with 2 channels at 48000 Hz.
 */
std::shared_ptr<audio::AudioOutput> AudioDevices::openOutput(SDL_AudioDeviceID device) {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_outputs.find(device) != _outputs.end()) {
        CULogError("Device '%s' is in use.",SDL_GetAudioDeviceName(device));
        return nullptr;
    }
...
std::shared_ptr<cugl::audio::AudioInput> AudioDevices::openInput() {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_inputs.find(SDL_AUDIO_DEVICE_DEFAULT_RECORDING) != _inputs.end()) {
        CULogError("Default output device is in use.");
        return nullptr;
    }
    ...
    if (result && result->init()) {
        _inputs[SDL_AUDIO_DEVICE_DEFAULT_RECORDING] = result;
        return result;
    }
    return nullptr;
}
 ...
 */
std::shared_ptr<audio::AudioInput> AudioDevices::openInput(SDL_AudioDeviceID device) {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_outputs.find(device) != _outputs.end()) {
        CULogError("Device '%s' is in use.",SDL_GetAudioDeviceName(device));
        return nullptr;
 ...
 */
std::shared_ptr<audio::AudioInput> AudioDevices::openInput(SDL_AudioDeviceID device,
                                                           Uint8 channels, Uint32 rate, Uint32 delay) {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_outputs.find(device) != _outputs.end()) {
        CULogError("Device '%s' is in use.",SDL_GetAudioDeviceName(device));
        return nullptr;
