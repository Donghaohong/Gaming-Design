...
ParticleVertex& ParticleVertex::set(const std::shared_ptr<JsonValue>& json) {
    ...
    if (json->isArray()) {
        ...
        position.set(values[0],values[1],0);
...
void ParticleSystem::update(float delta, const Vec3& camera) {
    ...
    ParticleInstance check[100];
    std::memcpy(check,_instances,sizeof(ParticleInstance)*100);
    ...
    if (_instanceBuffer != nullptr) {
        _instanceBuffer->loadData(_instances, _allocated);
    }
}
 ...
 */
void ParticleSystem::draw(const std::shared_ptr<ParticleShader>& shader) {
    if (_instanceBuffer == nullptr) {
        return;
    }
    
    shader->setDrawMode(_mesh.command);
    shader->setVertices(0, _templateBuffer);
    shader->setVertices(1, _instanceBuffer);
    shader->setIndices(_indexBuffer);
    shader->drawInstancesIndexed((Uint32)_mesh.indices.size(), (Uint32)_allocated);
}
...
bool ParticleSystem::createBuffers() {
    _templateBuffer = VertexBuffer::alloc(_mesh.vertices.size(),
                                          sizeof(ParticleVertex),
                                          BufferAccess::STATIC);
    if (_templateBuffer == nullptr) {
        return false;
    }
    
    _indexBuffer = IndexBuffer::alloc(_mesh.indices.size(),
                                      BufferAccess::STATIC);
    if (_indexBuffer == nullptr) {
        return false;
    }
    
    _instanceBuffer = VertexBuffer::alloc(_capacity,
    ...
    _templateBuffer->loadData<ParticleVertex>(_mesh.vertices);
    _indexBuffer->loadData(_mesh.indices);
    return true;
}
