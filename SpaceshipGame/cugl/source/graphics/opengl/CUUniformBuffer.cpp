...
bool UniformBuffer::init(size_t capacity, Uint32 blocks, bool stream) {
    ...
    size_t minAlignment = value;
    _blockcount = blocks;
    _blocksize = capacity;

    // Even if we are not streaming, we force alignment
    _blockstride = _blocksize;
    if (minAlignment > 0) {
        _blockstride = (_blocksize + minAlignment - 1) & ~(minAlignment - 1);
    }
    ...
    if (stream) {
        if (!_data->initMapped(_blockstride*_blockcount)) {
            delete _data;
            _data = nullptr;
            return false;
        }
...
void UniformBuffer::pushBytes(Sint64 block, size_t offset, const Uint8* data, size_t amt) {
    CUAssertLog(block < _blockcount, "Block %zu is out of range [%d]",(size_t)block,_blockcount);
