...
uniform int  uType;
// SDL3 requires that we add this
...
void main(void) {
    ...
    // Result is either a solid color or gradient
    ...
    // This is done last as it might be premultiplied
    if (mod(fType, 2.0) == 1.0) {
        vec4 texel = (uType >= 8) ? blursample(outTexCoord) : texture(uTexture, outTexCoord);
        if (uPremult > 0) {
            vec4 color = result;
            result.a   = color.a * texel.a;
            result.rgb = color.rgb * texel.rgb * color.a;
        } else {
            result *= texel;
        }
    }
