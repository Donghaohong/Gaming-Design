 ...
 * drawing.
 */
void PolygonObstacle::resetMesh() {
    Poly2 shape = _polygon*_units;
    
    Rect bounds = shape.getBounds();
    Vec2 offset = bounds.size*_anchor;
    shape -= offset+bounds.origin;
    ...
    _mesh.indices.reserve(2*shape.indices.size());
    for(int ii = 0; ii < shape.indices.size(); ii+=3) {
        _mesh.indices.push_back(shape.indices.at(ii  ));
        _mesh.indices.push_back(shape.indices.at(ii+1));
        _mesh.indices.push_back(shape.indices.at(ii+1));
        _mesh.indices.push_back(shape.indices.at(ii+2));
        _mesh.indices.push_back(shape.indices.at(ii+2));
        _mesh.indices.push_back(shape.indices.at(ii  ));
    }
