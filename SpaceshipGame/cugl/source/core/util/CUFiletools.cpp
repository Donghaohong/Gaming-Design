...
/**
 * Returns the Windows wide string equivalent of a UTF8 string.
 *
 * @param s The string to convert
 *
 * @return the Windows wide string equivalent of a UTF8 string.
 */
...
/**
 * Returns the UTF8 string equivalent of a Windows wide string.
 *
 * @param w The string to convert
 *
 * @return the UTF8 string equivalent of a Windows wide string.
 */
...
size_t file_size(const std::string path) {
    ...
    size_t result = 0;
    Uint8 buf[256];
    SDL_IOStream* rw = SDL_IOFromFile(fullpath.c_str(), "rb");
    
    if (rw) {
        // This is really the only reliable way to do this
        size_t amt = SDL_ReadIO(rw, buf, sizeof (buf));
        while (amt > 0) {
            result += amt;
            amt = SDL_ReadIO(rw, buf, sizeof (buf));
        }
        SDL_CloseIO(rw);
    }
    return result;
...
Uint64 file_timestamp(const std::string path) {
    ...
    std::wstring wide = utf8_to_wide(fullpath);
    ...
    bool ok = FileTimeToLocalFileTime(&search_data.ftLastWriteTime, &localFileTime);
    if (ok) {
        SYSTEMTIME sysTime;
 ...
 * converts a relative path to its absolute equivalent. Relative paths
 * are interpretted to be with respect to the asset directory.
 *
 * As with normalization, this function replaces all path separators with
 * the correct system-dependent versions. It also normalizes the path
 * prefix (e.g. capitalizing drive letters on Windows and so on).
 *
 * Finally, canonicalization removes all redundant directories (e.g.
...
const std::string canonicalize_path(const std::string path) {
    ...
    if (!is_absolute(path)) {
        result = Application::get()->getAssetDirectory()+result;
    }
...
bool file_create(const std::string path) {
    ...
    if (stat(fullpath.c_str(), &status) != 0) {
        SDL_IOStream* stream = SDL_IOFromFile(fullpath.c_str(), "w");
        if (stream) {
            SDL_CloseIO(stream);
            return true;
        }
...
std::vector<std::string> dir_contents(const std::string path) {
    ...
    WIN32_FIND_DATAW search_data{};
    std::wstring wide = utf8_to_wide(search_path);
    HANDLE handle = FindFirstFileW(wide.c_str(), &search_data);

    bool avail = true;
    while (avail && handle != INVALID_HANDLE_VALUE) {
        std::string file = wide_to_utf8(search_data.cFileName);
        if (file != "." && file != "..") {
...
std::vector<std::string> dir_contents(const std::string path, const std::function<bool(const std::string file)>& filter) {
    ...
    bool avail = true;
    while (avail && handle != INVALID_HANDLE_VALUE) {
        std::string file = wide_to_utf8(wide);
