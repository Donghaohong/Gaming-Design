 ...
 */
bool Vec2::doesSegmentIntersect(const Vec2& A, const Vec2& B,
                                const Vec2& C, const Vec2& D) {
    float S, T;
...
/**
 * Returns the barycentric coordinates for this point relative to a triangle.
 *
 * Barcentric coordinates define this point as the linear combination of
 * three other points, which are assumed to be the corner vertices of a
 * triangle.
 *
 * @param a     The first triangle vertex
 * @param b     The second triangle vertex
 * @param b     The third triangle vertex
 *
 * @return the barycentric coordinates for this point relative to a triangle.
 */
const Vec3 Vec2::barycentric(const Vec2& a, const Vec2& b, const Vec2& c) {
   float det = (b.y-c.y)*(a.x-c.x)+(c.x-b.x)*(a.y-c.y);
   Vec3 result;
   result.x = (b.y-c.y)*(x-c.x)+(c.x-b.x)*(y-c.y);
   result.y = (c.y-a.y)*(x-c.x)+(a.x-c.x)*(y-c.y);
   result.x /= det;
   result.y /= det;
