...
//
#ifndef __CU_GAMEPAD_H__
#define __CU_GAMEPAD_H__
 ...
 *
 * This class is built on top of the SDL Gamepad interface. This allows the
 * developer to use a uniform input independent of the controller input type
 * (i.e. D-input vs Xinput). This API is slightly more restrictive than the
 * SDL joystick interface, in that it does not have support for hats or track
 * balls). Instead, the types of input are limited to the following:
 *
 ...
 *
 * Note that SDL gamepads also support sensors (accelerometers) and touch pads
 * (such as the PS4 touchpad). None of that is currently supported.
 *
 * The advantage of the SDL gamepad API is that layout is uniform. All gamepads
 * have buttons and axes in the same place. Our API uses the XBox names of
 * buttons (A, B, X, Y) instead of the Playstation names, as they are easier to
 * reference.
 *
 * There should only be one instance of a specific gamepad at any given time.
 * In addition, gamepads can be connected and removed while the application
 * is running. For that reason, this class does not allow you to allocate a
 * gamepad object. Instead, you must access each gamepad through the
 * {@link GamepadInput} interface.
 */
class Gamepad {
public:
    /**
     ...
     * not all gamepads support all buttons. For example, most gamepads do not
     * support the paddles found on the XBox elite controller. To determine if
     * a button is supported, use the method {@link Gamepad#hasButton}.
     */
     ...
     *
     * This is the list of all axes supported by this interface. Note that not
     * all gamepads support all axes. For example, classic Nintendo gamepads
     * have no axes at all! To determine if an axis is supported, use the
     * method {@link Gamepad#hasAxis}.
     */
    enum class Axis : int {
        /** An invalid axis */
        INVALID = -1,
...
#pragma mark Listener Types
     ...
     *
     * This type represents an axis listener for the {@link Gamepad} class.
     *
     ...
     * method {@link Gamepad#getAxisPosition} for more information.
     *
     * Listeners are guaranteed to be called at the start of an animation frame,
     * before the method {@link Application#update(float) }.
     *
     * While gamepad listeners do not traditionally require focus like a
     * keyboard does, we have included that functionality. While only one
     ...
     *
     * This type represents a DPad listener for the {@link Gamepad} class.
     *
     * In CUGL, listeners are implemented as a set of callback functions, not 
     * as objects. This allows each listener to implement as much or as little
     * functionality as it wants. A listener is identified by a key which should
     ...
     * {@link Gamepad#getDPadPosition} for more information. Note that
     ...
     *
     * This type represents a button listener for the {@link Gamepad} class.
     *
     * In CUGL, listeners are implemented as a set of callback functions, not 
     * as objects. This allows each listener to implement as much or as little
     * functionality as it wants. A listener is identified by a key which should
     ...
     * and/or down. See the methods {@link Gamepad#isButtonPressed} and
     * {@link Gamepad#isButtonReleased} for more information. Note that
...
private:
     ...
     */
    bool init(SDL_JoystickID joyid, const std::string uid);
...
#pragma mark Input Methods
    ...
    /**
     * Records that a {@link GamepadDPadEvent} has occured
     *
     * @param stamp The event timestamp
     */
...
#pragma mark Attributes
     ...
     *
     * UIDs are unique identifiers assigned by {@link GamepadInput}.
     *
     * @return the UID of this gamepad.
     */
...
public:
     ...
     */
    GamepadAxisEvent() : axis(Gamepad::Axis::INVALID), value(0) {}
     ...
     */
    GamepadAxisEvent(const std::string key, Gamepad::Axis index,
...
public:
     ...
     */
    GamepadButtonEvent() : button(Gamepad::Button::INVALID), down(false) {}
     ...
     * @param stamp The timestamp for the event
     */
    GamepadButtonEvent(const std::string key, Gamepad::Button index,
...
public:
     ...
     */
    GamepadDPadEvent() : state(Gamepad::DPad::CENTERED) {}
     ...
     * @param stamp The timestamp for the event
     */
    GamepadDPadEvent(const std::string key, Gamepad::DPad pos,
...
public:
     ...
     * @param stamp The timestamp for the event
     */
    GamepadInputEvent(const std::string key, bool add,
                             const Timestamp& stamp) {
 ...
 *
 * While it is possible to have more than one gamepad attached at any time, SDL
 * broadcasts all gamepad state changes. Therefore, it is useful to have a
 * central hub that manages gamepads and dispatches events to the appropriate
 * gamepad. In addition, this particular input device monitors when gamepads
 * are added and removed.
 *
 * Gamepads only receive events when they are activated. See the methods
 * {@link GamepadInput#open} and {@link GamepadInput#close} for how to activate
 * and deactivate gamepads.
 */
...
public:
     ...
     * @typedef Listener
     *
     * This type represents a listener for the {@link GamepadInput} class.
     *
     ...
     * be a globally unique unsigned int.
     *
     * An event is delivered whenever a new gamepad is either added to or
     * removed from the list of devices. This can happen when a device loses
     * power, or it connected during a play session.
     *
     * Listeners are guaranteed to be called at the start of an animation frame,
     * before the method {@link Application#update(float) }.
     *
     * While gamepad input listeners do not traditionally require focus like a
     * keyboard does, we have included that functionality. While only one
     * listener can have focus at a time, all listeners will receive input from
     * the Gamepad input device.
     *
     * The function type is equivalent to
     *
     *      std::function<void(const GamepadInputEvent event, bool focus)>
     *
     * @param event     The gamepad input event
     * @param focus     Whether the listener currently has focus
     */
    typedef std::function<void(const GamepadInputEvent event,
                               bool focus)> Listener;
...
private:
    ...
    /** The active gamepads, identified by SDL id */
    std::unordered_map<SDL_JoystickID, std::shared_ptr<Gamepad>> _bysdl;
    /** The active gamepads, identified by UID */
    std::unordered_map<std::string, std::shared_ptr<Gamepad>> _byname;
...
#pragma mark Internals
     ...
     */
    std::string addDevice(SDL_JoystickID joyid);
    
    /**
     * Removes a new device to this manager.
     *
     * This method will close the associated gamepad, invalidating
...
#pragma mark Constructors
    /**
     * Creates and initializes a new gamepad manager.
     *
     * WARNING: Never allocate a gamepad manager directly. Always
     * use the {@link Input#activate()} method instead.
     */
    GamepadInput() : _filter(true) { }
    
    /**
     * Deletes this input device, disposing of all resources
     */
    virtual ~GamepadInput() { dispose(); }
...
public:
     ...
     *
     * @return true if this gamepad manager filters its devices
     */
...
#pragma mark Listener Methods
     ...
     * @param key   The identifier for the listener
     *
     * @return the gamepad manager listener for the given object key
     */
