 ...
 * vertex type is templated. This allows a mesh to be adapted to an arbitrary
 * {@link GraphicsShader}.
 *
...
public:
     ...
     */
    Mesh() { command = DrawMode::UNDEFINED; }
     ...
     * (such as color or texture coordinate) must be added later. The drawing
     * command will be {@link DrawMode#TRIANGLES}.
     *
    ...
    /**
     * Creates a mesh from the given {@link Path2} object.
     *
     * No vertex attribute other than position is set. Additional information
     * (such as color or texture coordinate) must be added later. The drawing
     * command will be {@link DrawMode#LINES}.
     *
     * @param path  The path defining this mesh
     */
    Mesh(const Path2& path) {
        set(path);
    }
...
#pragma mark Setters
     ...
     * references to elements of the other mesh. This method returns a reference
     * to this mesh for chaining.
     *
     ...
     * will be {@link DrawMode::TRIANGLES}.
     *
    ...
    /**
     * Sets the mesh to match the {@link Path2} object.
     *
     * No vertex attribute other than position is set. Additional information
     * (such as color or texture coordinate) must be added later. The command
     * will be {@link DrawMode#LINES}.
     *
     * This method returns a reference to this polygon for chaining.
     *
     * @param path  The path defining this mesh
     ...
     *
     * The resulting mesh has no indices. The command will be reset to
     * {@link DrawMode::UNDEFINED}.
     *
     * This method returns a reference to this mesh for chaining.
     *
     ...
     * {@link DrawMode#TRIANGLES}. Otherwise, if it is even, it will be
     * {@link DrawMode#LINES}. All other values will be undefined and the user
     * must manually set the type.
     *
     * This method returns a reference to this mesh for chaining.
     *
     ...
     * @return This mesh for chaining
     */
    Mesh& set(const std::vector<T>& verts, const std::vector<Uint32>& indx) {
        for(auto it = verts.begin(); it != verts.end(); ++it) {
            T vertex;
            vertex.position = *it;
            vertices.push_back(vertex);
    ...
    /**
     * Clears the contents of this mesh and sets the command to DrawMode::UNDEFINED
     *
     * @return This mesh for chaining
     */
    Mesh& clear() {
...
#pragma mark Operators
     ...
     *
     * The vertices of other are appended to the end of this mesh. The indices
     * are reindexed to account for this shift.
     *
     * This method will fail to append to the mesh if other does not share the
     * same command as this mesh.
     *
...
#pragma mark Slicing
     ...
     *
     * The only sliceable mesh types are {DrawMode#POINTS}, {@link DrawMode#LINES}
     * and {@link DrawMode#TRIANGLES}. That is because the mesh is represented
     * in regular, decomposable chunks. This method not only checks that the
     * command is correct, but that the index size is correct as well.
     *
     * @return true is this mesh is sliceable.
     */
    bool isSliceable() const {
        if (command == DrawMode::LINES) {
            return indices.size() % 2 == 0;
        } else if (command == DrawMode::TRIANGLES) {
            return indices.size() % 3 == 0;
        }
        return command == DrawMode::POINTS;
    }
     ...
     * the end). It will include the vertices referenced by those indices, and
     * only those vertices. The command will remain the same.
     *
     * The only sliceable mesh types are {DrawMode#POINTS}, {@link DrawMode#LINES}
     * and {@link DrawMode#TRIANGLES}. That is because the mesh is represented
     * in regular, decomposable chunks. Any attempt to slice another mesh type
     * will fail.
     *
    ...
    Mesh slice(size_t start, size_t end) const  {
        CUAssertLog(command == DrawMode::LINES || command == DrawMode::TRIANGLES,
                    "Mesh is not a sliceable type");
        
        int divider = command == DrawMode::TRIANGLES ? 3 : 2;
        CUAssertLog(start % divider != 0 && start < indices.size(),
                    "Start position %d is not a valid slice point", start);
        CUAssertLog(end % divider != 0 && end <= indices.size(),
                    "End position %d is not a valid slice point", end);
        
        std::unordered_map<Uint32,Uint32> visited;
        std::vector<T> verts;
        std::vector<Uint32> indx;
        for(size_t ii = start; ii < end; ii++) {
            auto find = visited.find(indices[ii]);
            if (find != visited.end()) {
                indx.push_back(find->second);
            } else {
                visited[indices[ii]] = (Uint32)verts.size();
     ...
     *
     * The sliced mesh will use the indices up to  (but not including) end. It
     * will include the vertices referenced by those indices, and only those
     * vertices. The command will remain the same.
     *
     * The only sliceable mesh types are {DrawMode#POINTS}, {@link DrawMode#LINES}
     * and {@link DrawMode#TRIANGLES}. That is because the mesh is represented
 ...
 *
 * For this template function to work, the type T must have a constructor that
 * takes a JsonValue to initialize an individual vertex. Note that
 * {@link SpriteVertex} has such a constructor.
 *
 * The mesh can be represented as either an array or JSON object. If it is an
 * array, it then it should contain JSON values compatible with the constructor
 * for that type. If it is a JSON object, then it supports the following
 * attributes:
 *
 ...
 *
 * All attributes are optional. If "vertices" are missing, the mesh will be
 * empty. If both "indices" and "triangulator" are missing, the mesh will use
 * a triangle fan. The "triangulator" choice will only be applied if the
 * "indices" are missing.
 *
    ...
    if (json->isObject()) {
        if (json->has("indices")) {
            ...
            if (choice == "fan") {
                ...
                mesh.command = DrawMode::TRIANGLE_FAN;
            } else if (choice == "strip") {
                for(int ii = 0; ii < mesh.vertices.size(); ii++) {
                    mesh.indices.push_back(ii);
                }
                mesh.command = DrawMode::TRIANGLE_STRIP;
            } else {
                if (choice == "monotone") {
                ...
                mesh.command = DrawMode::TRIANGLES;
            }
        }
    }
    
    // Default to triangle fan.
    if (mesh.command == DrawMode::UNDEFINED) {
        for(int ii = 0; ii < mesh.vertices.size(); ii++) {
            mesh.indices.push_back(ii);
        }
        mesh.command = DrawMode::TRIANGLE_FAN;
