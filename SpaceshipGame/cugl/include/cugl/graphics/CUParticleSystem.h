...
//
//  It has been modified to fit our agnostic OpenGL/Vulkan approach. This module 
//  provides a simplified interface that supports sprite animation as well as 
//  particle coloring. While the API for a particle system is 3d, this can 
//  easily be integrated into 2d scene graphs simply by setting the z-value to 0.
//
 ...
 *
 * Particle Vertices are used by {@link VertexBuffer} to render the particle
 ...
 *
 * Particle Instances are used by {@link ParticleShader} objects to render the
 * particle instances. So we do not refer to them with shared pointers, or have
 * a fancy constructor for them.
 */
 ...
 *
 * Each particle system has its own instance vertex buffer. This is done for
 ...
 * shader to display this particles. See {@link ParticleShader} for more
 * information.
 *
...
private:
    ...
    /** The vertex buffer for the particle template */
    std::shared_ptr<VertexBuffer> _templateBuffer;
    /** The index buffer for the particles */
    std::shared_ptr<IndexBuffer> _indexBuffer;
    /** The instance buffer for the particles */
...
#pragma mark Particles
     ...
     * affects the {@link #getInstances}. Therefore, the mesh should never be
     * modified directly. Changes should go through {@link #setMesh}.
     *
...
#pragma mark Simulation
    ...
    /**
     * Draws the render buffer with the given graphics shader.
     *
     * @param shader    The graphics shader to draw with
     */
    void draw(const std::shared_ptr<ParticleShader>& shader);
...
private:
    ...
    /**
     * Allocates the buffers for this particle system
     *
     * @return true if the buffers were successfully created
     */
    bool createBuffers();
