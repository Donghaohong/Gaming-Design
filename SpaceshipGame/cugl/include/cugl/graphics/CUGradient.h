 ...
 * And this is the format that this data is represented in the {@link #std140}
 ...
 * of the paint gradient, to pass them to a shader directly (though the
 * transform must be inverted first if it is passed directly).
 *
...
#pragma mark Attributes
     ...
     * to a shader, you should use {@link #pack} instead.
     *
...
#pragma mark Conversion
     ...
     */
    float* std140(float* array) const;
    
    /**
     * Reads the gradient into the provided array
     *
     * The gradient is written to the array so that it can be passed
     * the the shader as a push constant (e.g. NOT in std140 format).
     * It differs from {@link #std140} in that it only uses 9 floats
     * for the affine transform (as a 3x3 homogenous matrix).
     *
     * @param array     The array to store the values
     *
     * @return a reference to the array for chaining
     */
    float* pack(float* array) const;
