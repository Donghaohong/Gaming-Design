...
public:
     ...
     *
     * This scene will have a viewport that matches the display and a
     * perspective camera with a FOV of 67 degrees. The camera will be pointed
     * at the origin.
     *
     * The scene will not have an attached pipeline. You must add one with
     * {@link #setPipeline}.
     *
     ...
     *
     * The viewport is used to define a perspective camera on the display. The
     * camera will be pointed at the origin with a FOV of 67 degrees.
     *
     * The scene will not have an attached pipeline. You must add one with
     * {@link #setPipeline}.
     *
    ...
    bool initWithViewport(const Size& size) {
        return initWithViewport(0,0,size.width,size.height);
    }
    ...
    bool initWithViewport(float width, float height) {
        return initWithViewport(0,0,width,height);
    }
     ...
     * camera will be pointed at the origin with a FOV of 67 degrees.
     *
     * The viewport origin only affects the coordinate conversion methods
     * {@link Camera#project()} and {@link Camera#unproject()} which are used
     * to convert from the scene graph to a display. It is intended to
    ...
    bool initWithViewport(const Rect& rect) {
        return initWithViewport(rect.origin.x,rect.origin.y,
    ...
    bool initWithViewport(const Vec2& origin, const Size& size) {
        return initWithViewport(origin.x,origin.y,size.width, size.height);
    }
     ...
     *
     * The viewport will match the size of the display. The field of view given
     * is for the height, and is measured in degrees.
     *
     * The scene will not have an attached pipeline. You must add one with
     * {@link #setPipeline}.
     *
...
#pragma mark Static Constructors
     ...
     * {@link #setPipeline}.
     *
     * @return a newly allocated scene with the default FOV.
     */
    ...
    static std::shared_ptr<Scene3> allocWithViewport(const Size& size) {
        std::shared_ptr<Scene3> result = std::make_shared<Scene3>();
        return (result->initWithViewport(size) ? result : nullptr);
    }
    ...
    static std::shared_ptr<Scene3> allocWithViewport(float width, float height) {
        std::shared_ptr<Scene3> result = std::make_shared<Scene3>();
        return (result->initWithViewport(width,height) ? result : nullptr);
    }
    ...
    static std::shared_ptr<Scene3> allocWithViewport(const Rect& rect) {
        std::shared_ptr<Scene3> result = std::make_shared<Scene3>();
        return (result->initWithViewport(rect) ? result : nullptr);
    }
    ...
    static std::shared_ptr<Scene3> allocWithViewport(const Vec2& origin, const Size& size) {
        std::shared_ptr<Scene3> result = std::make_shared<Scene3>();
        return (result->initWithViewport(origin,size) ? result : nullptr);
    }
    ...
    static std::shared_ptr<Scene3> allocWithViewport(float x, float y, float width, float height) {
        std::shared_ptr<Scene3> result = std::make_shared<Scene3>();
        return (result->initWithViewport(x,y,width,height) ? result : nullptr);
    }
     ...
     * @param fov   The field of view
     *
     * @return a newly allocated scene with the given field of view.
     */
    static std::shared_ptr<Scene3> allocWithFOV(float fov) {
        std::shared_ptr<Scene3> result = std::make_shared<Scene3>();
        return (result->initWithFOV(fov) ? result : nullptr);
...
#pragma mark Scene Logic
     ...
     *
     * Scene3 nodes are typically rendered with a {@link Scene3Pipeline} object.
     * In particular the method {@link #render} traverses the scene graph and
     * sends those nodes to pipeline (though this behavior can be overridden).
     *
     * As graphics pipelines are fairly heavy-weight pipelines, we do not
     * construct a pipeline for each scene. Instead a pipeline has to be
     * assigned to the scene. If no pipeline is assigned, nothing is drawn.
     *
     * @return the pipeline for rendering this scene.
     */
    std::shared_ptr<Scene3Pipeline> getPipeline() const { return _pipeline; }
     ...
     * assigned to the scene. If no pipeline is assigned, nothing is drawn.
     *
     * @param pipeline  The pipeline for rendering this scene.
     */
     ...
     *
     * If this value is nullptr (the default), then this scene will be rendered
     * to the primary display. Otherwise, it will be rendered to the given
     * framebuffer. The framebuffer must be compatible with a sprite batch.
     * However, we do not require that the framebuffer be the same size as
     * the display.
     *
     * @return the offscreen framebuffer associated with this scene.
     */
     ...
     * the display.
     *
     * @param buffer    The offscreen framebuffer for this scene (or nullptr)
     */
