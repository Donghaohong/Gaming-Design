...
public:
    /**
     * The resizing constraint for this node.
     *
     * Constraints are used for top-level scene nodes in lieu of a
     * {@link Layout} when the scene needs to be resized. They have no effect
     * on internal nodes. Such nodes should have a constraint of {@link #LAYOUT}.
     * Note that a node with constraint {@link #NONE} will ignore any requests
     * from a layout manager.
     */
    enum class Constraint : int {
        /** The node should ignore any resizing or layout requests */
        NONE   = 0,
        /** The node should be resized to fill the full scene */
        SCENE  = 1,
        /** The node should be resized to fill the scene safe area */
        SAFE   = 2,
        /** The node should be managed by a parent layout manager */
        LAYOUT = 3
    };
...
//protected:
    ...
    /** The constraint for responding to layout requests */
...
public:
    ...
    /**
     * Initializes a node to fill the display.
     *
     * This initializer is intended for top-level nodes that will fill the
     * entire display. It has position (0,0) and the size of the display.
     * If added to a scene, this node will resize itself to match the viewport
     * of that scene (but will not reposition itself).
     *
     * @return true if initialization was successful.
     */
    ...
    /**
     * Initializes a node to fill the safe area of the display.
     *
     * This initializer is intended for top-level nodes that will fill the
     * safe area of the display (e.g. UI elements). Its position is the
     * bottom left corner of the safe area and is sized to match. If added to
     * a scene, this node will resize itself to match the safe area of that
     * scene.
     *
     * @return true if initialization was successful.
     */
...
#pragma mark Static Constructors
    ...
    /**
     * Returns a newly allocated node to fill the display.
     *
     * This initializer is intended for top-level nodes that will fill the
     * entire display. It has position (0,0) and the size of the display.
     * If added to a scene, this node will resize itself to match the viewport
     * of that scene (but will not reposition itself).
     *
     * @return a newly allocated node to fill the display.
     */
    static std::shared_ptr<SceneNode> allocForDrawableBounds() {
        std::shared_ptr<SceneNode> result = std::make_shared<SceneNode>();
        return (result->initForDrawableBounds() ? result : nullptr);
    }
    
    /**
     * Returns a newly allocated node to fill the safe area of the display.
     *
     * This initializer is intended for top-level nodes that will fill the
     * safe area of the display (e.g. UI elements). Its position is the
     * bottom left corner of the safe area and is sized to match. If added to
     ...
     * scene.
     *
     * @return a newly allocated node to fill the safe area of the display.
     */
    static std::shared_ptr<SceneNode> allocForSafeBounds() {
        std::shared_ptr<SceneNode> result = std::make_shared<SceneNode>();
        return (result->initForSafeBounds() ? result : nullptr);
    }
...
#pragma mark Rendering
     ...
     * drawing code. You are welcome to use any graphics calls that you wish.
...
#pragma mark Layout Automation
    /**
     * Returns the layout constraint for this node
     *
     * Layout constraints determine how a node responds to a layout manager.
     * Only nodes with constraint {@link Constraint#LAYOUT} behave as normal.
     * Nodes with constraint {@link Constraint#SCENE} or {@link Constraint#SAFE}
     * will resize themselves to fit the scene. On the other hand, nodes with
     * {@link Constraint#NONE} will be untouched on any layout request.
     *
     * Note that changing this value has no immediate results. You must either
     * call {@link #doLayout} on the parent or resize the scene for it to take
     * effect.
     *
     * @return the layout constraint for this node
     */
    Constraint getLayoutConstraint() const { return _constraint; }
    
    /**
     * Sets the layout constraint for this node
     *
     * Layout constraints determine how a node responds to a layout manager.
     * Only nodes with constraint {@link Constraint#LAYOUT} behave as normal.
     * Nodes with constraint {@link Constraint#SCENE} or {@link Constraint#SAFE}
