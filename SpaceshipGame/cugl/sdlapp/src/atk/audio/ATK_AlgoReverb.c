...
/**
 * Gathers the audio input into a single audio channel
 *
 * Reverb is designed to apply to a single mono signal and then distribute it
 * to the other channels. This function combines the wet mix into a single
 * channel so that we can apply reverb.
 *
 ...
 *
 * The input signal should already be collapsed into a mono signal in the input
 * buffer.
 *
 * @param filter    The reverb filter
 * @param frames    The number of frames to process
...
/**
 * Distributes the reverb to the output channels.
 *
 * This function is applied to the input signal as reverb needs to include a
 * dry mix in its calculations.
 *
 * Reverb is distributed to the left and right channels according to the wet1
 * and wet2 settings. These values are averaged for all other channels.
 *
 * @param filter    The reverb filter
 * @param input     The input signal (for the dry mix)
...
/**
 * Distributes the tail of the reverb to the output channels.
 *
 * This function is called when the input signal is exhausted (so there is no
 * dry mix) but there is still data left in the reverb filter to process. This
 * is essentially the final "echo" at the end.
 *
 * Reverb is distributed to the left and right channels according to the wet1
 * and wet2 settings. These values are averaged for all other channels.
 *
