 ...
 *
 * @return true if swap was successful, otherwise return false
 */
static int swap_buffers(ATK_LatencyAdapter* adapter) {
    bool result = SDL_TryWaitSemaphore(adapter->spoll);
    if (!result) {
        return result;
    }
    ...
    if (!adapter->paused && !SDL_GetSemaphoreValue(adapter->spush)) {
        SDL_SignalSemaphore(adapter->spush);
    }
    return true;
}
...
static void fill_adapter(ATK_LatencyAdapter* adapter) {
    if (adapter->callback == NULL) {
        if (!SDL_GetSemaphoreValue(adapter->spoll)) {
            SDL_SignalSemaphore(adapter->spoll);
        }
        return;
    }
...
static int thread_func(void* data) {
    ...
    while (SDL_GetAtomicInt(&(adapter->active))) {
        SDL_WaitSemaphore(adapter->spush);
...
ATK_LatencyAdapter* ATK_AllocLatencyAdapter(size_t input, size_t output,
    ...
    Uint8* front = NULL;
    Uint8* back  = NULL;
    SDL_Semaphore* spush = NULL;
    SDL_Semaphore* spoll = NULL;
    ...
    result->mutex = mutex;
    result->thread = NULL;
    SDL_SetAtomicInt(&(result->active), 1);
...
void ATK_FreeLatencyAdapter(ATK_LatencyAdapter* adapter) {
    ...
    SDL_SetAtomicInt(&(adapter->active), 0);
    SDL_SignalSemaphore(adapter->spush);
...
Sint64 ATK_PollLatencyAdapter(ATK_LatencyAdapter* adapter, Uint8* output, int len) {
    if ((size_t)len > adapter->frontavail-adapter->frontoffst) {
        if (!swap_buffers(adapter)) {
            return -1;
        }
    }
    
    Sint64 check = (Sint64)(adapter->frontavail-adapter->frontoffst);
    Sint64 amt = (Sint64)len < check ? len : check;
...
Sint64 ATK_PushLatencyAdapter(ATK_LatencyAdapter* adapter, const Uint8* input, int len) {
    ...
    bool result = SDL_TryLockMutex(adapter->mutex);
    if (!result) {
        return -1;
    }
...
void ATK_PauseLatencyAdapter(ATK_LatencyAdapter* adapter, int pauseon) {
    ...
    if (pauseon) {
        SDL_WaitSemaphore(adapter->spoll);
    } else if (!SDL_GetSemaphoreValue(adapter->spush)) {
        SDL_SignalSemaphore(adapter->spush);
    }
}
 ...
 */
bool ATK_BlockLatencyAdapter(ATK_LatencyAdapter* adapter) {
    if (!adapter->paused && !SDL_GetSemaphoreValue(adapter->spoll)) {
        SDL_WaitSemaphore(adapter->spoll);
        SDL_SignalSemaphore(adapter->spoll);
        return true;
    }
