...
inline static float tick_iir(float value, ATK_IIRFilter* filter) {
    ...
    float result = 0.0;
    result += filter->b[bdeg] * filter->ins[bdeg-1];
    for (size_t ii = bdeg-1; ii > 0; ii--) {
    ...
    result += filter->a[adeg] * filter->outs[adeg-1];
    for (size_t ii = adeg-1; ii > 0; ii--) {
...
static void fill_iir(const float* src, float* dst, size_t size,
    ...
    while(size--) {
        ...
        *output = result;
        SDL_memmove(filter->ins+1,  filter->ins, (bdeg-1)*sizeof(float));
        SDL_memmove(filter->outs+1, filter->outs, (adeg-1)*sizeof(float));
        filter->ins[0] = prev;
...
ATK_IIRFilter* ATK_AllocIIRFilter(float* a, size_t asize, float* b, size_t bsize) {
    ...
    if (asize > 0) {
        ...
        if (asize > 1) {
            outs = (float*)ATK_malloc(sizeof(float)*(adeg));
            if (outs == NULL) {
                ATK_OutOfMemory();
                ATK_free(acoef);
                return NULL;
            }
            memset(outs,0,sizeof(float)*(adeg));
        }
    } else {
    ...
    if (bsize > 1) {
        ins = (float*)ATK_malloc(sizeof(float)*(bdeg));
        if (ins == NULL) {
            ATK_OutOfMemory();
            ATK_free(acoef);
            ATK_free(outs);
        ...
        memset(ins,0,sizeof(float)*(bdeg));
    }
