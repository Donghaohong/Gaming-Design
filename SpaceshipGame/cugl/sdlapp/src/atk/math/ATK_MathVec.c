 ...
 * @param data      The data buffer
 * @param len       The number of elements to search
 * @param max       Optional pointer to store the minimum
 *
 * @return the minimum value in the data buffer
 */
...
void ATK_VecPairInterp(const float* input, const float* factors,
    ...
    while(amt--) {
        ...
        index = (int)temp;
        alpha = temp - index;
        left = (index < 0 || index >= (int)len) ? 0 : input[index];
        rght = (index < -1 || index >= (int)len-1) ? 0 : input[index+1];
...
void ATK_VecPairInterp_stride(const float* input, size_t istride, const float* factors,
    ...
    while(amt--) {
        ...
        index = (int)temp;
        alpha = temp - index;
        left = (index < 0 || index >= (int)len) ? 0 : input[index*istride];
        rght = (index < -1 || index >= (int)len-1) ? 0 : input[(index+1)*istride];
 ...
 */
void ATK_VecPowAmpToDecib(const float* input, float zero, bool power,
                          float* output, size_t len) {
    const float* src = input;
    float* dst = output;
 ...
 * @param len       The number of elements to convert
 */
void ATK_VecDecibToPowAmp(const float* input, float zero, bool power,
                          float* output, size_t len) {
    const float* src = input;
    float* dst = output;
    float factor = (float)(power ? 10 : 20);
