 ...
 * It is based on minimp3, which is a serviceable library but not without some
 * flaws.
 */
#pragma mark -
 ...
 * @param dst   The UTF-8 equivalent
 */
static void UCS2_to_UTF8(const Uint16 *src, Uint8 *dst) {
    bool swapped = SDL_Swap32BE(UNICODE_BOM_NATIVE) == UNICODE_BOM_NATIVE;
 ...
 */
const char* ATK_GetCommentID3Tag(const char* tag) {
...
const char* ATK_GetID3CommentTag(const char* tag) {
    ...
    for(size_t ii = 0; ii < len; ii++) {
        Uint32 code2;
        memcpy(&code2, MP3_TAGS[ii], sizeof(Uint32));
        if (code1 == code2) {
 ...
 */
static size_t GetID3v2Length(SDL_IOStream* source) {
    char buffer[ATK_ID3_DETECT_SIZE];
    size_t amt = SDL_ReadIO(source, buffer, ATK_ID3_DETECT_SIZE);
    if (!amt) {
        return 0;
    }
...
/**
 * Returns true if MP3 supports the given comment tag.
 *
 * @param tag   The tag to query
 *
 * @return true if MP3 supports the given comment tag.
 */
bool ATK_MP3_SupportsCommentTag(const char* tag) {
 ...
 */
const char* const * ATK_MP3_GetCommentTags(void) {
    return MP3_VERBOSE;
}
 ...
 */
static ATK_AudioComment* ATK_MP3_AllocComments(SDL_IOStream* source, int* len) {
    size_t amt = GetID3v2Length(source);
    char* buffer = ATK_malloc(sizeof(char)*amt);
    if (!SDL_ReadIO(source, buffer, amt)) {
        ATK_free(buffer);
        return NULL;
    }
    ...
    for(size_t ii = 0; pos < amt && ii < amt-4; ii++) {
        // Ensure memory alignment
        Uint32 size;
        memcpy(&size,buffer+pos+4,sizeof(Uint32));
    ...
    SDL_SeekIO(source, 0, SDL_IO_SEEK_SET);
    *len = (int)num;
    return result;
}
...
size_t read_stream(void *buf, size_t size, void *user_data) {
    SDL_IOStream *stream = (SDL_IOStream*)user_data;
...
int seek_stream(uint64_t position, void *user_data) {
    SDL_IOStream *stream = (SDL_IOStream*)user_data;
    return (SDL_SeekIO(stream,position,SDL_IO_SEEK_SET) < 0 ? 1 : 0);
}

/**
 * Allocates a new MP3 stream for the given file
 *
 * The file must be in the current working directory, or else this will return
 * NULL. It the responsibility of the user to free this stream when done.
 *
 * @param source	The RWops for the MP3 data
 *
 * @return a new MP3 stream for the given file
 */
MP3Stream* MP3Stream_Alloc(SDL_IOStream *source) {
	if (source == NULL) {
		return NULL;
	}
 ...
 */
bool MP3Stream_Free(MP3Stream* stream) {
    if (stream == NULL) {
 ...
 */
bool MP3Stream_SetSample(MP3Stream* stream, size_t sample) {
    if (stream == NULL) {
        return false;
    }
    return mp3dec_ex_seek(&(stream->context),sample) == 0;
}

/**
 * Reads in a page of MP3 data.
 *
 * This function reads in the current stream page into the buffer. The data
 * written into the buffer is linear PCM data with interleaved channels. If the
 * stream is at the end, nothing will be written.
 *
 * The size of a page is given by {@link MP3Stream_GetPageSize}. This buffer
 * should be large enough to hold this data. As the page size is in audio
 * frames, that means the buffer should be
 * pagesize * # of channels * sizeof(Sint16) bytes.
 *
 * @param stream	The MP3 stream
 ...
 *
 * The data written into the buffer is linear PCM data with interleaved
 * channels. If the stream is not at the initial page, it will rewind before
 * writing the data. It will restore the stream to the initial page when done.
 *
 * The buffer needs to be large enough to hold the entire MP3 stream. As the
 ...
 * stream is at the end, nothing will be written.
 *
 * The size of a page is given by {@link ATK_MP3_GetSourcePageSize}. This
 * buffer should large enough to hold this data. As the page size is in audio
 * frames, that means the buffer should be
 * pagesize * # of channels * sizeof(float) bytes.
 *
 * This function is a helper for {@link ATK_MP3_ReadSourcePage} and
 * {@link ATK_MP3_ReadSource}.
 *
...
ATK_AudioSource* ATK_LoadMP3(const char* filename) {
    ...
    return ATK_LoadMP3_IO(stream,1);
}
 ...
 */
ATK_AudioSource* ATK_LoadMP3_IO(SDL_IOStream* source, bool ownsrc) {
    if (source == NULL) {
 ...
 */
bool ATK_SourceIsMP3(SDL_IOStream* source) {
    if (source == NULL) {
        return false;
    }
    ...
    bool result = converter->context.samples > 0;
    MP3Stream_Free(converter);
    ATK_ClearError();
 ...
 */
bool ATK_MP3_UnloadSource(ATK_AudioSource* source) {
 ...
 */
bool ATK_MP3_IsSourceEOF(ATK_AudioSource* source) {
    CHECK_SOURCE(source,false)
    ATK_MP3_Decoder* decoder = (ATK_MP3_Decoder*)(source->decoder);
 ...
 * @return a new ATK_AudioSource from an MP3 readable/seekable IOStream
 */
ATK_AudioSource* ATK_LoadMP3_IO(SDL_RWops* source, bool ownsrc) {
    ATK_SetError("Codec MP3 is not supported");
    return NULL;
}
 ...
 * @returns true if this is MP3 data.
 */
bool ATK_SourceIsMP3(SDL_RWops* source) { return false; }
 ...
 * @return true if the source was successfully closed
 */
bool ATK_MP3_UnloadSource(ATK_AudioSource* source) { return false; }
 ...
 * @return true if the audio source is at the end of the stream
 */
bool ATK_MP3_IsSourceEOF(ATK_AudioSource* source) { return false; }
 ...
 * @return true if MP3 supports the given comment tag.
 */
bool ATK_MP3_SupportsCommentTag(const char* tag) { return false; }
