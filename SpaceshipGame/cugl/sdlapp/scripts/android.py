...
conceivably works with iOS (though not well), it does not work with Android at 
all. That is because an Android project is an amalgamation of C++ files, Java 
files, Makefiles and Gradle files. Configuring these projects is error prone, 
as a lot of different files have to be touched.
...
def config_ndkmake(config,project):
    ...
    sdlroot  = os.path.join(project,'app','jni','sdl3pckg')
