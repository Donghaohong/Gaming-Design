...
SDL3_ATK_MAKE   := $(LOCAL_PATH)
SDL3_ATK_PATH   := $(LOCAL_PATH)/$(CURR_DEPTH)/$(ATK_OFFSET)
SDL3_ATK_SOURCE := $(SDL3_ATK_PATH)/src/atk
...
include $(SDL3_ATK_MAKE)/$(KFFT_LIBRARY_PATH)/Android.mk
...
ifeq ($(SUPPORT_FLAC),true)
    include $(SDL3_ATK_MAKE)/$(FLAC_LIBRARY_PATH)/Android.mk
	SUPPORT_OGG = true
endif

# Build the library
ifeq ($(SUPPORT_VORB),true)
    include $(SDL3_ATK_MAKE)/$(VORB_LIBRARY_PATH)/Android.mk
	SUPPORT_OGG = true
endif

# Build the library
ifeq ($(SUPPORT_OGG),true)
    include $(SDL3_ATK_MAKE)/$(OGG_LIBRARY_PATH)/Android.mk
endif

# Keep file mangling at a minimum
LOCAL_PATH := $(SDL3_ATK_SOURCE)

include $(CLEAR_VARS)

LOCAL_MODULE := SDL3_atk
...
LOCAL_C_INCLUDES := $(SDL3_ATK_PATH)/include
LOCAL_C_INCLUDES += $(SDL3_ATK_PATH)/$(KFFT_LIBRARY_PATH)
...
ifeq ($(SUPPORT_FLAC),true)
	LOCAL_C_INCLUDES += $(SDL3_ATK_PATH)/$(FLAC_LIBRARY_PATH)/include
...
ifeq ($(SUPPORT_VORB),true)
	LOCAL_C_INCLUDES += $(SDL3_ATK_PATH)/$(VORB_LIBRARY_PATH)/include
...
ifeq ($(SUPPORT_OGG),true)
    LOCAL_C_INCLUDES += $(SDL3_ATK_PATH)/$(OGG_LIBRARY_PATH)/include
    LOCAL_STATIC_LIBRARIES += ogg
endif

# This is an include only library
ifeq ($(SUPPORT_MP3),true)
	LOCAL_C_INCLUDES += $(SDL3_ATK_PATH)/$(MP3_LIBRARY_PATH)
    LOCAL_CFLAGS += -DLOAD_MP3
endif

LOCAL_EXPORT_C_INCLUDES += $(SDL3_ATK_PATH)
...
###########################
#
# SDL3_atk static library
#
###########################

LOCAL_MODULE := SDL3_atk_static

LOCAL_MODULE_FILENAME := libSDL3_atk
