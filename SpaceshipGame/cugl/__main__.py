...
PLATFORMS = { 'android' : 'Android build via Android Studio',
              'apple'   : 'Combined macOS/iOS build via XCode',
...
def setup():
    ...
    parser.add_argument('-t', '--target', type=str,
                        choices=['android','apple','windows','cmake'],
...
def normalize(config,args):
    ...
    if not 'backend' in  config:
        ...
        config['backend'] = config['backend'].lower()
        if not config['backend'] in ['headless','opengl','vulkan']:
            config['backend'] = 'opengl'
...
def expand_modules(config):
    ...
    :type config:  ``dict``
    """
    modules = ['graphics','audio', 'scene2','scene3','physics2','netcode','physics2::distrib','physics2::lights']
    modset = {}
    ...
    if config['modules']['physics2::lights']:
        config['modules']['physics2'] = True
        config['modules']['graphics'] = True
