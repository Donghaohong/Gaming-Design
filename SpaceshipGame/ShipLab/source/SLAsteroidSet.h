...
//  Technically, we could just use an unordered_set for this.  This is a
//  collection type in C++. But as we saw in the intro class, sometimes it makes
//  sense to factor out information that is common to all asteroids into the
//  container class. And that is what we are doing here.
//
 ...
 *
 * All asteroids share the same texture and physical information. Therefore, we
 * put all common information in the AsteroidSet. Individual asteroid
 * information (scale, velocity, and position) goes in the asteroid itself.
 *
 * One of the things you will notice about this class is that AsteroidSet is
 * composed of two unordered sets: current and pending. That is because of how
 * we plan to destroy asteroids. In C++ it is 100% to delete items from a set
 * that you are looping over (as long as you do it correctly). But it is not
 * safe to ADD elements to a set when you loop over it. However, when we destroy
 * an asteroid, we want to replace it with three smaller asteroids.
 *
 * To make this action safe, when you spawn a new asteroid, it is not added to
 * the current set immediately. Instead, it is added to the pending set.
 * Asteroids are moved from the pending set to the current set when you call
 * {@link #update}. So you can delete an asteroid from current and spawn smaller
 * asteroids in pending without worrying about an infinite loop.
 */
class AsteroidSet {
...
public:
    ...
    public:
         ...
         */
        const std::shared_ptr<cugl::graphics::SpriteSheet>& getSprite() const {
            return _sprite;
        }
         ...
         */
        void setSprite(const std::shared_ptr<cugl::graphics::SpriteSheet>& sprite);
...
public:
     ...
     * we have to be prepared for this to be null at all times. This texture
     * will be used to generate the sprite sheet for each asteroid. Asteroids
     * must have different sprite sheets because, while they share a texture,
     * they do not share the same animation frame.
     *
     * The sprite sheet information (size, number of columns) should have been
     * set in the initial JSON. If not, this texture will be ignored.
     *
     ...
     *
     * All asteroids are added to a pending set; they do not appear in the
     * current set until {@link #update} is called. This makes it safe to add
     * new asteroids while still looping over the current asteroids.
     *
     ...
     * edge of the screen, then it appears across the edge on the opposite side.
     * However, this method performs no collision detection. Collisions are
     * resolved afterwards.
     */
    void update(cugl::Size size);
