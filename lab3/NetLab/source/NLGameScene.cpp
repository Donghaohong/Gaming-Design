...
void GameScene::processData(const std::string source,
                            const std::vector<std::byte>& data) {
    // Color messages are exactly 4 bytes (RGBA, as sent by transmitColor)
    if (data.size() != 4) {
        return;
    }

    Uint8 r = std::to_integer<Uint8>(data[0]);
    Uint8 g = std::to_integer<Uint8>(data[1]);
    Uint8 b = std::to_integer<Uint8>(data[2]);
    Uint8 a = std::to_integer<Uint8>(data[3]);

    Application::get()->setClearColor(Color4(r,g,b,a));
}
...
void GameScene::transmitColor(Color4 color) {
    // We never receive our own network messages, so set local color now.
    ...
    msg[0] = std::byte{(unsigned char)color.r};
    msg[1] = std::byte{(unsigned char)color.g};
    msg[2] = std::byte{(unsigned char)color.b};
    msg[3] = std::byte{(unsigned char)color.a};
