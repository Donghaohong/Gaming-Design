...
Icon generation is always annoying, because each platform has its own specific 
format. This script automates this process by generating the standard formats 
for macOS, iOS, Android and Windows.

To work it must be given a 1024x1024 PNG file for the icon. The icon may have 
transparency, but this transparency will be removed on all mobile platforms 
(where it is not allowed). On those platforms the image will be composited on 
top of a background color. By default this background color is white, but it 
can be specified with a custom attribute.

The transparent attribute will only apply the backing color on mobile platforms. 
Otherwise it will be applied on both desktop and mobile platforms. The rounded 
attribute will apply rounded corners on the desktop icons. Both it and 
transparent cannot be true; transparent will take priority.
...
def web_to_rgb(web):
    ...
    The web color is a string that may or may not start with an #. It should 
    have 6 hexadecimal digits (2 characters per channel). The value returned is 
    a 3-element tuple of ints in the range 0..255.
...
def float_image(image):
    ...
    This method requires that that image is be square.  It resizes the image to 
    be twice the size of the highest resolution icon (for better downscaling), 
    and adds the empty padding that dynamic icons require.
...
class IconMaker(object):
    ...
    It works by specifying a foreground image and a background color. If the 
    foreground image is opaque, the color has no effect. But images with 
    transparency require a background color on mobile devices.
    """
    ...
    def generate(self,root):
        ...
        The icons will be placed relative to the given root directory. Each 
        platform will get a subfolder for its name (e.g. iOS, macOS, android, 
        windows). See the platform specific generation methods for information 
        about further subfolders.
    ...
    def gen_macos(self,root):
        ...
        The icons will be placed in the 'Mac.xcassets/AppIcon.appiconset' 
        subdirectory of the root directory. To add icons directly to an XCode 
        project, the root should be the Resources folder of your XCode project.
    ...
    def gen_ios(self,root):
        ...
        The icons will be placed in the 'iOS.xcassets/AppIcon.appiconset' 
        subdirectory of the root directory. To add icons directly to an XCode 
        project, the root should be the Resources folder of your XCode project.
        ...
        for s in formats:
            ...
            tint = { "appearances" : [ { "appearance" : "luminosity", "value" : "tinted" } ] }
            tint = { **tint, **info}
            data.append(tint)
    ...
    def gen_android(self,root):
        ...
        The icons will be placed in the given directory. If the directory is 
        the resource directory for an existing Android project, it will copy 
        the images into the appropriate folders. Otherwise, it will generate 
        the folders so that you can copy them manually.
