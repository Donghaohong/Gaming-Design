...
Android nor iOS have package managers.
...
against Linux and the platforms listed above, and we have made modifications to 
the default SDL CMake file. The other options create an IDE project for the 
given platform.  For example, `android` creates an Android Studio project while 
`ios` or `macos` create an XCode project. The `windows` option creates a Visual 
Studio project. All projects are fully configured and ready to run.

We also have an experimental `flatpak` option. This is a project built on top of 
CMake for packaging x86 Linux builds for rapid deployment on the Steam Deck. 
...
- SDL3, version 3.28.0
- SDL3-Image, version 3.2.5
- SDL3-TTF, version 3.2.3
- SDL3-ATK, version 3.2.0

For SDL3-Image we enable JPEG, PNG, and WEBP formats. This is sufficient for 
our applications. For SDL3-TTF, we enable FreeType, Harfbuzz, and PlutoSVG

Other SDL3 extensions are not supported, as our course at Cornell prefers to 
use alternate packages for audio and networking.
