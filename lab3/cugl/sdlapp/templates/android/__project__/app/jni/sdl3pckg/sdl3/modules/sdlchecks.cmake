...
macro(CheckOSS)
  if(SDL_OSS)
    ...
    if(HAVE_OSS_SYS_SOUNDCARD_H)
      set(HAVE_OSS TRUE)
      sdl_glob_sources(${SDL3_BASE_DIR}/src/audio/dsp/*.c)
...
macro(CheckALSA)
  if(SDL_ALSA)
    ...
    if(ALSA_FOUND)
      sdl_glob_sources("${SDL3_BASE_DIR}/src/audio/alsa/*.c")
...
macro(CheckPipewire)
  if(SDL_PIPEWIRE)
    ...
    if(PC_PIPEWIRE_FOUND)
      ...
      sdl_glob_sources("${SDL3_BASE_DIR}/src/audio/pipewire/*.c")
      sdl_glob_sources("${SDL3_BASE_DIR}/src/camera/pipewire/*.c")
...
macro(CheckPulseAudio)
  if(SDL_PULSEAUDIO)
    ...
    if(PC_PULSEAUDIO_FOUND)
      ...
      sdl_glob_sources("${SDL3_BASE_DIR}/src/audio/pulseaudio/*.c")
...
macro(CheckJACK)
  if(SDL_JACK)
    ...
    if(PC_JACK_FOUND)
      set(HAVE_JACK TRUE)
      sdl_glob_sources("${SDL3_BASE_DIR}/src/audio/jack/*.c")
...
macro(CheckSNDIO)
  if(SDL_SNDIO)
    ...
    if(PC_SNDIO_FOUND)
      set(HAVE_SNDIO TRUE)
      sdl_glob_sources("${SDL3_BASE_DIR}/src/audio/sndio/*.c")
...
macro(CheckX11)
  ...
  if(SDL_X11)
    ...
    if(X11_LIB AND HAVE_XEXT_H)
      ...
      sdl_glob_sources("${SDL3_BASE_DIR}/src/video/x11/*.c")
...
macro(CheckWayland)
  if(SDL_WAYLAND)
    ...
    if(WAYLAND_FOUND)
      ...
      sdl_glob_sources("${SDL3_BASE_DIR}/src/video/wayland/*.c")
      ...
      file(GLOB WAYLAND_PROTOCOLS_XML RELATIVE "${SDL3_BASE_DIR}/wayland-protocols/" "${SDL3_BASE_DIR}/wayland-protocols/*.xml")
      foreach(_XML IN LISTS WAYLAND_PROTOCOLS_XML)
        string(REGEX REPLACE "\\.xml$" "" _PROTL "${_XML}")
        WaylandProtocolGen("${WAYLAND_SCANNER}" "${WAYLAND_SCANNER_CODE_MODE}" "${SDL3_BASE_DIR}/wayland-protocols/${_XML}" "${_PROTL}")
      endforeach()
...
macro(CheckCOCOA)
  if(SDL_COCOA)
    ...
    if(HAVE_COCOA)
      sdl_glob_sources("${SDL3_BASE_DIR}/src/video/cocoa/*.m")
...
macro(CheckVivante)
  if(SDL_VIVANTE)
    ...
    if(HAVE_VIVANTE_VDK OR HAVE_VIVANTE_EGL_FB)
      ...
      sdl_glob_sources("${SDL3_BASE_DIR}/src/video/vivante/*.c")
...
macro(CheckOpenVR)
  if(SDL_OPENVR)
    ...
    sdl_glob_sources("${SDL3_BASE_DIR}/src/video/openvr/*.c")
...
macro(CheckEGL)
  if(SDL_OPENGL OR SDL_OPENGLES)
    ...
    list(APPEND CMAKE_REQUIRED_INCLUDES "${SDL3_BASE_DIR}/src/video/khronos")
    check_c_source_compiles("
        #define EGL_API_FB
...
macro(CheckQNXScreen)
  if(QNX AND HAVE_OPENGL_EGL)
    ...
    if(HAVE_QNX_SCREEN)
      ...
      sdl_glob_sources("${SDL3_BASE_DIR}/src/video/qnx/*.c")
...
macro(CheckPTHREAD)
  ...
  if(SDL_PTHREADS)
    ...
    if(HAVE_PTHREADS)
      ...
      sdl_sources(
        "${SDL3_BASE_DIR}/src/thread/pthread/SDL_systhread.c"
        "${SDL3_BASE_DIR}/src/thread/pthread/SDL_sysmutex.c"   # Can be faked, if necessary
        "${SDL3_BASE_DIR}/src/thread/pthread/SDL_syscond.c"    # Can be faked, if necessary
        "${SDL3_BASE_DIR}/src/thread/pthread/SDL_sysrwlock.c"   # Can be faked, if necessary
        "${SDL3_BASE_DIR}/src/thread/pthread/SDL_systls.c"
      )
      if(HAVE_PTHREADS_SEM)
        sdl_sources("${SDL3_BASE_DIR}/src/thread/pthread/SDL_syssem.c")
      else()
        sdl_sources("${SDL3_BASE_DIR}/src/thread/generic/SDL_syssem.c")
      endif()
...
macro(CheckUSBHID)
  ...
  if(HAVE_USBHID)
    ...
    sdl_glob_sources("${SDL3_BASE_DIR}/src/joystick/bsd/*.c")
...
macro(CheckHIDAPI)
  if(ANDROID)
    enable_language(CXX)
    sdl_sources("${SDL3_BASE_DIR}/src/hidapi/android/hid.cpp")
  endif()
  if(IOS OR TVOS)
    sdl_sources("${SDL3_BASE_DIR}/src/hidapi/ios/hid.m")
  ...
  if(SDL_HIDAPI)
    ...
    if(HAVE_HIDAPI)
      ...
      if(SDL_JOYSTICK AND SDL_HIDAPI_JOYSTICK)
        ...
        sdl_glob_sources("${SDL3_BASE_DIR}/src/joystick/hidapi/*.c")
      endif()
    else()
  ...
  if(SDL_RPI)
    ...
    if(TARGET PkgConfig::PC_BCM_HOST AND TARGET PkgConfig::PC_BRCMEGL)
      ...
      if(SDL_VIDEO)
        ...
        sdl_glob_sources("${SDL3_BASE_DIR}/src/video/raspberry/*.c")
...
macro(CheckKMSDRM)
  if(SDL_KMSDRM)
    ...
    if(PC_LIBDRM_FOUND AND PC_GBM_FOUND AND HAVE_OPENGL_EGL)
      ...
      sdl_glob_sources("${SDL3_BASE_DIR}/src/video/kmsdrm/*.c")
