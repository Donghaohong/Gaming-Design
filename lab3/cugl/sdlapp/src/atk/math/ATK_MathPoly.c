 ...
 */
static bool bairstow_factor(const float*poly, float* quad, float* result,
    ...
    for(int ii = 0; ii < (int)prefs->maxIterations; ii++) {
        ...
        synthetic_divide(poly, degree, quad, 2, result);
        synthetic_divide(result, degree, quad, 2, temp);
 ...
 */
size_t ATK_PolyScale(const float* poly, size_t degree,
 ...
 * @param prefs     The prefs for the algorithm
 *
 * @return true if Bairstow's method completes successfully
 */
bool ATK_PolyRoots(const float* poly, size_t degree, float* roots,
 ...
 * @return true if Bairstow's method completes successfully
 */
bool ATK_PolyRealRoots(const float* poly, size_t degree, float* roots,
                       const ATK_BairstowPrefs* prefs) {
    size_t deg, left;
    const float* src = standardize_inplace(poly, degree, &deg);
    left = degree-deg;
