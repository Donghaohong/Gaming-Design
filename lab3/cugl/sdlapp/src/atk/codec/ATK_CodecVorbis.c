 ...
 * files. We have found that this is generally the preferred format for Open
 * Source game audio.
 */
#ifdef LOAD_VORB
#pragma mark -
...
ATK_AudioSource* ATK_LoadVorbis(const char* filename) {
    ...
    return ATK_LoadVorbis_IO(stream,1);
}
 ...
 */
ATK_AudioSource* ATK_LoadVorbis_IO(SDL_IOStream* source, bool ownsrc) {
    if (source == NULL) {
 ...
 */
bool ATK_SourceIsVorbis(SDL_IOStream* source) {
    if (source == NULL) {
        return false;
    }
 ...
 */
bool ATK_Vorbis_UnloadSource(ATK_AudioSource* source) {
 ...
 */
bool ATK_Vorbis_IsSourceEOF(ATK_AudioSource* source) {
 ...
 * @return a new ATK_AudioSource from an OGG Vorbis readable/seekable IOStream
 */
ATK_AudioSource* ATK_LoadVorbis_IO(SDL_RWops* source, bool ownsrc) {
    ATK_SetError("Codec OGG Vorbis is not supported");
    return NULL;
}
 ...
 * @returns true if this is OGG Vorbis data.
 */
bool ATK_SourceIsVorbis(SDL_RWops* source) { return false; }
 ...
 * @return true if the source was successfully closed
 */
Sint32 ATK_Vorbis_UnloadSource(ATK_AudioSource* source) { return false; }
 ...
 * @return true if the audio source is at the end of the stream
 */
bool ATK_Vorbis_IsSourceEOF(ATK_AudioSource* source) { return false; }
...
ATK_AudioEncoding* ATK_EncodeVorbis(const char* filename,
    ...
    return ATK_EncodeVorbis_IO(stream,1,metadata);
}
 ...
 */
ATK_AudioEncoding* ATK_EncodeVorbis_IO(SDL_IOStream* source, bool ownsrc,
                                       const ATK_AudioMetadata* metadata) {
    ...
    while (avail) {
        size_t check = (size_t)(encoder->oggpage.header_len);
        if (SDL_WriteIO(source, encoder->oggpage.header, encoder->oggpage.header_len) < check) {
            ATK_Vorbis_FreeEncoding(result);
            return NULL;
        }
        if (SDL_WriteIO(source, encoder->oggpage.body, encoder->oggpage.body_len) < check) {
            ATK_Vorbis_FreeEncoding(result);
            return NULL;
        }
 ...
 */
bool ATK_Vorbis_FinishEncoding(ATK_AudioEncoding* encoding) {
    if (encoding == NULL) {
        return false;
    }
    ...
    while(result && vorbis_analysis_blockout(&encoder->vdsp,&encoder->vblock)==1) {
        ...
        while(result && vorbis_bitrate_flushpacket(&encoder->vdsp,&encoder->oggpacket)) {
            ...
            while (result && avail) {
                size_t check = (size_t)(encoder->oggpage.header_len);
                if (SDL_WriteIO(encoder->stream,encoder->oggpage.header,encoder->oggpage.header_len) < check) {
                    result = false;
                }
                if (SDL_WriteIO(encoder->stream,encoder->oggpage.body,encoder->oggpage.body_len) < check) {
                    result = false;
                }
 ...
 * @return a new Vorbis encoding stream to write to the given IOStream
 */
ATK_AudioEncoding* ATK_EncodeVorbis_IO(SDL_RWops* source, bool ownsrc,
                                     const ATK_AudioMetadata* metadata) {
    ATK_SetError("Codec Vorbis is not supported");
    return NULL;
}
