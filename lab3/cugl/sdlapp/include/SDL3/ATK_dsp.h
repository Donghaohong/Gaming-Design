 ...
 * the building blocks for audio effects. This features of the component are
 * inspired by the famous STK (Synthesis ToolKit):
 *
 ...
 * processing. The features in this component greatly benefit from compiling
 ...
 */
extern SDL_DECLSPEC ATK_WaveformGen* SDLCALL ATK_AllocWaveform(ATK_WaveformShape shape, double freq, double phase);
 ...
 * @return a newly allocated waveform generator for the given shape and frequency
 */
extern SDL_DECLSPEC ATK_WaveformGen* SDLCALL ATK_AllocUpperWaveform(ATK_WaveformShape shape, double freq, double phase);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeWaveform(ATK_WaveformGen* gen);

/**
 * Resets the waveform generator to its initial state
 *
 * The generator will be in the state it was when first created.  When a
 * generator is used, the first sample is guaranteed to be 0.
 *
 * @param gen   The waveform generator
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetWaveform(ATK_WaveformGen* gen);
 ...
 */
extern SDL_DECLSPEC float SDLCALL ATK_StepWaveform(ATK_WaveformGen* gen);
 ...
 *
 * @return true if data was successfully generated
 */
extern SDL_DECLSPEC bool SDLCALL ATK_FillWaveform(ATK_WaveformGen* gen, float* buffer, size_t size);
 ...
 * @return true if data was successfully generated
 */
extern SDL_DECLSPEC bool SDLCALL ATK_FillWaveform_stride(ATK_WaveformGen* gen, float* buffer,
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_GetRealFFTBestSize(size_t size);
 ...
 * true inverse on the input buffer. Note that inverse FFTs may only be used
 * by the functions {@link ATK_ApplyRealInvFFT} and
 ...
 */
extern SDL_DECLSPEC ATK_RealFFT* SDLCALL ATK_AllocRealFFT(size_t size, bool inverse);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeRealFFT(ATK_RealFFT* fft);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_GetRealFFTSize(ATK_RealFFT* fft);
 ...
 *
 * @return true if the FFT is successful
 */
extern SDL_DECLSPEC bool SDLCALL ATK_ApplyRealFFT(ATK_RealFFT* fft, const float* input, float* output);
 ...
 * @return true if the FFT is successful
 */
extern SDL_DECLSPEC bool SDLCALL ATK_ApplyRealFFT_stride(ATK_RealFFT* fft,
 ...
 * @return true if the FFT is successful
 */
extern SDL_DECLSPEC bool SDLCALL ATK_ApplyRealInvFFT(ATK_RealFFT* fft, const float* input, float* output);
 ...
 * @return true if the FFT is successful
 */
extern SDL_DECLSPEC bool SDLCALL ATK_ApplyRealInvFFT_stride(ATK_RealFFT* fft,
                                                            const float* input, size_t istride,
                                                            float* output, size_t ostride);
 ...
 * @return true if the FFT is successful
 */
extern SDL_DECLSPEC bool SDLCALL ATK_ApplyRealFFTMag(ATK_RealFFT* fft, const float* input, float* output);
 ...
 * @return true if the FFT is successful
 */
extern SDL_DECLSPEC bool SDLCALL ATK_ApplyRealFFTMag_stride(ATK_RealFFT* fft,
                                                            const float* input, size_t istride,
                                                            float* output, size_t ostride);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_GetComplexFFTBestSize(size_t size);
 ...
 */
extern SDL_DECLSPEC ATK_ComplexFFT* SDLCALL ATK_AllocComplexFFT(size_t size, bool inverse);
 ...
 * @param fft   The FFT state
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeComplexFFT(ATK_ComplexFFT* fft);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_GetComplexFFTSize(ATK_ComplexFFT* fft);
 ...
 * @param output    The FFT output
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyComplexFFT(ATK_ComplexFFT* fft, const float* input, float* output);
 ...
 * @param ostride   The output stride
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyComplexFFT_stride(ATK_ComplexFFT* fft, const float* input, size_t istride,
                                                            float* output, size_t ostride);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplySplitComplexFFT(ATK_ComplexFFT* fft, const float* realin, const float* imagin,
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplySplitComplexFFT_stride(ATK_ComplexFFT* fft,
 ...
 *
 * This type is used for FIR filters as well. The code will optimize for the
 * filter type. Filters are stateful and should be reset whenever they are
 * applied to a new audio signal.
 */
 ...
 */
extern SDL_DECLSPEC float SDLCALL ATK_BandwidthQ(float bandwidth);
 ...
 */
extern SDL_DECLSPEC float SDLCALL ATK_ShelfSlopeQ(float slope);
 ...
 */
extern SDL_DECLSPEC ATK_IIRFilter* SDLCALL ATK_AllocIIRFilter(float* a, size_t asize,
 ...
 */
extern SDL_DECLSPEC ATK_IIRFilter* SDLCALL ATK_AllocFOFilter(ATK_FOFilter type, float param);
 ...
 * Specialized filters should compute the Q factor from either
 * {@link ATK_BandwidthQ} or {@link ATK_ShelfSlopeQ}.
 *
 *
 * @param type      The filter type
 * @param freq      The normalized frequency (frequence / sample rate)
 ...
 */
extern SDL_DECLSPEC ATK_IIRFilter* SDLCALL ATK_AllocSOFilter(ATK_SOFilter type, float frequency,
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeIIRFilter(ATK_IIRFilter* filter);

/**
 * Resets the state of a IIR (infinite impulse response) filter
 *
 * IIR filters have to keep state of the inputs they have received so far. This
 * makes it not safe to use a filter on multiple streams simultaneously.
 * Resetting a filter zeroes the state so that it is the same as if the filter
 * were just allocated.
 *
 * @param filter    The IIR filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetIIRFilter(ATK_IIRFilter* filter);
 ...
 */
extern SDL_DECLSPEC float SDLCALL ATK_StepIIRFilter(ATK_IIRFilter* filter, float value);

/**
 * Applies the IIR filter to an input buffer, storing the result in output
 *
 * Both input and output should have size len. It is safe for these buffers to
 * be the same. IIR filters have to keep state of the inputs they have received
 * so far. This makes it not safe to use a filter on multiple streams
 * simultaneously.
 *
 * @param filter    The IIR filter
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyIIRFilter(ATK_IIRFilter* filter, const float* input,
                                                float* output, size_t len);
/**
 * Applies the IIR filter to an input buffer, storing the result in output
 *
 * Both input and output should have size len. It is safe for these buffers to
 * be the same assuming that the strides match. IIR filters have to keep state
 * of the inputs they have received so far. This makes it not safe to use a
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyIIRFilter_stride(ATK_IIRFilter* filter,
                                                           const float* input,  size_t istride,
 ...
 */
extern SDL_DECLSPEC ATK_DelayFilter* SDLCALL ATK_AllocDelayFilter(size_t delay);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeDelayFilter(ATK_DelayFilter* filter);
 ...
 * @param filter    The delay filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetDelayFilter(ATK_DelayFilter* filter);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_GetDelayFilterMaximum(ATK_DelayFilter* filter);

/**
 * Returns the next value of the delay filter.
 *
 * The value returned will have maximum delay. Delay filters have to keep state
 * of the inputs they have received so far, so this function moves the filter
 * forward. This makes it not safe to use a filter on multiple streams
 * simultaneously.
 *
 * @param filter    The delay filter
 * @param value     The input value
 ...
 */
extern SDL_DECLSPEC float SDLCALL ATK_StepDelayFilter(ATK_DelayFilter* filter, float value);
 ...
 */
extern SDL_DECLSPEC float SDLCALL ATK_TapOutDelayFilter(ATK_DelayFilter* filter, size_t tap);
 ...
 * modify the filter at the given position, but does not move it forward (so
 * calls to {@link ATK_StepDelayFilter} are unaffected if this tap is not at
 * the end).
 *
 * @param filter    The delay filter
 * @param tap       The tap position
 ...
 * @return the value in this filter with the given tap position.
 */
extern SDL_DECLSPEC void SDLCALL ATK_TapInDelayFilter(ATK_DelayFilter* filter, size_t tap, float value);
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyDelayFilter(ATK_DelayFilter* filter, const float* input,
                                                      float* output, size_t len);
/**
 * Applies the delay to an input buffer, storing the result in output
 *
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyDelayFilter_stride(ATK_DelayFilter* filter,
                                                             const float* input,  size_t istride,
                                                             float* output, size_t ostride, size_t len);
 ...
 * the last len delayed values will be lost if tap is less than the maximum
 * delay. If you want to have a delay less than the maximum delay without losing
 * state, you should use {@link ATK_TapOutDelayFilter}.
 *
 * @param filter    The delay filter
 * @param input     The input buffer
 * @param output    The output buffer
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_TapApplyDelayFilter(ATK_DelayFilter* filter, const float* input,
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_TapApplyDelayFilter_stride(ATK_DelayFilter* filter,
                                                                const float* input,  size_t istride,
                                                                float* output, size_t ostride,
 ...
 */
extern SDL_DECLSPEC ATK_FractionalFilter* SDLCALL ATK_AllocFractionalFilter(float delay, int allpass);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeFractionalFilter(ATK_FractionalFilter* filter);
 ...
 * @param filter    The fractional delay filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetFractionalFilter(ATK_FractionalFilter* filter);
 ...
 * @return the maximum delay supported by this filter.
 */
extern SDL_DECLSPEC float SDLCALL ATK_GetFractionalFilterDelay(ATK_FractionalFilter* filter);
 ...
 * @return the next value of the delay filter.
 */
extern SDL_DECLSPEC float SDLCALL ATK_StepFractionalFilter(ATK_FractionalFilter* filter, float value);

/**
 * Returns the value in this filter with the given tap position.
 *
 * The value tap should be less than the maximum delay. This function does not
 * modify the filter or move it forward (e.g. the state is unchanged). Note
 ...
 * @return the value in this filter with the given tap position.
 */
extern SDL_DECLSPEC float SDLCALL ATK_TapOutFractionalFilter(ATK_FractionalFilter* filter, float tap);
 ...
 * @return the value in this filter with the given tap position.
 */
extern SDL_DECLSPEC void SDLCALL ATK_TapInFractionalFilter(ATK_FractionalFilter* filter, size_t tap, float value);
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyFractionalFilter(ATK_FractionalFilter* filter, const float* input,
                                                           float* output, size_t len);
/**
 * Applies the delay to an input buffer, storing the result in output
 *
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyFractionalFilter_stride(ATK_FractionalFilter* filter,
                                                                  const float* input,  size_t istride,
                                                                  float* output, size_t ostride, size_t len);
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_TapApplyFractionalFilter(ATK_FractionalFilter* filter, const float* input,
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_TapApplyFractionalFilter_stride(ATK_FractionalFilter* filter,
                                                                     const float* input,  size_t istride,
                                                                     float* output, size_t ostride,
 ...
 */
extern SDL_DECLSPEC ATK_AllpassFilter* SDLCALL ATK_AllocAllpassFilter(size_t delay, float feedback);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeAllpassFilter(ATK_AllpassFilter* filter);
 ...
 * @param filter    The allpass filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetAllpassFilter(ATK_AllpassFilter* filter);
 ...
 * @param feedback  The feedback coefficient
 */
extern SDL_DECLSPEC void SDLCALL ATK_UpdateAllpassFilter(ATK_AllpassFilter* filter, float feedback);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_GetAllpassFilterDelay(ATK_AllpassFilter* filter);
 ...
 */
extern SDL_DECLSPEC float SDLCALL ATK_StepAllpassFilter(ATK_AllpassFilter* filter, float value);
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyAllpassFilter(ATK_AllpassFilter* filter, const float* input,
                                                        float* output, size_t len);
/**
 * Applies the filter to an input buffer, storing the result in output
 *
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyAllpassFilter_stride(ATK_AllpassFilter* filter,
                                                               const float* input,  size_t istride,
                                                               float* output, size_t ostride, size_t len);
 ...
 *
 * This filter has an integral delay, like {@link ATK_DelayFilter}. However, it
 * has additional feedforward and feedback coefficients to introduce
 * interferance in the signal. Because of this interferance, we do not allow
 * comb filters to be tapped in or out like a normal delay filter.
 */
 ...
 *
 * The delay of this filter can never be resized. However, the coefficients can
 * be updated at any time with {@link ATK_UpdateCombFilter}.
 *
 * @param delay     The delay length
 * @param feedback  The feedback coefficient
 ...
 */
extern SDL_DECLSPEC ATK_CombFilter* SDLCALL ATK_AllocCombFilter(size_t delay, float feedback, float damping);
 ...
 * @param filter    The comb filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeCombFilter(ATK_CombFilter* filter);
 ...
 * @param filter    The comb filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetCombFilter(ATK_CombFilter* filter);
 ...
 * @param damping   The filter damping factor
 */
extern SDL_DECLSPEC void SDLCALL ATK_UpdateCombFilter(ATK_CombFilter* filter, float feedback, float damping);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_GetCombFilterDelay(ATK_CombFilter* filter);
 ...
 * @return the next value of the comb filter.
 */
extern SDL_DECLSPEC float SDLCALL ATK_StepCombFilter(ATK_CombFilter* filter, float value);
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyCombFilter(ATK_CombFilter* filter, const float* input,
                                                     float* output, size_t len);
/**
 * Applies the filter to an input buffer, storing the result in output
 *
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyCombFilter_stride(ATK_CombFilter* filter,
                                                            const float* input,  size_t istride,
                                                            float* output, size_t ostride, size_t len);
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_AddCombFilter(ATK_CombFilter* filter, const float* input,
                                                   float* output, size_t len);
/**
 * Applies the filter to an input buffer, adding the result to output
 *
 ...
 * @param len       The number of elements in the buffers
 */
extern SDL_DECLSPEC void SDLCALL ATK_AddCombFilter_stride(ATK_CombFilter* filter,
                                                          const float* input,  size_t istride,
                                                          float* output, size_t ostride, size_t len);
 ...
 * @returns a newly allocated Blackman window.
 */
extern SDL_DECLSPEC float* SDLCALL ATK_AllocBlackmanWindow(size_t size, int half);
 ...
 * @param half      Whether to restrict to a half window
 */
extern SDL_DECLSPEC void SDLCALL ATK_FillBlackmanWindow(float* buffer, size_t size, int half);
 ...
 * @returns a newly allocated Hamming window.
 */
extern SDL_DECLSPEC float* SDLCALL ATK_AllocHammingWindow(size_t size, int half);
 ...
 * @param half      Whether to restrict to a half window
 */
extern SDL_DECLSPEC void SDLCALL ATK_FillHammingWindow(float* buffer, size_t size, int half);
 ...
 * @returns a newly allocated Hann window.
 */
extern SDL_DECLSPEC float* SDLCALL ATK_AllocHannWindow(size_t size, int half);
 ...
 * @param half      Whether to restrict to a half window
 */
extern SDL_DECLSPEC void SDLCALL ATK_FillHannWindow(float* buffer, size_t size, int half);
 ...
 * @returns a newly allocated Kaiser window.
 */
extern SDL_DECLSPEC float* SDLCALL ATK_AllocKaiserWindow(size_t size, float beta, int half);
 ...
 * @param half      Whether to restrict to a half window
 */
extern SDL_DECLSPEC void SDLCALL ATK_FillKaiserWindow(float* buffer, size_t size, float beta, int half);
 ...
 */
extern SDL_DECLSPEC ATK_Convolution* SDLCALL ATK_AllocConvolution(float* kernel, size_t size, size_t block);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeConvolution(ATK_Convolution* filter);
 ...
 * @param filter    The convolution filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetConvolution(ATK_Convolution* filter);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_ScaleConvolution(ATK_Convolution* filter, float scalar);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_GetConvolutionSize(ATK_Convolution* filter);
 ...
 */
extern SDL_DECLSPEC float SDLCALL ATK_StepConvolution(ATK_Convolution* filter, float value);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyNaiveConvolution(ATK_Convolution* filter, const float* input,
                                                           float* output, size_t len);
 ...
 * @param len       The number of elements to process
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyNaiveConvolution_stride(ATK_Convolution* filter,
                                                                  const float* input, size_t istride,
                                                                  float* output, size_t ostride, size_t len);
 ...
 * @param len       The number of elements to process
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyFFTConvolution(ATK_Convolution* filter, const float* input,
                                                         float* output, size_t len);
 ...
 * @param len       The number of elements to process
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyFFTConvolution_stride(ATK_Convolution* filter,
                                                                const float* input, size_t istride,
                                                                float* output, size_t ostride, size_t len);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_FinishConvolution(ATK_Convolution* filter, float* buffer);
 ...
 * @return the number of elements stored in the buffer.
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_FinishConvolution_stride(ATK_Convolution* filter,
