 ...
 * orientation of the device. The accelerometer input is normalized for all
 * devices so that portrait is the default interface. That means that the
 * x-axis is the short edge of the device and the y-axis is the long edge of
 * the device. Note that this is not the native accelerometer interface for
 * some Android devices. See the method {@link #isSwapped}.
 *
 * As with most devices, we provide support for both listeners and polling the
 * mouse. Polling the accelerometer will query the rotational axes at the start 
 * of the frame. The advantage of listeners is that they are a lot less 
 * frequent. If the acceleration does not change significantly from frame to 
 * frame, no event will be generated. See the method
...
private:
    /** The SDL id for the accelerometer */
    SDL_Sensor* _input;
    ...
    /** Whether the x and y axis are swapped */
    bool _swapped;
    /** A scaling factor to support Android large-display behavior */
    Vec3 _scale;
...
#pragma mark Calibration
     ...
     *
     * As the accelerometer is normalized, in portrait, the x-axis is the
     * short edge. Setting this value to landscape will swap the x and y
     * axes so that the x-axis is the long edge and the y-axis is the short
     * edge. By default, the reference frame is portrait.
     *
    ...
    /**
     * Returns true if this accelerometer is swapped relative to its native implementation.
     *
     * For some Android devices, the device is landscape-native. On such a
     * device, the x and y axes of the acelerometer are swapped. The x-axis is
     * the long edge and the y-axis is the short edge. This method returns
     * true if this is the case.
     *
     * Note that this method is not affected by {@link #setReferenceFrame}. It
     * is only an indication of the default behavior of this object relative
     * to the native accelerometer.
     *
     * @return true if this accelerometer is swapped relative to its native implementation.
     */
    bool isSwapped() const { return _swapped; }
    
    // Allow the display to control the scale factor
