...
#pragma mark Constructors
     ...
     */
    static std::shared_ptr<TextLayout> allocWithTextWidth(const std::string text,
                                                          const std::shared_ptr<Font>& font, float width) {
        std::shared_ptr<TextLayout> result = std::make_shared<TextLayout>();
...
#pragma mark Glyph Querying
     ...
     */
    size_t getGlyphIndex(const Vec2& p) const { return getGlyphIndex(p.x,p.y); }
     ...
     */
    size_t getNearestIndex(const Vec2& p) const { return getNearestIndex(p.x,p.y); }
...
#pragma mark Glyph Generation
     ...
     */
    std::unordered_map<size_t,std::shared_ptr<GlyphRun>> getGlyphs() const;
     ...
     * @return a set of glyph runs to render the text layout
     */
    std::unordered_map<size_t,std::shared_ptr<GlyphRun>> getGlyphs(const Rect& rect) const;
     ...
     */
    size_t getGlyphs(std::unordered_map<size_t,std::shared_ptr<GlyphRun>>& runs) const;
...
#pragma mark Glyph Debugging
     ...
     * @return a (line) mesh of the quad outlines for the text glyphs
     */
    Mesh<SpriteVertex> getGlyphBoxes(const Rect& rect) const;
     ...
     * @return the number of quads generated
     */
    size_t getGlyphBoxes(Mesh<SpriteVertex>& mesh, const Rect& rect) const;
