 ...
 * whenever possible. However this sprite batch is different from a classic 
 * sprite batch (from XNA or LibGDX) in that it provides a complete 2d graphics 
 * pipeline supporting both solid shapes and outlines, with texture, gradient, 
 * and scissor mask support.
 *
 ...
 * the default color is white. Outlines use the same texturing rules that solids 
 * do. There is also support for a simple, limited radius blur effect on 
 * textures.
 *
 * Color gradient support is provided by the {@link Gradient} class. All 
 * gradients will be tinted by the current color (so the color should be reset 
 * to white before using a gradient).
 *
 * Scissor masks are supported by the {@link Scissor} class. This is useful for
 * constraining shapes to an internal window. A scissor mask must be a 
 * transformed rectangle; it cannot mask with arbitrary polygons.
 *
 ...
 *
 * A review of this class shows that there are a lot of redundant drawing 
 * methods. The scene graphs only use the {@link Mesh} methods. This goal has 
 * been to make this class more accessible to students familiar with classic 
 * sprite batches found in LibGDX or XNA.
 *
 ...
 *
 * This is an extremely heavy-weight class. There is rarely any need to have 
 * more than one of these at a time. If you want to implement your own shader 
 * effects, it is better to construct your own custom pipeline with 
 * {@link GraphicsShader} and {@link VertexBuffer}.
 */
class SpriteBatch {
#pragma mark Values
public:
...
private:
    ...
    std::shared_ptr<GraphicsShader> _shader;
    /** The vertex buffer for this sprite batch */
    std::shared_ptr<VertexBuffer>  _vertbuff;
    /** The index buffer for this sprite batch */
    std::shared_ptr<IndexBuffer>   _indxbuff;
    /** The uniform buffer for this sprite batch */
    std::shared_ptr<UniformBuffer> _unifbuff;
    /** The active framebuffer (for offscreen rendering) */
    std::shared_ptr<FrameBuffer> _framebuff;
...
#pragma mark Attributes
     ...
     */
    Uint32 getVerticesDrawn() const { return _vertTotal; }
     ...
     */
    Uint32 getCallsMade() const { return _callTotal; }
     ...
     */
    const std::shared_ptr<GraphicsShader>& getShader() const { return _shader; }
     ...
     *
     * This method is a convenience method that calls {@link #setPerspective}
     * on the combined perspective matrix of the camera.
     *
     * @param camera    The camera providing the perspective matrix
     */
    void setCamera(const std::shared_ptr<Camera>& camera) {
        setPerspective(camera->getCombined());
    }
     ...
     *
     * By default this value is {@link BlendMode#PREMULT}, as image files are
     * loaded with premultiplied alpha. You cannot determine the blend mode for
     * a spritebatch after it is set. You can only get the blend state for that
     * mode with the method {@link #getBlendState}.
     *
     * @param mode  The color blend mode for this spritebatch
     */
    void setBlendMode(const BlendMode& mode) {
        BlendState blend(mode);
        setBlendState(blend);
    }
     ...
     *
     * @param blend The color blend state for this spritebatch
     */
...
private:
     ...
     */
    void blurTexture(const std::shared_ptr<Texture>& texture, float step);
     ...
     */
    Uint32 prepare(const Rect& rect);
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 prepare(const Rect& rect, const Affine2& mat);
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 prepare(const Poly2& poly);
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 prepare(const Poly2& poly, const Vec2& off);
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 prepare(const Poly2& poly, const Affine2& mat);
    
    /**
     * Returns the number of vertices added to the drawing buffer.
     *
     * This method is an alternate version of {@link #prepare} for the same
     * arguments. It breaks up the drawing into chunks for cases in which the
     * polygon exceeds the spritebatch capacity. In the case of OpenGL, this
     * is implemented as multiple drawing passes. In the case of Vulkan, this
     * may require the creation of addition vertex buffers. For that reason, 
     * it is possible that this method will fail (returning 0).
     *
     * All vertices will be uniformly transformed by the transform matrix.
     *
     * @param poly  The polygon to add to the buffer
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 chunkify(const Poly2& poly, const Affine2& mat);
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 prepare(const Mesh<SpriteVertex>& mesh, const Affine2& mat, bool tint = true);
     ...
     * This method is an alternate version of {@link #prepare} for the same
     * arguments. It breaks up the drawing into chunks for cases in which the
     * mesh exceeds the spritebatch capacity. In the case of OpenGL, this
     * is implemented as multiple drawing passes. In the case of Vulkan, this
     * may require the creation of addition vertex buffers. For that reason, 
     * it is possible that this method will fail (returning 0).
     *
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 chunkify(const Mesh<SpriteVertex>& mesh, const Affine2& mat, bool tint = true);
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 prepare(const SpriteVertex* vertices, size_t size, const Affine2& mat, bool tint = true);
     ...
     * This method is an alternate version of {@link #prepare} for the same
     * arguments. It breaks up the drawing into chunks for cases in which the
     * number of vertices exceeds the spritebatch capacity. In the case of 
     * OpenGL, this is implemented as multiple drawing passes. In the case of 
     * Vulkan, this may require the creation of addition vertex buffers. For 
     * that reason, it is possible that this method will fail (returning 0).
     *
     * @param vertices  The vertices to draw as a triangle fan
     * @param size      The number of vertices in the fan
     * @param mat       The transform to apply to the vertices
     ...
     * @return the number of vertices added to the drawing buffer.
     */
    Uint32 chunkify(const SpriteVertex* vertices, size_t size, const Affine2& mat, bool tint = true);
