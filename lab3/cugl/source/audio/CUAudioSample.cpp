...
bool AudioSample::init(const std::string file, bool stream) {
    if (!filetool::is_absolute(file)) {
        _path = Application::get()->getAssetDirectory();
        _path.append(file);
        _path = filetool::canonicalize_path(_path);
    } else {
        _path = filetool::canonicalize_path(file);
    }

    if (!filetool::file_exists(_path)) {
        CULogError("Cannot find file %s",_path.c_str());
        return false;
    }
...
std::shared_ptr<AudioDecoder> AudioSample::getDecoder() {
    return AudioDecoder::alloc(_path,_type);
}
