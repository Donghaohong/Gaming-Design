...
static void audio_callback(void*  userdata, SDL_AudioStream *stream, int additional, int total) {
    ...
    available = std::min(total,available);
    
    if (available > 0) {
        Uint8 *data = SDL_stack_alloc(Uint8, available);
        if (data) {
            while (available > 0) {
                int count = SDL_GetAudioStreamData(stream,data,available);
                if (count <= 0) {
                    break;
                }
                device->push(data,count);
                available -= count;
            }
        }
        SDL_stack_free(data);
...
/**
 * Returns the estimated sample period (in frames) for the given stream
 *
 ...
 * is a capture device that is resampling.
 *
...
static Uint32 period_estimate(SDL_AudioDeviceID device, SDL_AudioStream* stream) {
    ...
    double ratio  = (double)out.freq / (double)in.freq;
...
bool AudioInput::init() {
    ...
    return init(SDL_AUDIO_DEVICE_DEFAULT_RECORDING,
                DEFAULT_CHANNELS,DEFAULT_SAMPLING,read,write);
}
...
bool AudioInput::init(Uint8 channels, Uint32 rate) {
    ...
    Uint32 read = AudioDevices::get()->getReadSize();
    Uint32 write = AudioDevices::get()->getWriteSize();
    return init(SDL_AUDIO_DEVICE_DEFAULT_RECORDING,channels,rate,read,write);
}
...
bool AudioInput::init(Uint8 channels, Uint32 rate, Uint32 readsize, Uint32 writesize) {
    return init(SDL_AUDIO_DEVICE_DEFAULT_RECORDING,channels,rate,readsize,writesize);
}
 ...
 * @return true if initialization was successful
 */
bool AudioInput::init(SDL_AudioDeviceID device) {
    CUAssertLog(AudioDevices::get(),"Attempt to allocate a node without an active audio device manager");
    Uint32 read = AudioDevices::get()->getReadSize();
    Uint32 write = AudioDevices::get()->getWriteSize();
 ...
 * @return true if initialization was successful
 */
bool AudioInput::init(SDL_AudioDeviceID device, Uint8 channels, Uint32 rate,
                      Uint32 readsize, Uint32 writesize) {
    
    if (SDL_IsAudioDevicePlayback(device)) {
        CUAssertLog(false, "Device is not a recording device");
        return false;
    }
...
bool AudioInput::reopenDevice() {
    ...
    _dvname = SDL_GetAudioDeviceName(_device);

    // Get the buffer size
    _writesize = period_estimate(_device,_stream);
...
Uint32 AudioInput::record(float* buffer, Uint32 frames) {
    if (_timeout.load(std::memory_order_relaxed) && _record.load(std::memory_order_relaxed)) {
        ...
        if (_playmark >= 0) {
            ...
            _playback.insert(_playback.end(), buffer, buffer+temp);
        }
        while (temp) {
            if (_bufftail+temp > _capacity) {
                std::memcpy(_buffer+_bufftail*_channels,input,
                            (_capacity-_bufftail)*_channels*sizeof(float));
...
/**
 * Records the specified number of bytes to this audio node
 *
 * AUDIO THREAD ONLY: Users should never access this method directly.
 * The only exception is when the user needs to create a custom subclass
 * of this AudioOutput.
 ...
 */
Uint32 AudioInput::push(Uint8* stream, int len) {
    float* data = reinterpret_cast<float*>(stream);
    Uint32 frames = len/(sizeof(float)*_channels);
    record(data, frames);
    return len;
}
