...
bool AudioDecoder::init(const std::string file, AudioType type) {
    ...
    if (!filetool::is_absolute(file)) {
        path = Application::get()->getAssetDirectory();
        path.append(file);
        path = filetool::canonicalize_path(path);
    } else {
        path = filetool::canonicalize_path(file);
    }
    
    switch(type) {
    case AudioType::WAV_FILE:
        _source = ATK_LoadWAV(path.c_str());
        break;
    case AudioType::MP3_FILE:
        _source = ATK_LoadMP3(path.c_str());
        break;
    case AudioType::OGG_FILE:
        _source = ATK_LoadVorbis(path.c_str());
        break;
    case AudioType::FLAC_FILE:
        _source = ATK_LoadFLAC(path.c_str());
        break;
    default:
