...
/**
 * Initializes a wave form with the given JSON specificaton.
 *
 ...
 * @return true if initialization was successful
 */
bool AudioWaveform::initWithData(const std::shared_ptr<JsonValue>& data) {
    ...
    bool success = init(channels,sampling,type,frequency);
    if (success) {
        setUpper(data->getBool("upper",false));
        setDuration(data->getFloat("duration",-1));
    }
    return success;
}
