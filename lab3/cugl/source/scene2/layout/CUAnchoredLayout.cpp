        ...
        switch (child->getLayoutConstraint()) {
            case SceneNode::Constraint::LAYOUT:
            {
                ...
                if (jt != _entries.end()) {
                    ...
                    Vec2 offset;
                    offset.x = entry.absolute ? entry.x_offset : entry.x_offset*bounds.size.width;
                    offset.y = entry.absolute ? entry.y_offset : entry.y_offset*bounds.size.height;
                    placeNode(child, entry.anchor, bounds, offset);
                }
            }
                break;
