...
Layout::Anchor Layout::getAnchor(const std::string x_anchor, const std::string y_anchor) {
    ...
    if (x_anchor == "left") {
        ...
        } else if (y_anchor == "top+bottom" || y_anchor == "scale") {
    ...
    } else if (x_anchor == "left+right" || x_anchor == "scale"){
        if (y_anchor == "top") {
            anchor = Anchor::TOP_FILL;
        } else if (y_anchor == "bottom"){
            anchor = Anchor::BOTTOM_FILL;
 ...
 * {@link GridLayout}.
 *
 * The value offset should be given in coordinates, and not percentages
 * (e.g. absolute). The offset is added to the position, so positive
 * offsets have the effect of shifting the node to up and the right.
 *
 ...
 */
void Layout::placeNode(SceneNode* node, Anchor anchor, const Rect& bounds,
                       const Vec2& offset) {
    Vec2 spot;
    Vec2 adjust = offset;
    switch (anchor) {
        ...
        case Anchor::LEFT_FILL:
            ...
            if (!node->getNodeToParentTransform().isIdentity()){
                Rect box = node->getBoundingBox();
                box.size.height = bounds.size.height-2*adjust.y;
                box = node->getParentToNodeTransform().transform(box);
                node->setContentSize(box.size);
            } else {
        ...
        case Anchor::BOTTOM_FILL:
            ...
            if (!node->getNodeToParentTransform().isIdentity()){
                Rect box = node->getBoundingBox();
                box.size.width = bounds.size.width-2*adjust.x;
                box = node->getParentToNodeTransform().transform(box);
                node->setContentSize(box.size);
            } else {
        ...
        case Anchor::TOTAL_FILL:
            ...
            if (!node->getNodeToParentTransform().isIdentity()){
                ...
                node->setContentSize(bounds.size.width-2*adjust.x,
                                     bounds.size.height-2*adjust.y);
            }
            adjust = Vec2::ZERO;
            break;
    }
 ...
 * {@link GridLayout}.
 *
 * The two offsets specify the distance of the node from the bottom-left
 * corner and the top-right corner of the parent node, respectively. This
 * is useful for fills where we want to resize the node with the parent,
 * but still want to preserve edge distances from the parent.
 *
 * @param node      The node to reposition
 * @param anchor    The anchor rule for this node
 * @param bounds    The area to compute the position from
 * @param offbl     The offset from the bottom left corner of the parent
 * @param offtr     The offset from the top right corner of the parent
 */
void Layout::placeNode(SceneNode* node, Anchor anchor, const Rect& bounds,
                       const Vec2& offbl, const Vec2& offtr) {
    Vec2 spot;
    switch (anchor) {
        case Anchor::TOP_LEFT:
            spot.set(0, bounds.size.height);
        ...
        case Anchor::LEFT_FILL:
            ...
            if (!node->getNodeToParentTransform().isIdentity()){
                Rect box = node->getBoundingBox();
                box.size.height = bounds.size.height-(offbl.y + offtr.y);
                box = node->getParentToNodeTransform().transform(box);
                node->setContentSize(box.size);
            } else {
                node->setContentHeight(bounds.size.height-(offbl.y + offtr.y));
            }
            spot.x += offbl.x;
            break;
        case Anchor::CENTER_FILL:
        ...
        case Anchor::BOTTOM_FILL:
            ...
            if (!node->getNodeToParentTransform().isIdentity()){
                Rect box = node->getBoundingBox();
                box.size.width = bounds.size.width-(offbl.x + offtr.x);
                box = node->getParentToNodeTransform().transform(box);
                node->setContentSize(box.size);
            } else {
                node->setContentWidth(bounds.size.width-(offbl.x + offtr.x));
            }
            spot.y += offbl.y;
            break;
        case Anchor::MIDDLE_FILL:
        ...
        case Anchor::TOTAL_FILL:
            ...
            if (!node->getNodeToParentTransform().isIdentity()){
                ...
                node->setContentSize(bounds.size.width-(offbl.x + offtr.x),
                                     bounds.size.height-(offbl.y + offtr.y));
            }
            break;
    }
