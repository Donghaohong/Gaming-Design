...
bool BinaryWriter::init(const std::string file, unsigned int capacity) {
    ...
    _stream = SDL_IOFromFile(_name.c_str(), "w");
    if (!_stream) {
...
void BinaryWriter::flush() {
    size_t amt = SDL_WriteIO(_stream, _cbuffer, _bufoff);
 ...
 */
void BinaryWriter::write(const char* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
    ...
    while (length-pos > _capacity-_bufoff) {
        memcpy(&(_cbuffer[_bufoff]), array+pos, _capacity-_bufoff);
        pos += _capacity-_bufoff;
        _bufoff = _capacity;
        flush();
    }
    
    memcpy(&(_cbuffer[_bufoff]), array+pos, length-pos);
 ...
 */
void BinaryWriter::write(const Uint8* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
 ...
 */
void BinaryWriter::write(const Sint16* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
 ...
 */
void BinaryWriter::write(const Uint16* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
 ...
 */
void BinaryWriter::write(const Sint32* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
 ...
 */
void BinaryWriter::write(const Uint32* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
 ...
 */
void BinaryWriter::write(const Sint64* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
 ...
 */
void BinaryWriter::write(const Uint64* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
 ...
 */
void BinaryWriter::write(const float* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
 ...
 */
void BinaryWriter::write(const double* array, size_t length) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
...
/**
 * Writes the bytes from a vector to a binary file.
 *
 * The array is written to the internal buffer, but is not necessarily
 * flushed automatically.  It will be written when the buffer reaches
 * capacity or the file is closed.
 */
void BinaryWriter::writeBytes(const std::vector<std::byte>& bytes) {
    CUAssertLog(_stream, "Attempt to write to a closed stream");
    size_t pos = 0;
    size_t length = bytes.size()*sizeof(std::byte);
    if (_bufoff+length > _capacity) {
        flush();
    }
    while (length-pos > _capacity-_bufoff) {
        memcpy(&(_cbuffer[_bufoff]), bytes.data()+pos,_capacity-_bufoff);
        pos += _capacity-_bufoff;
        _bufoff = _capacity;
        flush();
    }
        
    memcpy(&(_cbuffer[_bufoff]), bytes.data()+pos, length-pos);
    _bufoff += (Sint32)(length-pos);
}
