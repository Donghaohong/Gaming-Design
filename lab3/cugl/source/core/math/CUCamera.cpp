 ...
 */
void Camera::setUp(const Vec3& vec) {
    Vec3 temp = vec;
    temp.normalize();
    if (!temp.isZero()) {
        // Up and direction must ALWAYS be orthonormal vectors
...
Vec3 Camera::unproject(const Vec3& windowCoords) const {
    return unproject(windowCoords, Rect(Vec2::ZERO,Application::get()->getDrawableSize()));
}
...
Vec3 Camera::project(const Vec3& worldCoords) const {
    return project(worldCoords, Rect(Vec2::ZERO,Application::get()->getDrawableSize()));
}
...
Vec2 Camera::windowToScreenCoords(const Vec3& windowCoords) const {
    return windowToScreenCoords(windowCoords, Rect(Vec2::ZERO,Application::get()->getDrawableSize()));
}
...
Vec3 Camera::screenToWindowCoords(const Vec2&screenCoords) const {
    return screenToWindowCoords(screenCoords, Rect(Vec2::ZERO,Application::get()->getDrawableSize()));
}
