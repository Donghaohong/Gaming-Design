 ...
 */
void Obstacle::setPhysicsUnits(float units) {
    _units = units;
    resetMesh();
}
 ...
 * The wireframe will consist of the physics fixtures adjusted by
 * {@link #getPhysicsUnits}. For proper results, all obstacles should
 * have the same physics units.
 */
const Mesh<SpriteVertex>& Obstacle::getDebugMesh() {
 ...
 */
const Affine2 Obstacle::getGraphicsTransform() const {
    Affine2 transform;
