...
//  Version: 1/4/26
//
#include <cugl/physics2/lights/CUPointLight.h>
...
void PointLight::initRays() {
    ...
    _fractions.clear();
    _fractions.resize(_rayNum,1);
    
    float angleNum = 360.0f / _rayNum;
    for (int ii = 0; ii < _rayNum; ii++) {
        float a = M_PI*(_angle + angleNum * ii)/180.0f;
        float c = cosf(a);
        float s = sinf(a);
 ...
 */
bool PointLight::contains(const Vec2& p) {
    // fast fail
