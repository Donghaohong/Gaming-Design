...
ObstacleScene NetPhysicsController::addSharedObstacle(Uint32 factoryID,
    ...
    _outEvents.push_back(PhysicsObstacleEvent::allocCreation(factoryID,objId,bytes));
    return pair;
}
...
void NetPhysicsController::removeSharedObstacle(std::shared_ptr<physics2::Obstacle> obj) {
    ...
    if (map.count(obj)) {
        ...
        _outEvents.push_back(PhysicsObstacleEvent::allocDeletion(objId));
...
void NetPhysicsController::acquireObs(std::shared_ptr<physics2::Obstacle> obs, Uint64 duration){
    ...
    auto event = PhysicsObstacleEvent::allocOwnerAcquire(id, duration);
...
void NetPhysicsController::releaseObs(std::shared_ptr<physics2::Obstacle> obs) {
    if (!_isHost) {
        ...
        Uint64 id = _world->getObstacleIds().at(obs);
        auto event = PhysicsObstacleEvent::allocOwnerRelease(id);
        _outEvents.push_back(event);
    }
}
...
void NetPhysicsController::updateSimulation() {
    ...
    if (_itprDebug) {
        ...
        CULog("Average step: %g", ((float)_stepSum)/_itprCount);
    }
}
 ...
 */
void NetPhysicsController::processPhysicsObstacleEvent(
                            const std::shared_ptr<PhysicsObstacleEvent>&event) {
    if (event->getSourceId() == "")
        return; // Ignore physic syncs from self.

    if (event->getType() == PhysicsObstacleEvent::EventType::CREATION) {
    ...
    if (event->getType() == PhysicsObstacleEvent::EventType::DELETION) {
        _cache.erase(obj);
        _world->removeObstacle(obj);
        if (_sharedObsToNodeMap.count(obj)) {
            _sharedObsToNodeMap.at(obj)->removeFromParent();
    ...
    switch (event->getType()) {
        case PhysicsObstacleEvent::EventType::BODY_TYPE:
			obj->setBodyType(event->getBodyType());
			break;
        case PhysicsObstacleEvent::EventType::POSITION:
            obj->setPosition(event->getPosition());
            break;
        case PhysicsObstacleEvent::EventType::VELOCITY:
            obj->setLinearVelocity(event->getLinearVelocity());
            break;
        case PhysicsObstacleEvent::EventType::ANGLE:
			obj->setAngle(event->getAngle());
			break;
        case PhysicsObstacleEvent::EventType::ANGULAR_VEL:
            obj->setAngularVelocity(event->getAngularVelocity());
            break;
        case PhysicsObstacleEvent::EventType::BOOL_CONSTS:
			...
			break;
        case PhysicsObstacleEvent::EventType::FLOAT_CONSTS:
            ...
            break;
        case PhysicsObstacleEvent::EventType::OWNER_ACQUIRE:
            _world->getOwnedObstacles().erase(obj);
            //CULog("Erased ownership for %llu",event->getObjId());
            break;
        case PhysicsObstacleEvent::EventType::OWNER_RELEASE:
            if (_isHost) {
 ...
 */
void NetPhysicsController::processPhysicsSyncEvent(
                            const std::shared_ptr<PhysicsSyncEvent>& event) {
    if (event->getSourceId() == "") {
        return; // Ignore physic syncs from self.
    }
    const std::vector<PhysicsSyncEvent::Parameters>& params = event->getSyncList();
    for (auto it = params.begin(); it != params.end(); it++) {
        PhysicsSyncEvent::Parameters param = (*it);
 ...
 */
void NetPhysicsController::packPhysicsSync(SyncType type) {
    auto event = PhysicsSyncEvent::alloc();
    ...
    for (auto it = objs.begin(); it != objs.end(); it++) {
        ...
        if (obj->isShared()) {
            if (obj->hasDirtyPosition()) {
                _outEvents.push_back(PhysicsObstacleEvent::allocPos(id,obj->getPosition()));
            }
            if (obj->hasDirtyAngle()) {
				_outEvents.push_back(PhysicsObstacleEvent::allocAngle(id,obj->getAngle()));
			}
            if (obj->hasDirtyVelocity()) {
				_outEvents.push_back(PhysicsObstacleEvent::allocVel(id,obj->getLinearVelocity()));
			}
            if (obj->hasDirtyAngularVelocity()) {
				_outEvents.push_back(PhysicsObstacleEvent::allocAngularVel(id,obj->getAngularVelocity()));
			}
            if (obj->hasDirtyType()) {
                _outEvents.push_back(PhysicsObstacleEvent::allocBodyType(id,obj->getBodyType()));
            }
            if (obj->hasDirtyBool()) {
                PhysicsObstacleEvent::BoolConsts values;
				...
				_outEvents.push_back(PhysicsObstacleEvent::allocBoolConsts(id,values));
			}
            if (obj->hasDirtyFloat()) {
                PhysicsObstacleEvent::FloatConsts values;
                ...
                _outEvents.push_back(PhysicsObstacleEvent::allocFloatConsts(id,values));
            }
