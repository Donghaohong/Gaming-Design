...
//
#include <cugl/physics2/distrib/CUPhysicsSyncEvent.h>
...
PhysicsSyncEvent::Parameters::Parameters() {
    obsId = 0;
    x = 0;
    y = 0;
    vx = 0;
 ...
 */
void PhysicsSyncEvent::addObstacle(Uint64 id, const std::shared_ptr<physics2::Obstacle>& obs) {
    if (_obsSet.count(id)) {
        return;
    }
 ...
 */
std::vector<std::byte> PhysicsSyncEvent::serialize() {
    _serializer.reset();
 ...
 */
void PhysicsSyncEvent::deserialize(const std::vector<std::byte>& data) {
    if (data.size() < 4) {
        return;
    }
