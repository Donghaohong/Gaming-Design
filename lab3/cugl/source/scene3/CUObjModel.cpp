...
void ObjMesh::computeTangents() {
    int start = 0;
    int step = 0;
    if (_mesh.command == DrawMode::TRIANGLES) {
        start = 0;
        step = 3;
    } else if (_mesh.command == DrawMode::TRIANGLE_STRIP ||
               _mesh.command == DrawMode::TRIANGLE_FAN) {
        start = 2;
        step = 1;
    } else {
...
bool ObjMesh::createBuffer() {
    ...
    _vertbuff = VertexBuffer::alloc(_mesh.vertices.size(),sizeof(OBJVertex),BufferAccess::DYNAMIC);
    _indxbuff = IndexBuffer::alloc(_mesh.indices.size(),BufferAccess::DYNAMIC);

    _vertbuff->loadData(_mesh.vertices.data(), (int)_mesh.vertices.size());
    _indxbuff->loadData(_mesh.indices.data(), (int)_mesh.indices.size());
...
void ObjMesh::draw(const std::shared_ptr<ObjShader>& shader) {
    ...
    if (_material) {
        ...
        Material::applyDefaults(shader);
    }
    
    shader->setVertices(0, _vertbuff);
    shader->setIndices(_indxbuff);
