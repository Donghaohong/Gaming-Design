...
void ObjBatch::flush(const std::shared_ptr<Camera>& camera) {
    ...
    _shader->setCullMode(CullMode::BACK);
    _shader->setDepthFunc(CompareOp::LESS);
    _shader->enableDepthTest(true);
    _shader->enableDepthWrite(true);

    _shader->begin();
    _shader->setPerspective(camera->getCombined());
    _shader->setLightPosition(camera->getPosition());
...
/**
 * Draws all appended nodes to the given framebuffer.
 *
 * The framebuffer should be consistent with that of the {@link Display}.
 * Nodes will be drawn in the order appended.
 *
 * @param camera    The camera to draw with
 * @param fbo       The framebuffer to draw to
 */
void ObjBatch::flush(const std::shared_ptr<Camera>& camera,
                     const std::shared_ptr<graphics::FrameBuffer>& fbo) {
    if (_entries.empty()) {
        return;
    }
