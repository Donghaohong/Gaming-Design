...
void ObjParser::processFace(const char* begin, const char* end, const std::shared_ptr<ModelInfo>& obj) {
    ...
    std::shared_ptr<GroupInfo> group = obj->currentGroup();
    if (group == nullptr ||
        (group->command != DrawMode::UNDEFINED && group->command != DrawMode::TRIANGLES)) {
        group = obj->acquireGroup();
    }
    
    const char* curr = begin+2;
    Uint32 count = (Uint32)group->vertCache.size();
    ...
    group->command = DrawMode::TRIANGLES;
    group->touched = false;
    Uint32 base = indices[0];
    Uint32 left = indices[1];
...
void ObjParser::processLine(const char* begin, const char* end, const std::shared_ptr<ModelInfo>& obj) {
    ...
    std::shared_ptr<GroupInfo> group = obj->currentGroup();
    if (group == nullptr ||
        (group->command != DrawMode::UNDEFINED && group->command != DrawMode::LINES)) {
        group = obj->acquireGroup();
    }
    ...
    group->command = DrawMode::LINES;
    group->touched = false;
    Uint32 left = indices[0];
...
void ObjParser::processPoints(const char* begin, const char* end, const std::shared_ptr<ModelInfo>& obj) {
    ...
    std::shared_ptr<GroupInfo> group = obj->currentGroup();
    if (group == nullptr ||
        (group->command != DrawMode::UNDEFINED && group->command != DrawMode::POINTS)) {
        group = obj->acquireGroup();
    }
...
void ObjParser::processIllum(const char* begin, const char* end, const std::shared_ptr<MaterialLib>& mtl) {
    ...
    char* right;
    unsigned illum = (unsigned)std::strtoul(curr,&right,10);
    if (curr == right) {
