...
void Gimbal::setMinimumZoom(float min) {
    CUAssertLog(min > 0, "Minimum zoom %g is not positive", min);
    CUAssertLog(min <= _maxZoom, "Minimum zoom %g exceeds maximum %g.", min, _maxZoom);
    _minZoom = min;
}
...
void Gimbal::setMaximumZoom(float max) {
    CUAssertLog(max > 0, "Maximum zoom %g is not positive", max);
    CUAssertLog(_minZoom <= max, "Maximum value %g is less than minimum %g.", max, _minZoom);
    _maxZoom = max;
}
