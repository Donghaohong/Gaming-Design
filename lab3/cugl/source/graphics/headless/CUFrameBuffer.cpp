...
//  framebuffer with (potentially multiple) attached output buffers. This allows
//  us to draw to a texture for potential post processing.
//
//  This version does not actually support any computation. It just stores data,
//  allowing us to use it in non-graphical simulations.
//
...
std::shared_ptr<FrameBuffer> FrameBuffer::getDisplay() {
    if (!_gFramebuffer) {
        ...
        _gFramebuffer->_renderPass = RenderPass::alloc(data->outputs.size(),true);
        ...
        _gFramebuffer->_viewport[0] = 0;
        _gFramebuffer->_viewport[1] = 0;
