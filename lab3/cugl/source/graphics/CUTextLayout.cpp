 ...
 */
std::unordered_map<size_t, std::shared_ptr<GlyphRun>> TextLayout::getGlyphs() const {
 ...
 * @return a set of glyph runs to render the text layout
 */
std::unordered_map<size_t,std::shared_ptr<GlyphRun>> TextLayout::getGlyphs(const Rect& rect) const {
    std::unordered_map<size_t, std::shared_ptr<GlyphRun>> result;
 ...
 */
size_t TextLayout::getGlyphs(std::unordered_map<size_t,std::shared_ptr<GlyphRun>>& runs) const {
 ...
 * @return the number of glyphs successfully processed
 */
size_t TextLayout::getGlyphs(std::unordered_map<size_t,std::shared_ptr<GlyphRun>>& runs,
 ...
 * @return a (line) mesh of the quad outlines for the text glyphs
 */
Mesh<SpriteVertex> TextLayout::getGlyphBoxes(const Rect& rect) const {
    Mesh<SpriteVertex> result;
    result.command = DrawMode::LINES;
 ...
 * @return the number of quads generated
 */
size_t TextLayout::getGlyphBoxes(Mesh<SpriteVertex>& mesh, const Rect& rect) const {
    Rect bounds = getBounds();
    bounds.intersect(rect);
    size_t total = 0;
    for(auto it = _rows.begin(); it != _rows.end(); ++it) {
