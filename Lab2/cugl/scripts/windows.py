...
MODULES = {
    ...
    'main' : ('cugl-main','2398D013-23E8-4050-9D7D-D519C39DF917'),
    'graphics' : ('cugl-graphics-opengl','62CBAD5B-D4E7-4B0E-8D42-5065E22E602F'),
    'graphics-headless' : ('cugl-graphics-headless','B6B90309-0F84-4720-8C57-B4F07D0148E1'),
    'graphics-opengl' : ('cugl-graphics-opengl','62CBAD5B-D4E7-4B0E-8D42-5065E22E602F'),
    'physics2' : ('cugl-physics2','D760AAB9-E462-4798-B80F-F84A9AE30540'),
    'netcode' : ('cugl-netcode','9C944826-FC11-43A6-A70D-4B5507542E39'),
    'physics2::distrib' : ('cugl-physics2-distrib','0B582C4D-2886-494D-A585-1E90152E337F'),
    'physics2::lights'  : ('cugl-physics2-lights','40C7B72D-0C66-406F-89B1-FB54D9D8DE4A'),
...
def make_directory(config):
    ...
    This also copies over a private copy of the cugl-main VS project so we can
    ...
    # Create a cugl-main subfolder if necessary
    maindir = os.path.join(build,'cugl-main')
    if not os.path.exists(maindir):
        os.mkdir(maindir)

    # Copy the cugl-core project
    src = os.path.join(config['cugl'],'buildfiles',MAKEDIR,'cugl-main','cugl-main.vcxproj')
    dst = os.path.join(maindir,'cugl-main.vcxproj')
    shutil.copyfile(src, dst)

    src = os.path.join(config['cugl'],'buildfiles',MAKEDIR,'cugl-main','cugl-main.vcxproj.filters')
    dst = os.path.join(maindir,'cugl-main.vcxproj.filters')
    shutil.copyfile(src, dst)

    src = os.path.join(config['cugl'],'buildfiles',MAKEDIR,'cugl-main','cugl-main.vcxproj.user')
    dst = os.path.join(maindir,'cugl-main.vcxproj.user')
    shutil.copyfile(src, dst)
    ...
    source = os.path.join(maindir,'cugl-main.vcxproj')
    util.file_replace(source,contents)

    source = os.path.join(maindir,'cugl-main.vcxproj.filters')
    util.file_replace(source,contents)
...
def filter_modules(config,project):
    ...
    for key in config['modules']:
        if config['modules'][key]:
            if key == 'graphics':
                libs.append(key+'-'+config['backend'])
            else:
                libs.append(key)
    # TODO: Refactor the graphics removal in main
    if config['backend'] == 'headless':
        libs.append('graphics-headless')
...
def expand_global_configurations(config,project):
    ...
    if config['backend'] == 'headless':
        context['CU_USE_OPENGL'] = 'CU_USE_HEADLESS'
    elif config['backend'] == 'vulkan':
        context['CU_USE_OPENGL'] = 'CU_USE_VULKAN'
