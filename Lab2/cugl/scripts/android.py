...
def config_ndkmake(config,project):
    ...
    contents['__CUGL_OPENGL__'] = 'false' if backend == 'vulkan' else 'true'
    ...
    contents['__CUGL_PHYSICS2_LIGHTS__']  = 'true' if config['modules']['physics2::lights'] else 'false'
    ...
    util.directory_replace(sdlroot,contents,lambda path : os.path.split(path)[1] == 'Android.mk')
...
def config_modules(config,project):
    ...
    backend = config['backend']
    context = {}    
    context['__CUGL_BACKEND__'] = f'"{backend}"'
    ...
    context['__CUGL_PHYSICS2_LIGHTS__' ] = 'ON' if config['modules']['physics2::lights'] else 'OFF'
