...
TARGET_ID = 'EB0F3CA927FB9DCC0037CC66'
...
def make_directory(config):
    ...
    if config['backend'] == 'headless':
        contents['CU_USE_OPENGL'] = 'CU_USE_HEADLESS'
    elif config['backend'] == 'vulkan':
        contents['CU_USE_OPENGL'] = 'CU_USE_VULKAN'
...
def reassign_pbxproj(config,pbxproj):
    ...
    sdl3dir  = '../'+util.path_to_posix(os.path.join(config['build_to_cugl'],'sdlapp'))
    ...
    for category in ['PBXProject','XCConfigurationList','PBXNativeTarget']:
        ...
        for pos in range(len(section)):
            if 'cugl-app' in section[pos]:
                section[pos] = section[pos].replace('cugl-app',config['short'].lower())
...
def populate_assets(config,pbxproj):
    ...
    for pos in range(len(groups)):
        if app_rsc and app_rsc in groups[pos]:
            groups[pos] = place_entries(groups[pos],appref)
...
def populate_sources(config,pbxproj):
    ...
    for obj in pbxproj['PBXNativeTarget']:
        ...
        if pos0 != -1 and pos1 != -1:
            if TARGET_ID in obj:
                app_src = obj[pos0:pos1].strip()

    # Add them to the builds
    groups = pbxproj['PBXSourcesBuildPhase']
    for pos in range(len(groups)):
        if app_src and app_src in groups[pos]:
            groups[pos] = place_entries(groups[pos],appref)
...
def filter_modules(config,pbxproj):
    ...
    for pos in range(len(section)):
        text = section[pos]
        modules = {'audio' :'libcugl-audio.a',
                   'scene2':'libcugl-scene2.a',
                   'scene3':'libcugl-scene3.a',
                   'physics2':'libcugl-physics2.a',
                   'netcode' :'libcugl-netcode.a',
                   'physics2::distrib':'libcugl-physics2-distrib.a',
                   'physics2::lights' :'libcugl-physics2-lights.a'}
        backends = {'headless':'libcugl-backend-headless.a',
                    'opengl':'libcugl-backend-opengl.a',
                    'vulkan':'libcugl-backend-vulkan.a'}
        ...
        for item in alllibs:
            ...
            for key in modules:
                if not config['modules'][key] and modules[key] in item:
                    skip = True
            for key in backends:
                if key != config['backend'] and backends[key] in item:
                    skip = True
            if not skip:
                thelibs.append(item)
...
def expand_includes(config,pbxproj):
    ...
    Note that we do not split up headers by build type (e.g. 'debug' and 
    'release'). All build types get the same headers.
    ...
    if 'apple' in entries and entries['apple']:
        allincludes += config['include_dict']['apple']
    else:
        allincludes += []
    if allincludes:
...
def expand_defines(config,pbxproj):
    ...
    Note that defines are broken up into the build types (e.g. 'debug' and 
    'release').
    ...
    buildconfigs = get_buildconfigs(TARGET_ID,pbxproj)
...
def update_schemes(config,project):
    ...
    context = { 'cugl-app':config['short'].lower() }
    context['__DISPLAY_NAME__'] = config['name']
    util.file_replace(management,context)

    name = config['short'].lower()
    src = os.path.join(project,'xcshareddata','xcschemes','cugl-app.xcscheme')
    dst = os.path.join(project,'xcshareddata','xcschemes',name+'.xcscheme')
    shutil.move(src,dst)
    context = {'cugl-app':name,'container:app.xcodeproj':'container:'+config['camel']+'.xcodeproj'}
...
def configure_external(config,module,xcode,platform):
    """
    ...
    uuidsvc = config['uuids']
    suffix = '-mac' if platform == 'macos' else '-ios'
    ...
    for uuid in targetset:
        ...
        if includes:
            if 'apple' in config['include_dict']:
                config['include_dict']['apple'].extend(includes)
            else:
                config['include_dict']['apple'] = includes

        # Find any define information available (to store in config)
        defines = extract_defines(uuid,pbxproj)
        if defines:
            if 'apple' in config['define_dict']:
                config['define_dict']['apple'] = util.merge_dict(config['define_dict']['apple'],defines)
            else:
                config['define_dict']['apple'] = defines
...
def link_external(config,pbxproj,libraries,platform):
    """
    Links the given libraries into the given pbxproj

    Libraries should be a data structure generated by configure_external, which
    has all of the information necessary for linking.

    This code is an absolute mess. The only reason I am doing this is because
    students begged for this feature for years. I am ashamed to push this code 
    into the wild.
    ...
    :type platform:  ``str`` (one of 'macos', 'ios')
    """
    ...
    filefmt = '\t\t%s /* %s in Frameworks */ = {isa = PBXBuildFile; fileRef = %s /* %s */; %s };\n'

    if platform == 'macos':
        filters = 'platformFilters = (macos, );'
    else:
        filters = 'platformFilters = (ios, maccatalyst, tvos, xros, );'
    ...
    for lib in libraries:
        if lib != '':
            ...
            pbxproj['PBXBuildFile'].append(filefmt % (fileuuid,library,proxyuuid,library,filters))
...
def link_frameworks(config,pbxproj,libraries,platform):
    """
    ...
    doing this is because students begged for this feature for years. I am 
    ashamed to push this code into the wild.
    ...
    # Eliminate anything already included
    eliminate = []
    for name in frameworks:
    ...
    # Add a group for externals if necessary
    subidx = -1
    for pos in range(len(section)):
        lines = section[pos].split('\n')
        if 'Externals' in lines[0]:
            subidx = pos
            break
    
    if subidx == -1:
        uuidsvc = config['uuids']
        uuid = uuidsvc.getAppleUUID('GROUP://Externals')
        uuid = uuidsvc.applyPrefix('CG',uuid)
        grpfmt = '\t\t\t\t%s /* Externals */,' % uuid
        
        try:
            lines = section[idx].split('\n')
            end = lines.index('\t\t\t);')
            lines = lines[:end]+[grpfmt]+lines[end:]
            section[idx] = '\n'.join(lines)
        except:
            pass
        ...
        entry += '\t\t\tchildren = (\n'
        entry += '\t\t\t);\n'
        entry += '\t\t\tname = Externals;\n'
        entry += '\t\t\tsourceTree = "<group>";\n'
        entry += '\t\t};\n'
        section.append(entry);
        
        subidx = len(section)-1
        lines = section[-1].split('\n')
    
    # Again, search for duplicates
    duplicates = []
    for name in frameworks:
        if name in section[subidx]:
    ...
    if platform == 'macos':
        ...
        filters = 'platformFilters = (ios, maccatalyst, macos, xros, );'

    section = pbxproj['PBXBuildFile']
    for name in frameworks:
        section.append(filefmt % (frameworks[name]['fileuuid'],name,frameworks[name]['refuuid'],name,filters))
    
    # Remove filters on any duplicates
    if duplicates:
        for name in duplicates:
            for pos in range(len(section)):
                if name in section[pos]:
                    filt = section[pos].find('platformFilters')
                    if filt != -1:
                        section[pos] = section[pos][:filt]+'};'
...
def add_macos_externals(config,pbxproj):
    ...
    We have to add macos and ios separately because of how CMake configures.
    There is no way to guarantee a cross-platform build with CMake generation.
    ...
    for item in config['external']:
        ...
        command.append('-DCMAKE_OSX_DEPLOYMENT_TARGET=11.5')
        ...
        if xcode:
            libraries = configure_external(config,item,xcode,"macos")
            link_external(config,pbxproj,libraries,"macos")
            link_frameworks(config,pbxproj,libraries,"macos")
...
def add_ios_externals(config,pbxproj):
    ...
    for item in config['external']:
        ...
        # If the project exists, link it up
        if xcode:
            libraries = configure_external(config,item,xcode,"ios")
            link_external(config,pbxproj,libraries,"ios")
            link_frameworks(config,pbxproj,libraries,"ios")
