...
std::shared_ptr<scene2::SceneNode> Scene2Loader::build(const std::string key,
    ...
    case Widget::ORDER:
        ...
        break;
    // TODO: The Vulkan refactor has broken this node
    //case Widget::CANVAS:
    ...
    if (node->getContentSize() == Size::ZERO) {
        ...
        node->setContentSize(Display::get()->getViewBounds().size);
    }
    ...
    if (jt != _forms.end()) {
        ...
        case Form::GRID:
            ...
            break;
        case Form::FIGMA:
            layout = scene2::FigmaLayout::allocWithData(form);
            break;
        case Form::NONE:
        case Form::UNKNOWN:
            break;
