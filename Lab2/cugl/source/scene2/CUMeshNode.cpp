...
bool MeshNode::initWithFileMesh(const std::string filename, const Mesh<SpriteVertex>& mesh) {
    if (TexturedNode::initWithFile(filename)) {
        ...
        _contentSize = bounds.origin+bounds.size;
        return true;
    }
    return false;
}
 ...
 * @return  true if the sprite is initialized properly, false otherwise.
 */
bool MeshNode::initWithTexture(const std::shared_ptr<graphics::Texture>& texture) {
...
const SpriteVertex& MeshNode::getVertex(size_t index) const {
    CUAssertLog(index < _mesh.vertices.size(), "Index %zu is out of range.",index);
...
void MeshNode::setPolygon(const Poly2& poly) {
    ...
    for(auto it = _mesh.vertices.begin(); it != _mesh.vertices.end(); ++it) {
        ...
        s = (it->position.x+off.x)/bounds.size.width;
        t = (it->position.x+off.x)/bounds.size.height;
 ...
 */
void MeshNode::setPolygon(const Rect& rect) {
    setPolygon(Poly2(rect));
}
