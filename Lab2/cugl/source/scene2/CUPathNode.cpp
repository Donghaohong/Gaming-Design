...
void PathNode::draw(const std::shared_ptr<SpriteBatch>& batch, const Affine2& transform, Color4 tint) {
    ...
    if (_stencil) {
        batch->setStencilMode(StencilMode::CLAMP);
    }
    batch->drawMesh(_mesh, transform);
    if (_fringe > 0) {
        if (_stencil) {
            batch->setStencilMode(StencilMode::MASK);
        }
        batch->drawMesh(_border, transform);
    }
    
    if (_stencil) {
        batch->setStencilMode(StencilMode::NONE);
    }
...
void PathNode::updateExtrusion() {
    ...
    if (_stroke > _fringe) {
        ...
        if (_fringe > 0) {
            ...
            _border.command = DrawMode::TRIANGLES;
        ...
        Uint32 white =  Color4::WHITE.getRGBA();
