...
void FloatLayout::layoutHorizontal(SceneNode* node) {
    ...
    for(size_t row = 0; row < count.size(); row++) {
        ...
        for(size_t col = 0; col < count[row]; col++) {
            ...
            if (child && child->getLayoutConstraint() == SceneNode::Constraint::LAYOUT) {
                ...
                if (child->getLayoutConstraint() == SceneNode::Constraint::SCENE) {
                ...
                } else if (child->getLayoutConstraint() == SceneNode::Constraint::SAFE) {
                    if (child->getScene() != nullptr) {
                        Vec2 anchor = child->getAnchor();
