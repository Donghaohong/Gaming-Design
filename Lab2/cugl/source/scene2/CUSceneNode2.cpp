...
/**
 * Returns the constraint for the given JSON entry
 *
 * Constraints are used to specify the behavior of a node with respect to its
 * parent layout manager.
 *
 * @param value The JSON value
 *
 * @return the constraint for the given JSON entry
 */
static SceneNode::Constraint json_constraint(const std::string value) {
    if (strtool::tolower(value) == "scene") {
        return SceneNode::Constraint::SCENE;
    } else if (strtool::tolower(value) == "safe") {
        return SceneNode::Constraint::SAFE;
    } else if (strtool::tolower(value) == "none") {
        return SceneNode::Constraint::NONE;
    } else if (strtool::tolower(value) == "layout") {
        return SceneNode::Constraint::LAYOUT;
    }
...
bool SceneNode::initWithData(const AssetManager* manager,
    ...
    _constraint = json_constraint(data->getString("constraint","layout"));
 ...
 * @return true if initialization was successful.
 */
bool SceneNode::initForDrawableBounds() {
    _constraint = SceneNode::Constraint::SCENE;
    return initWithBounds(Application::get()->getDrawableSize());
}
 ...
 * @return true if initialization was successful.
 */
bool SceneNode::initForSafeBounds() {
    _constraint = SceneNode::Constraint::SAFE;
    return initWithBounds(Application::get()->getSafeBounds());
 ...
 */
void SceneNode::setScene(Scene2* scene) {
    _graph = scene;
    if (scene != nullptr) {
        if (_constraint == SceneNode::Constraint::SCENE) {
            _anchor = Vec2::ANCHOR_BOTTOM_LEFT;
            _angle = 0;
            _scale.set(1,1);
            _position.setZero();
            _contentSize = scene->getSize();
            updateTransform();
        } else if (_constraint == SceneNode::Constraint::SAFE) {
            _anchor = Vec2::ANCHOR_BOTTOM_LEFT;
            _angle = 0;
            _scale.set(1,1);
            
            Rect area = scene->getSafeBounds();
            _position = area.origin;
            _contentSize = area.size;
            updateTransform();
        }
    }
}
