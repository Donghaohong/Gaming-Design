...
static bool is_control(Uint32 thechar) {
    return thechar == 0 || (thechar >= 9 && thechar <= 13) ||
 ...
 *
 * This initializer will perform the layout computation, but it will not
 * create any images or SDL surfaces. It will consume glyphs from the
 * provided glyphset as it adds them to the atlas. So if it successfully
 * adds all glyphs, the value glyphset will be emptied.
 *
 * It is possible for the atlas to reject some glyphs. This is typically
 * because the resulting image size would exceed the maximum allowable
 * image size. In that case, the remaining elements in glyphset are
 * glyphs that must be processed by another atlas.
 *
 * If this atlas cannot process any of the elements in glyphset (because
 * they are unsupported), then this method returns false.
 *
...
bool Font::Atlas::getQuad(Uint32 thechar, Vec2& offset, Mesh<SpriteVertex>& mesh, const Rect& rect) const {
    CUAssertLog(mesh.command == DrawMode::TRIANGLES, "The mesh is not formatted for triangles");
    ...
    bounds.origin.y += quad.origin.y-offset.y;
    ...
    SpriteVertex temp;
    Uint32 size = (Uint32)mesh.vertices.size();
 ...
 *
 * This method does not generate the atlas image, but does all other
 * work in creates the atlas. In particular it creates the image data
 * so that image creation is just one call to the graphics API. This
 * creation will happen once {@link materialize()} is called. As a
 * result, it is safe to call this method outside of the main thread.
 *
...
bool Font::Atlas::build() {
    ...
    SDL_FillSurfaceRect(_surface,&srcrect,
                        SDL_MapSurfaceRGBA(_surface, 255, 255, 255, 255));
    ...
    for(auto it = glyphmap.begin(); it != glyphmap.end(); ++it) {
        ...
        // Move the origin to expect a flipped image in OpenGL
        it->second.origin.y = _surface->h-it->second.origin.y-it->second.size.height;
    }
...
/**
 * Creates the image for this atlas.
 *
 * In OpenGL, this method must be called on the main thread. On the
 * other hand, it is safe to call this off thread. It is only safe to
 * call this method after a succesful call to {@link #build()}.
 *
 * @return true if image creation was successful.
 */
bool Font::Atlas::materialize() {
    if (_surface != nullptr) {
        sdl_flip_vertically(_surface);
        SDL_PremultiplySurfaceAlpha(_surface,false);
        std::byte* bytes = reinterpret_cast<std::byte*>(_surface->pixels);
        texture = Texture::allocWithData(bytes, _surface->w, _surface->h,
                                         TexelFormat::COLOR_RGBA,
                                         ImageAccess::READ_WRITE, true);
        SDL_DestroySurface(_surface);
        _surface = nullptr;
    }
 ...
 *
 * This method is necessary because SDL surface allocation is quite involved
 * when you want proper alpha support.
 *
...
SDL_Surface* Font::Atlas::allocSurface(int width, int height) {
    ...
    result = SDL_CreateSurface(width, height, SDL_PIXELFORMAT_RGBA32);
 ...
 * {@link Application#getAssetDirectory}.
 *
...
bool Font::init(const std::string file, Uint32 size) {
    ...
    if (!filetool::is_absolute(file)) {
        ...
        fullpath = filetool::canonicalize_path(file);
    }
    
    _data = TTF_OpenFont(fullpath.c_str(), size);
    if (_data == nullptr) {
        CUAssertLog(false, "Font initialization error: %s", SDL_GetError());
        return false;
    }
    _fontSize = size;
    const char* strng = TTF_GetFontFamilyName(_data);
    _name = std::string(strng);

    strng = TTF_GetFontStyleName(_data);
    _stylename = std::string(strng);
    
    _fontHeight   = TTF_GetFontHeight(_data);
    _fontAscent   = TTF_GetFontAscent(_data);
    _fontDescent  = TTF_GetFontDescent(_data);
    _fontLineSkip = TTF_GetFontLineSkip(_data);
    _fixedWidth   = TTF_FontIsFixedWidth(_data);
    
    _sampler = Sampler::allocWithFilters(TextureFilter::LINEAR,
...
bool Font::hasGlyph(Uint32 thechar) const {
    return thechar == TAB_CHAR || TTF_FontHasGlyph(_data, thechar);
}
...
void Font::setHinting(Hinting hinting) {
    if (_hints != hinting) {
        _hints = hinting;
        TTF_SetFontHinting(_data, (TTF_HintingFlags)hinting);
        clearAtlases();
    }
}
...
const Font::Metrics Font::getMetrics(Uint32 thechar) const {
    ...
    CUAssertLog(TTF_FontHasGlyph(_data, thechar),
                "Character '%c' is not supported", thechar);
...
unsigned int Font::getKerning(Uint32 a, Uint32 b) const {
    ...
    CUAssertLog(TTF_FontHasGlyph(_data, a), "Character '%c' is not supported", a);
    CUAssertLog(TTF_FontHasGlyph(_data, b), "Character '%c' is not supported", b);
...
/**
 * Creates an image for each atlas in the collection.
 *
 * This method should be called to finalize the work of {@link #buildAtlasesAsync}.
 * In OpenGL, this method must be called on the main thread. However, it can
 * be safely called off thread in Vulkan
 */
...
bool Font::hasAtlases(const std::string charset) const {
    ...
    CUAssertLog(utf8::find_invalid(check, end) == end,
                "String '%s' has an invalid UTF-8 encoding",begin);
 ...
 *
 * Each glyph run will consist of a quad mesh and a texture to render those
 * quads. Rendering all of the glyph runs together will render the entire
 * string.  Generally the quads are non-overlapping, so any blending mode
 * is supported. However, if the atlas padding is non-zero (to support font
 * blur), the quads will overlap at the padding intervals. Therefore, we
 * recommend (premultiplied) alpha blending when you render a string.
 *
 ...
 */
std::unordered_map<size_t, std::shared_ptr<GlyphRun>> Font::getGlyphs(const std::string text, const Vec2& origin) {
    std::unordered_map<size_t, std::shared_ptr<GlyphRun>> result;
    const char* begin = text.c_str();
    const char* end = begin+text.size();
 ...
 * @return a set of glyph runs to render the given string
 */
std::unordered_map<size_t, std::shared_ptr<GlyphRun>> Font::getGlyphs(const char* substr, const char* end,
                                                                      const Vec2& origin) {
    std::unordered_map<size_t, std::shared_ptr<GlyphRun>> result;
 ...
 * @return a set of glyph runs to render the given string
 */
std::unordered_map<size_t, std::shared_ptr<GlyphRun>> Font::getGlyphs(const std::string text,
                                                                      const Vec2& origin, const Rect& rect,
                                                                      float track) {
    std::unordered_map<size_t, std::shared_ptr<GlyphRun>> result;
    const char* begin = text.c_str();
    const char* end = begin+text.size();
 ...
 */
size_t Font::getGlyphs(std::unordered_map<size_t, std::shared_ptr<GlyphRun>>& runs,
                       const std::string text, const Vec2& origin) {
    const char* begin = text.c_str();
    const char* end = begin+text.size();
    Rect bounds(origin,getSize(begin, end));
 ...
 */
size_t Font::getGlyphs(std::unordered_map<size_t, std::shared_ptr<GlyphRun>>& runs,
                       const char* substr, const char* end, const Vec2& origin) {
    Rect bounds(origin,getSize(substr, end));
 ...
 */
size_t Font::getGlyphs(std::unordered_map<size_t, std::shared_ptr<GlyphRun>>& runs,
                       const char* substr, const char* end,
...
std::deque<Uint32> Font::gatherGlyphs() {
    ...
    for(Uint32 ii = 32; ii < 127; ii++) {
        if (_atlasmap.find(ii) == _atlasmap.end() && TTF_FontHasGlyph(_data, ii)) {
...
std::deque<Uint32> Font::gatherGlyphs(const std::string charset) {
    ...
    while (begin != end) {
        ...
        if (thechar == TAB_CHAR && _atlasmap.find(thechar) == _atlasmap.end()) {
            if (_atlasmap.find(SPACE_CHAR) == _atlasmap.end() && TTF_FontHasGlyph(_data, SPACE_CHAR)) {
            ...
            _glyphsize.emplace(TAB_CHAR,metrics);
            // NOT added to atlas, since space is a proxy
        } else if (_atlasmap.find(thechar) == _atlasmap.end() && TTF_FontHasGlyph(_data, thechar)) {
...
std::deque<Uint32> Font::gatherGlyphs(const std::vector<Uint32>& charset) {
    ...
    for(auto it = charset.begin(); it != charset.end(); ++it) {
        if (*it == TAB_CHAR && _atlasmap.find(*it) == _atlasmap.end()) {
            ...
            _glyphsize.emplace(TAB_CHAR,metrics);
            // NOT added to atlas, since space is a proxy
        } else if (_atlasmap.find(*it) == _atlasmap.end() && TTF_FontHasGlyph(_data, *it)) {
...
Font::Metrics Font::computeMetrics(Uint32 thechar) const {
    ...
    Metrics metrics;
    int success = TTF_GetGlyphMetrics(_data, thechar, &metrics.minx, &metrics.maxx,
    ...
    if (success != -1) {
        ...
        int w = 0;
        int h = 0;
        TTF_GetStringSize(_data, str, 0, &w, &h);
...
int Font::computeKerning(Uint32 a, Uint32 b) const {
    ...
    int w1, w2;
    TTF_GetStringSize(_data, str, 0, &w1, &w2);
...
bool Font::getOutline(Uint32 thechar, Vec2& offset, Mesh<SpriteVertex>& mesh, const Rect& rect) {
    CUAssertLog(mesh.command == DrawMode::LINES, "The mesh is not formatted for lines");
    ...
    // Bottom left
    Uint32 white = Color4::WHITE.getPacked();
    temp.position = quad.origin;
    temp.color = white;
    temp.texcoord.x = 0;
    temp.texcoord.y = 0;
