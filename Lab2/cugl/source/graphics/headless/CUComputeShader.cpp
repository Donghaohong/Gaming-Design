...
//
//  In the headless backend, this will store data for the shader, but will not
//  actually compute anything. This allows the headless mode to be used for
...
// For maximum capatibility, we fail silently
...
void ComputeShader::setUniformBuffer(const std::string name, const std::shared_ptr<UniformBuffer>& buffer) {
    CUAssertLog(_data != nullptr, "Compute shader is not initialized");
    CUAssertLog(buffer != nullptr, "Cannot assign an empty uniform buffer");
        
    auto it = _invalues.find(name);
    auto jt = _inputs.find(name);
    if (it == _invalues.end() && jt == _inputs.end()) {
    ...
    for(int ii = 0; ii < jt->second.arraysize; ii++) {
 ...
 */
void ComputeShader::setUniformBuffers(const std::string name,
    ...
    if (resource->sources.size() != jt->second.arraysize) {
        resource->sources.resize(jt->second.arraysize, buffers.back());
    }
}
...
const std::shared_ptr<UniformBuffer> ComputeShader::getUniformBuffer(const std::string name) const {
    auto it = _invalues.find(name);
    if (it == _invalues.end() ||
        (it->second->type != ResourceType::MONO_BUFFER &&
         it->second->type != ResourceType::MULTI_BUFFER)) {
        return nullptr;
    }
...
void ComputeShader::setStorageBuffer(const std::string name, const std::shared_ptr<StorageBuffer>& buffer) {
    ...
    auto it = _invalues.find(name);
    auto jt = _outvalues.find(name);
    if (it == _invalues.end() && jt == _outvalues.end()) {
        CUAssertLog(false,"Invalid assignment to variable %s",name.c_str());
        return;
    }
    ...
    if (it == _invalues.end()) {
        data = jt->second;
        arraysize = _outputs.find(name)->second.arraysize;
    } else {
        data = it->second;
        arraysize = _inputs.find(name)->second.arraysize;
    }
    
    if (data->type != ResourceType::MONO_STORAGE &&
        data->type != ResourceType::MULTI_STORAGE &&
        data->type != ResourceType::COMPUTE_STORAGE) {
        CUAssertLog(false,"Invalid assignment to variable %s",name.c_str());
        return;
    }
    
    if (jt != _outvalues.end() &&
        buffer->getAccess() != BufferAccess::COMPUTE_ONLY &&
        buffer->getAccess() != BufferAccess::COMPUTE_READ &&
        buffer->getAccess() != BufferAccess::COMPUTE_ALL) {
        CUAssertLog(false,"Buffer assigned to variable %s does not support compute",name.c_str());
        return;
    }

    ResourceStorage* resource = reinterpret_cast<ResourceStorage*>(data);
    resource->sources.clear();
 ...
 */
void ComputeShader::setStorageBuffers(const std::string name,
    ...
    if (jt != _outvalues.end() &&
        buffers.front()->getAccess() != BufferAccess::COMPUTE_ONLY &&
        buffers.front()->getAccess() != BufferAccess::COMPUTE_READ &&
        buffers.front()->getAccess() != BufferAccess::COMPUTE_ALL) {
        CUAssertLog(false,"Buffer assigned to variable %s does not support compute",name.c_str());
        return;
    }
    ...
    resource->sources.clear();
    resource->sources = buffers;
    if (resource->sources.size() != arraysize) {
        resource->sources.resize(arraysize, buffers.back());
    }
}
...
const std::shared_ptr<StorageBuffer> ComputeShader::getStorageBuffer(const std::string name) const {
    ...
    ResourceData* data = (it != _invalues.end() ? it->second : jt->second);
    if (data->type != ResourceType::MONO_STORAGE &&
        data->type != ResourceType::MULTI_STORAGE &&
        data->type != ResourceType::COMPUTE_STORAGE) {
        return nullptr;
...
void ComputeShader::setBlock(const std::string name, Uint32 block) {
    ...
    ResourceData* data = (it != _invalues.end() ? it->second : jt->second);
    if (data->type == ResourceType::MULTI_BUFFER) {
        ResourceUniform* uniform = reinterpret_cast<ResourceUniform*>(data);
        uniform->block = block;
    } else if (data->type == ResourceType::MULTI_STORAGE) {
        ResourceStorage* storage = reinterpret_cast<ResourceStorage*>(data);
        storage->block = block;
    } else {
        CUAssertLog(false,"Input variable %s is not a dynamic buffer",name.c_str());
    }
}
...
void ComputeShader::setVertexBuffer(const std::string name, const std::shared_ptr<VertexBuffer>& buffer) {
    CUAssertLog(_data != nullptr, "Compute shader is not initialized");
    CUAssertLog(buffer != nullptr, "Cannot assign an empty vertex buffer");

    auto it = _outvalues.find(name);
    if (it == _outvalues.end() || it->second->type != ResourceType::COMPUTE_VERTEX) {
        CUAssertLog(false,"Invalid assignment to variable %s",name.c_str());
        return;
    }
    
    ResourceVertex* resource = reinterpret_cast<ResourceVertex*>(it->second);
...
void ComputeShader::setIndexBuffer(const std::string name, const std::shared_ptr<IndexBuffer>& buffer) {
    CUAssertLog(buffer != nullptr, "Cannot assign an empty index buffer");

     auto it = _outvalues.find(name);
     if (it == _outvalues.end() || it->second->type != ResourceType::COMPUTE_INDEX) {
         CUAssertLog(false,"Invalid assignment to variable %s",name.c_str());
         return;
     }
     
     ResourceIndex* resource = reinterpret_cast<ResourceIndex*>(it->second);
     resource->source = buffer;
}
...
void ComputeShader::setTextures(const std::string name,
    ...
    resource->sources.clear();
    if (resource->sources.size() != jt->second.arraysize) {
        resource->sources.resize(jt->second.arraysize, textures.back());
    }
}
...
const std::shared_ptr<Texture> ComputeShader::getTexture(const std::string name) const {
    ...
    auto it = _invalues.find(name);
    if (it == _invalues.end() || it->second->type != ResourceType::TEXTURE) {
        return nullptr;
    }
...
void ComputeShader::setTextureArray(const std::string name, const std::shared_ptr<TextureArray>& array)  {
    ...
    if (it->second->type != ResourceType::TEXTURE_ARRAY) {
        CUAssertLog(false,"Invalid assignment to variable %s",name.c_str());
    }
 ...
 */
void ComputeShader::setTextureArrays(const std::string name,
    ...
    resource->sources.clear();
    if (resource->sources.size() != jt->second.arraysize) {
        resource->sources.resize(jt->second.arraysize, arrays.back());
    }
}
...
const std::shared_ptr<TextureArray> ComputeShader::getTextureArray(const std::string name) const {
    ...
    auto it = _invalues.find(name);
    if (it == _invalues.end() || it->second->type != ResourceType::TEXTURE_ARRAY) {
        return nullptr;
    }
...
void ComputeShader::setImage(const std::string name,
    ...
    if (data->type != ResourceType::IMAGE &&
        data->type != ResourceType::COMPUTE_IMAGE) {
        CUAssertLog(false,"Invalid assignment to variable %s",name.c_str());
        return;
    }
    
    if (jt != _outvalues.end() &&
        image->getAccess() != ImageAccess::COMPUTE_ONLY &&
        image->getAccess() != ImageAccess::COMPUTE_READ &&
        image->getAccess() != ImageAccess::COMPUTE_ALL) {
        CUAssertLog(false,"Buffer assigned to variable %s does not support compute",name.c_str());
        return;
    }
    
    ResourceImage* resource = reinterpret_cast<ResourceImage*>(data);
    resource->sources.clear();
    for(int ii = 0; ii < arraysize; ii++) {
...
void ComputeShader::setImages(const std::string name,
    ...
    // Only check the front
    if (jt != _outvalues.end() &&
        images.front()->getAccess() != ImageAccess::COMPUTE_ONLY &&
        images.front()->getAccess() != ImageAccess::COMPUTE_READ &&
        images.front()->getAccess() != ImageAccess::COMPUTE_ALL) {
        CUAssertLog(false,"Buffer assigned to variable %s does not support compute",name.c_str());
        return;
    }
    ...
    if (resource->sources.size() != arraysize) {
        resource->sources.resize(arraysize, images.back());
    }
}
...
void ComputeShader::setImageArray(const std::string name, const std::shared_ptr<ImageArray>& array) {
    ...
    if (it->second->type != ResourceType::IMAGE_ARRAY) {
        CUAssertLog(false,"Invalid assignment to variable %s",name.c_str());
    }
 ...
 */
void ComputeShader::setImageArrays(const std::string name,
...
const std::shared_ptr<ImageArray> ComputeShader::getImageArray(const std::string name) const  {
    auto it = _invalues.find(name);
    if (it == _invalues.end() || it->second->type != ResourceType::IMAGE_ARRAY) {
        return nullptr;
    }
...
void ComputeShader::setSampler(const std::string name, const std::shared_ptr<Sampler>& sampler) {
    ...
    if (it->second->type != ResourceType::SAMPLER) {
        CUAssertLog(false,"Invalid assignment to variable %s",name.c_str());
    }
...
void ComputeShader::setSamplers(const std::string name,
    ...
    resource->sources.clear();
    if (resource->sources.size() != jt->second.arraysize) {
        resource->sources.resize(jt->second.arraysize, samplers.back());
    }
}
...
const std::shared_ptr<Sampler> ComputeShader::getSampler(const std::string name) {
    auto it = _invalues.find(name);
    if (it == _invalues.end() || it->second->type != ResourceType::SAMPLER) {
        return nullptr;
    }
    
    ResourceSampler* resource = reinterpret_cast<ResourceSampler*>(it->second);
    return resource->sources.front();
}
