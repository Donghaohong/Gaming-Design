...
void NetEventController::enablePhysics(std::shared_ptr<NetWorld>& world, ObstacleLink linkFunc) {
    ...
    attachEventType<PhysicsSyncEvent>();
    attachEventType<PhysicsObstacleEvent>();
    if(_isHost) {
...
void NetEventController::updateNet() {
    if(_network){
        ...
        if (_status == Status::INGAME && _physEnabled) {
            _physController->packPhysicsSync(NetPhysicsController::SyncType::FULL_SYNC);
...
void NetEventController::processReceivedEvent(const std::shared_ptr<NetEvent>& e) {
    if (auto game = std::dynamic_pointer_cast<GameStateEvent>(e)) {
        ...
        if (auto phys = std::dynamic_pointer_cast<PhysicsSyncEvent>(e)) {
            if(_physEnabled)
                _physController->processPhysicsSyncEvent(phys);
        }
        else if (auto phys = std::dynamic_pointer_cast<PhysicsObstacleEvent>(e)) {
            if (_physEnabled) {
                _physController->processPhysicsObstacleEvent(phys);
            }
        }
        else {
