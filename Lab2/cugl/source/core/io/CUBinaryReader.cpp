...
bool BinaryReader::init(const std::string file, unsigned int capacity) {
    ...
    if (!filetool::is_absolute(file)) {
        _name = Application::get()->getSaveDirectory();
        _name.append(file);
        _name = filetool::normalize_path(_name);
    } else {
        _name = filetool::normalize_path(file);
    }
    _stream = SDL_IOFromFile(_name.c_str(), "rb");
    if (!_stream) {
        return false;
    }
    
    _ssize = SDL_GetIOSize(_stream);
    _scursor = 0;
    _capacity = capacity;
...
void BinaryReader::fill(unsigned int bytes) {
    ...
    size_t amt = SDL_ReadIO(_stream, &_buffer[_bufsize], _capacity-_bufsize);
    _bufsize += (Uint32)amt;
    _scursor += amt;
}
 ...
 */
size_t BinaryReader::read(char* buffer, size_t maximum) {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
    while (ready(1) && pos < maximum) {
        size_t available = _bufsize-_bufoff;
        size_t wanted = maximum-pos;
 ...
 */
size_t BinaryReader::read(Uint8* buffer, size_t maximum)  {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
    while (ready(1) && pos < maximum) {
        size_t available = _bufsize-_bufoff;
 ...
 */
size_t BinaryReader::read(Sint16* buffer, size_t maximum) {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
    unsigned int bytes = 2;
    while (ready(bytes) && pos < maximum) {
        size_t available = bytes*((_bufsize-_bufoff)/bytes);
        size_t wanted = (maximum-pos)*bytes;
        wanted = wanted < available ? wanted : available;
        memcpy(&(buffer[pos]),&(_buffer[_bufoff]),wanted);
        _bufoff += (Sint32)wanted;
 ...
 */
size_t BinaryReader::read(Uint16* buffer, size_t maximum)  {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
    unsigned int bytes = 2;
    while (ready(bytes) && pos < maximum) {
 ...
 */
size_t BinaryReader::read(Sint32* buffer, size_t maximum) {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
 ...
 */
size_t BinaryReader::read(Uint32* buffer, size_t maximum) {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
    unsigned int bytes = 4;
    while (ready(bytes) && pos < maximum) {
 ...
 * @return the number of 32 bit signed integers read from the stream
 */
size_t BinaryReader::read(Sint64* buffer, size_t maximum) {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
 ...
 * @return the number of 32 bit unsigned integers read from the stream
 */
size_t BinaryReader::read(Uint64* buffer, size_t maximum) {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
    unsigned int bytes = 8;
    while (ready(bytes) && pos < maximum) {
 ...
 */
size_t BinaryReader::read(float* buffer, size_t maximum) {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
    unsigned int bytes = 4;
    while (ready(bytes) && pos < maximum) {
 ...
 */
size_t BinaryReader::read(double* buffer, size_t maximum) {
    CUAssertLog(ready(), "Attempt to read a finished stream");
    unsigned int pos = 0;
    unsigned int bytes = 8;
    while (ready(bytes) && pos < maximum) {
...
/**
 * Returns all of the bytes from this stream as a single vector.
 *
 * Calling this method will reset the reader to the beginning before
 * reading. If no bytes can be read, this method returns an empty vector.
 *
 * @return all of the bytes from this stream as a single vector.
 */
std::vector<std::byte> BinaryReader::readBytes() {
    if (_scursor != 0) {
        Sint64 pos = SDL_SeekIO(_stream, 0, SDL_IO_SEEK_SET);
        if (pos != -1) {
            _scursor = pos;
        }
    }
    ...
    if (_ssize > _scursor) {
        result.resize(_ssize-_scursor);
        size_t size = SDL_ReadIO(_stream, result.data(),
                                 sizeof(std::byte)*result.size());
