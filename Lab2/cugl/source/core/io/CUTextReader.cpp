...
bool TextReader::init(const std::string file, unsigned int capacity) {
    ...
    if (!filetool::is_absolute(file)) {
        ...
        _name.append(file);
        _name = filetool::canonicalize_path(_name);
    } else {
        _name = filetool::canonicalize_path(file);
    }
    _stream = SDL_IOFromFile(_name.c_str(), "r");
    if (!_stream) {
        return false;
    }
...
void TextReader::fill() {
    ...
    _bufoff = 0;
    size_t amt = SDL_ReadIO(_stream, _cbuffer, _capacity-_sbuffer.size());
