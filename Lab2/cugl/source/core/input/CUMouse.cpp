    ...
    switch (event.type) {
        case SDL_EVENT_MOUSE_BUTTON_UP:
            if (event.button.which != SDL_TOUCH_MOUSEID) {
                pos = Display::get()->getMouseScreenPosition(event.button.x, event.button.y);
                MouseEvent mevent(SDL_BUTTON_MASK(event.button.button), pos, stamp);
        ...
        case SDL_EVENT_MOUSE_MOTION:
            if (event.motion.which != SDL_TOUCH_MOUSEID) {
                if (_awareness == PointerAwareness::DRAG && event.motion.state > 0) {
                    ...
                    Vec2 previous = Display::get()->getMouseScreenPosition(
        ...
        case SDL_EVENT_MOUSE_WHEEL:
            if (event.wheel.which != SDL_TOUCH_MOUSEID) {
                MouseWheelEvent mevent(Vec2((float)event.wheel.x, (float)event.wheel.y),
                                       stamp,event.wheel.direction == SDL_MOUSEWHEEL_FLIPPED);
...
void Mouse::queryEvents(std::vector<Uint32>& eventset) {
    eventset.push_back((Uint32)SDL_EVENT_MOUSE_MOTION);
    eventset.push_back((Uint32)SDL_EVENT_MOUSE_BUTTON_UP);
    eventset.push_back((Uint32)SDL_EVENT_MOUSE_BUTTON_DOWN);
    eventset.push_back((Uint32)SDL_EVENT_MOUSE_WHEEL);
}
