...
bool Touchscreen::updateState(const SDL_Event& event, const Timestamp& stamp) {
    ...
    switch (event.type) {
        ...
        {
            pos = Display::get()->getTouchScreenPosition(event.tfinger.x,event.tfinger.y);
            TouchEvent tevent(event.tfinger.fingerID, pos,
        ...
        case SDL_EVENT_FINGER_MOTION:
        {
            ...
            Vec2 previous = Display::get()->getTouchScreenPosition(event.tfinger.x-event.tfinger.dx,
...
void Touchscreen::queryEvents(std::vector<Uint32>& eventset) {
    eventset.push_back((Uint32)SDL_EVENT_FINGER_DOWN);
    eventset.push_back((Uint32)SDL_EVENT_FINGER_UP);
    eventset.push_back((Uint32)SDL_EVENT_FINGER_MOTION);
}
