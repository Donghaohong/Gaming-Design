...
/** We want to normalize the accelerometer values to 0-1 */
#define SDL_ACCEL_FACTOR 10.f;
...
bool Accelerometer::init() {
    ...
    for (int ii=0; ii < count; ii++) {
        if (SDL_GetSensorTypeForID(sensors[ii]) == SDL_SENSOR_ACCEL) {
            _input = SDL_OpenSensor(sensors[ii]);
        }
    }
    
    _scale.set(1,1,1);
    _swapped = Display::get()->hasSwappedAxes();
    setReferenceFrame(Display::get()->getConfiguredOrientation());
...
bool Accelerometer::updateState(const SDL_Event& event, const Timestamp& stamp) {
    ...
    if (event.type == SDL_EVENT_SENSOR_UPDATE) {
        _current.set(event.sensor.data[_xaxis],event.sensor.data[_yaxis],event.sensor.data[2]);
        _current *= _scale;
        _current /= SDL_ACCEL_FACTOR;
        _update = true;
...
void Accelerometer::queryEvents(std::vector<Uint32>& eventset) {
    eventset.push_back((Uint32)SDL_EVENT_SENSOR_UPDATE);
}
...
void Accelerometer::setReferenceFrame(Display::Orientation orientation) {
    switch (orientation) {
        ...
        case Display::Orientation::LANDSCAPE_REVERSED:
            _reference = Display::Orientation::LANDSCAPE;
            if (_swapped) {
                _xaxis = 0; _yaxis = 1;
            } else {
                _xaxis = 1; _yaxis = 0;
        ...
        case Display::Orientation::UPSIDE_DOWN:
            _reference = Display::Orientation::PORTRAIT;
            if (_swapped) {
                _xaxis = 1; _yaxis = 0;
            } else {
                _xaxis = 0; _yaxis = 1;
