...
void Material::setShininess(float value) {
    CUAssertLog(value >= 0, "Unsupported shininess value: %g",value);
    _shininess = value;
}
...
#pragma mark Usage
/**
 * Applies this material to the given shader.
 *
 * This method does not do anything unless the shader is in an active
 * draw pass.
 */
void Material::apply(const std::shared_ptr<ObjShader>& shader) {
...
/**
 * Applies a default material to the given shader.
 *
 * This default material will have all white colors and no textures.
 * This method does not do anything unless the shader is in an active
 * draw pass.
 */
void Material::applyDefaults(const std::shared_ptr<ObjShader>& shader) {
    CUAssertLog(shader != nullptr,"Undefined shader");

    shader->setIllum(0);
    shader->setSpecularExponent(0);
    shader->setAmbientColor(Color4::WHITE);
    shader->setDiffuseColor(Color4::WHITE);
    shader->setSpecularColor(Color4::WHITE);
    
    shader->setAmbientTexture(nullptr);
    shader->setDiffuseTexture(nullptr);
    shader->setSpecularTexture(nullptr);
    shader->setNormalTexture(nullptr);
}
