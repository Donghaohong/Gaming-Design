...
static void updateTextureInfo(const std::shared_ptr<TextureInfo>& info, JsonValue* json) {
    ...
    info->minflt = json_filter(json->getString("minfilter","nearest"));
    info->magflt = json_filter(json->getString("magfilter","linear"));
    info->wrapS  = json_wrap(json->getString("wrapS","clamp"));
    info->wrapT  = json_wrap(json->getString("wrapT","clamp"));
...
std::shared_ptr<Texture> MtlLoader::materializeTexture(const std::shared_ptr<TextureInfo>& info,
    ...
    if (result == nullptr) {
        ...
        if (it != _surfaces.end()) {
            bool swap = false;
            if (loader->hasMipMaps() != info->mipmaps) {
                loader->setMipMaps(info->mipmaps);
                swap = true;
            }
