...
bool Scene3::init() {
    ...
    _camera = PerspectiveCamera::alloc(0, 0, _size.width, _size.height);
    _camera->update();
 ...
 * @return true if initialization was successful.
 */
bool Scene3::initWithViewport(float x, float y, float width, float height) {
    if (!Scene::init()) {
        return false;
    }
 ...
 * @return true if initialization was successful.
 */
bool Scene3::initWithFOV(float fov) {
    return false;
}
...
void Scene3::render() {
    ...
    if (_framebuffer) {
        _pipeline->flush(_camera,_framebuffer);
    } else {
