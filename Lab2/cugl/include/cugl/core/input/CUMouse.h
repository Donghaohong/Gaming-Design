...
#pragma mark Listeners
     ...
     *
     * This type of listener only responds to button presses and releases, not
     * mouse movement. Listeners are guaranteed to be called at the start of an
     * animation frame, before the method {@link Application#update(float) }.
     *
     * The listener does not receive any information indicating whether the
     * event is a press or a release. That is handled when the listener is
     * registered. On the other hand, the listener will get a counter if the
     * press/release is a sequence of rapid clicks. This is a way of detecting
     * double or even triple clicks. The click counter will continue to
     * increment as long as there is a click every .5 seconds.
     *
     * While mouse listeners do not traditionally require focus like a keyboard
     * does,we have included that functionality. While only one listener can
     * have focus at a time, all listeners will receive input from the Mouse
     * device.
     *
     ...
     * be a globally unique unsigned int.
     *
     * This type of listener only responds to the wheel mouse, not any other
     * buttons or mouse movement. Listeners are guaranteed to be called at the
     * start of an animation frame, before the method
