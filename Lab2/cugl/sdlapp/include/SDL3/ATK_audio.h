 ...
 * multichannel audio rather than isolated signals. One of its primary uses is
 * to replace the CVT functionality of SDL.
 */
 ...
 *
 * This callback is similar to the one used in older versions of SDL_AudioCVT.
 * It allows us to only fill a portion of a stream, rather than forcing us to
 * pad with silence.
 *
 ...
 */
extern SDL_DECLSPEC ATK_LatencyAdapter* SDLCALL ATK_AllocLatencyAdapter(size_t input, size_t output,
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeLatencyAdapter(ATK_LatencyAdapter* adapter);
 ...
 */
extern SDL_DECLSPEC Sint64 SDLCALL ATK_PollLatencyAdapter(ATK_LatencyAdapter* adapter, Uint8* output, int len);
 ...
 */
extern SDL_DECLSPEC Sint64 SDLCALL ATK_PushLatencyAdapter(ATK_LatencyAdapter* adapter, const Uint8* input, int len);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_PauseLatencyAdapter(ATK_LatencyAdapter* adapter, int pauseon);

/**
 * Resets the latency adapter
 *
 * Resetting empties and zeroes all buffers. It also returns the latency
 * adapter to a paused state. The adapter will need to be unpaused with a call
 * to {@link ATK_PauseLatencyAdapter}
 *
 * @param adapter   The latency adapter
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetLatencyAdapter(ATK_LatencyAdapter* adapter);
 ...
 * @param adapter   The latency adapter
 *
 * @return true if this function blocked
 */
extern SDL_DECLSPEC bool SDLCALL ATK_BlockLatencyAdapter(ATK_LatencyAdapter* adapter);
 ...
 * have had problems with that resampler in the past. In SDL 2.0.14, there was
 * a bug that could cause the resampler to be caught zero-padding in an
 ...
 *
 * More importantly, in the public API of SDL (as opposed to the internal APIs),
 * resampling can only happen just before audio is sent to the device. In an
 * audio engine, you often want to resample much earlier in the DSP graph
 * (e.g. reading in an audio file compressed with a much lower sample rate).
 * That is why we have separated out this feature.
 */
 ...
 *
 * @return a newly allocated resampler
 */
extern SDL_DECLSPEC ATK_Resampler* SDLCALL ATK_AllocResampler(const ATK_ResamplerDef* def);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeResampler(ATK_Resampler* resampler);
 ...
 * @param resampler The resampler state
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetResampler(ATK_Resampler* resampler);
 ...
 */
extern SDL_DECLSPEC Sint64 SDLCALL ATK_PollResampler(ATK_Resampler* resampler, float* output, size_t frames);
 ...
 */
extern SDL_DECLSPEC Sint64 SDLCALL ATK_PushResampler(ATK_Resampler* resampler, const float* input, size_t frames);
 ...
 */
extern SDL_DECLSPEC bool SDLCALL ATK_PeekResampler(ATK_Resampler* resampler);
 ...
 *
 * @return a newly allocate channel redistributor
 */
extern SDL_DECLSPEC ATK_Redistributor* SDLCALL ATK_AllocRedistributor(Uint32 inchan, Uint32 outchan,
                                                                      const float* matrix);
 ...
 */
extern SDL_DECLSPEC const float* SDLCALL ATK_GetRedistributorMatrix(ATK_Redistributor* distrib);
 ...
 * @param distrib   The redistributor state
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeRedistributor(ATK_Redistributor* distrib);
 ...
 * samples for each channel. Thus input should hold frames*inchan samples,
 * while output should be able to store frames*outchan samples (inchan and
 * outchan were specified when the redistributor was allocated).
 *
 ...
 */
extern SDL_DECLSPEC Sint64 SDLCALL ATK_ApplyRedistributor(ATK_Redistributor* distrib,
 ...
 */
extern SDL_DECLSPEC bool SDLCALL ATK_ConvertAudioFormat(const Uint8* input, SDL_AudioFormat informat,
                                                        Uint8* output, SDL_AudioFormat outformat, size_t samples);
 ...
 * that structure in the past. While it is fine for format and channel
 * conversion, we have found it to be quite unreliable for rate conversion. In
 * SDL 2.0.14 there was a bug in the resampler that could cause the converter
 * to be caught in an infinite zero-padding loop, resulting in the audio
 * cutting out. While this has been fixed in more recent versions of SDL, we
 * prefer this version which gives us a little more control over the conversion
 * process. In particular, it is possible to convert audio before it is sent to
 * the device.
 */
typedef struct ATK_AudioCVT ATK_AudioCVT;

/**
 * A structure storing the CVT settings.
 *
 * The CVT is an opaque structure that cannot be changed once it is allocated.
 * However, there are several settings that can be customized before the
 * resampler is created.
 */
typedef struct ATK_AudioCVTDef {
    /**  The SDL audio specification of the input signal */
    SDL_AudioSpec input;
    /**  The SDL audio specification of the output signal */
    SDL_AudioSpec output;
    /**
     * The maximum number of frames to process in one step
     *
     * This defines the maximum amount of data that can be pushed to the CVT 
     * via {@link ATK_PushAudioCVT}. This should be taken into consideration
     * when polling from the CVT.
     */
    size_t frames;
    /**
     * An optional callback function to fill the buffer.
     *
     * The CVT can only process data in its buffer. If this value is not
     * NULL, it will use this callback function to populate the buffer on
     * demand. If it is NULL, then the user must populate the buffer manually
     * with {@link ATK_PushAudioCVT}.
     */
    ATK_AudioCallback callback;
    /**
     * The user data for the callback function.
 ...
 */
extern SDL_DECLSPEC ATK_AudioCVT* SDLCALL ATK_AllocAudioCVT(const ATK_AudioCVTDef* def);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeAudioCVT(ATK_AudioCVT* cvt);
 ...
 * @param cvt   The audio CVT
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetAudioCVT(ATK_AudioCVT* cvt);
 ...
 * @return the number of bytes read or -1 on error
 */
extern SDL_DECLSPEC Sint64 SDLCALL ATK_PollAudioCVT(ATK_AudioCVT* cvt, Uint8* output, size_t len);
 ...
 * @return the number of bytes pushed or -1 on error
 */
extern SDL_DECLSPEC Sint64 SDLCALL ATK_PushAudioCVT(ATK_AudioCVT* cvt, const Uint8* input, size_t len);
 ...
 * set them explicitly. All of values should be between 0 and 1. More
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_AlgoReverbDefaults(ATK_AlgoReverbDef* def);
 ...
 */
extern SDL_DECLSPEC ATK_AlgoReverb* SDLCALL ATK_AllocAlgoReverb(ATK_AlgoReverbDef* def, Uint32 rate,
 ...
 * @param def       The algorithmic reverb settings
 */
extern SDL_DECLSPEC void SDLCALL ATK_UpdateAlgoReverb(ATK_AlgoReverb* filter, ATK_AlgoReverbDef* def);
 ...
 * @param filter    The algorithmic reverb filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_FreeAlgoReverb(ATK_AlgoReverb* filter);
 ...
 * @param filter    The algorithmic reverb filter
 */
extern SDL_DECLSPEC void SDLCALL ATK_ResetAlgoReverb(ATK_AlgoReverb* filter);
 ...
 */
extern SDL_DECLSPEC void SDLCALL ATK_StepAlgoReverb(ATK_AlgoReverb* filter,
 ...
 * @param frames    The number of audio frames to process
 */
extern SDL_DECLSPEC void SDLCALL ATK_ApplyAlgoReverb(ATK_AlgoReverb* filter,
                                                     const float* input, float* output,
                                                     size_t frames);
 ...
 */
extern SDL_DECLSPEC size_t SDLCALL ATK_DrainAlgoReverb(ATK_AlgoReverb* filter, float* buffer, size_t frames);

// TODO: Convolutional reverb

#pragma mark -
#pragma mark Time/Pitch Shift Filters
// TODO: Need a WSOLA filter
//https://sps.ewi.tudelft.nl/Education/courses/ee4c03/assignments/audio_lpc/roelands93icassp.pdf
