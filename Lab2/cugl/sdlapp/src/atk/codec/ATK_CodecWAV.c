 ...
 * matching the Vorbis comment interface. This function returns the Vorbis
 * comment equivalent for an INFO tag. For information on the INFO specification
 * see
 *
...
const char* ATK_GetInfoCommentTag(const char* tag) {
    size_t len = SDL_arraysize(WAV_TAGS);
    Uint32 code1;
    memcpy(&code1,tag,sizeof(Uint32));
    for(size_t ii = 0; ii < len; ii++) {
        Uint32 code2;
        memcpy(&code2,WAV_TAGS[ii],sizeof(Uint32));
        if (code1 == code2) {
...
/**
 * Returns true if WAV supports the given comment tag.
 *
 * @param tag   The tag to query
 *
 * @return true if WAV supports the given comment tag.
 */
bool ATK_WAV_SupportsCommentTag(const char* tag) {
 ...
 */
const char* const * ATK_WAV_GetCommentTags(void) {
    return WAV_VERBOSE;
}
 ...
 */
ATK_AudioComment* ATK_WAV_AllocComments(const Uint8* data, size_t size, int* len) {
    Uint32* curr = (Uint32*)data;
    if (SDL_Swap32LE(*curr) != INFO) {
        return NULL;
    }
    ...
    for(size_t pos = 4; pos < size-4; ) {
        Uint32 len = SDL_Swap32LE(*(Uint32*)(data+pos+4));
        if (len) {
 ...
 * @param block     The wave block with the audio data
 *
 * @return the position of the start of the block (-1 on fail)
 */
static Sint64 Block_Reset(SDL_IOStream* source, WaveBlock* block) {
    memset(block->data, 0, block->pagesize);
    Sint64 result = SDL_SeekIO(source, block->start, SDL_IO_SEEK_SET);
    if (result > 0) {
 ...
 */
static Sint64 Block_Read(SDL_IOStream* source, WaveBlock* block) {
    size_t off = block->position-block->start;
    size_t amt = block->length-off;
    if (amt >= block->pagesize) {
        amt = block->pagesize;
    }
    block->size = (Sint32)SDL_ReadIO(source, block->data, amt);
    if (block->size > 0) {
 ...
 */
static Sint64 Block_Seek(SDL_IOStream* source, WaveBlock* block, size_t position) {
    Sint64 result = SDL_SeekIO(source, position, SDL_IO_SEEK_SET);
    if (result >= 0) {
 ...
 *
 * If the fact chunk has more frames than sampleframes, this function will
 * return an error. Otherwise, it returns the minimum of the fact chuck and the
 * sample frames.
 *
 ...
 *
 * @param cstate    The decoder state
 ...
 *
 * This function has been factored out out of the original MS_ADPCM_Decode to
 * allow us to stream the results. Instead of working on the active chunk, we
 * have moved the data chunk to a WaveBlock.
 *
 * @param file          The WAV file struct
 *
 * @return true on success
 */
static bool MS_ADPCM_BeginDecode(WaveFile *file) {
    size_t outputsize;
    ...
    return Block_Reset(file->source, block);
}

/**
 * Reads a single page of data from the given file.
 *
 * The buffer should be able to store the output of a page. This is defined by
 * the output samplesize, the number of channels and the sizeof(Sint16). If the
 * read fails, this function returns -1.
 *
 * @param file      The WAV file struct
 ...
 * @return true on success
 */
static bool MS_ADPCM_EndDecode(WaveFile *file) {
    WaveBlock *block = &file->data;
    if (block->data != NULL) {
        ATK_free(block->data);
        block->data = NULL;
 ...
 *
 * @param cindex        The interpolation index
 ...
 * @return true on success
 */
static bool IMA_ADPCM_BeginDecode(WaveFile *file) {
    size_t outputsize;
    WaveBlock *block = &file->data;
    ADPCM_DecoderState* state = (ADPCM_DecoderState*)file->decoderdata;
    Sint8 *cstate;
 ...
 * @return true on success
 */
static bool IMA_ADPCM_EndDecode(WaveFile *file) {
    WaveBlock *block = &file->data;
 ...
 *
 * This function does not allocate any memory. We delay allocating memory until
 * we begin decoding
 *
 * @param file          The WAV file struct
 * @param datalength    The amount of data encoded
 ...
 * Starts the decoding process on the DATA block
 *
 * This function has been factored out out of the original LAW_Decode to allow
 * us to stream the results. Instead of working on the active chunk, we have
 * moved the data chunk to a WaveBlock. In addition, the local look-up tables
 * are copied to file->decoderdata. This memory will be deallocated by
 * {@link LAW_EndDecode}.
 *
 * @param file          The WAV file struct
 *
 * @return true on success
 */
static bool LAW_BeginDecode(WaveFile *file) {
 ...
 *
 * The buffer should be able to store the output of a page. This is defined by
 * WAV_PAGE_SIZE, the number of channels and the sizeof(Sint16). If the read
 ...
 * @return true on success
 */
static bool LAW_EndDecode(WaveFile *file) {
    WaveBlock *block = &file->data;
    if (block->data != NULL) {
        ATK_free(block->data);
        block->data = NULL;
 ...
 * Starts the decoding process on the DATA block
 *
 * This function has been factored out out of the original PCM_Decode to allow
 * us to stream the results. Instead of working on the active chunk, we have
 * moved the data chunk to a WaveBlock.
 *
 * @param file          The WAV file struct
 *
 * @return true on success
 */
static bool PCM_BeginDecode(WaveFile *file) {
    WaveFormat *format = &file->format;
    WaveBlock *block = &file->data;
 ...
 * @return true on success
 */
static bool WaveNextChunk(SDL_IOStream *src, WaveChunk *chunk) {
    ...
    if (SDL_SeekIO(src, nextposition, SDL_IO_SEEK_SET) != nextposition) {
        ...
        return false;
    } else if (SDL_ReadIO(src, chunkheader, CHUNK_SIZE) != CHUNK_SIZE) {
        return false;
    }
...
static bool WaveReadFormat(WaveFile *file) {
    ...
    bool success = true;
    success = success && SDL_ReadU16LE(fmtsrc, &(format->formattag));
    success = success && SDL_ReadU16LE(fmtsrc, &(format->channels));
    success = success && SDL_ReadU32LE(fmtsrc, &(format->frequency));
    success = success && SDL_ReadU32LE(fmtsrc, &(format->byterate));
    success = success && SDL_ReadU16LE(fmtsrc, &(format->blockalign));
    if (!success) {
        return SDL_SetError("Unable to read WAVE fmt chunk");
    }
    format->encoding = format->formattag;

    /* This is PCM specific in the first version of the specification. */
    if (fmtlen >= 16) {
        SDL_ReadU16LE(fmtsrc, &(format->bitspersample));
    ...
    if (fmtlen >= 18) {
        SDL_ReadU16LE(fmtsrc, &(format->extsize));
    }
    ...
    if (format->formattag == EXTENSIBLE_CODE) {
        ...
        success = SDL_ReadU16LE(fmtsrc, &(format->validsamplebits));
        success = success &&  SDL_ReadU32LE(fmtsrc, &(format->channelmask));
        format->samplesperblock = format->validsamplebits;
        if (!success) {
            return SDL_SetError("Unable to read extensible header");
        }

        SDL_ReadIO(fmtsrc, format->subformat, 16);
 ...
 * @return true on success
 */
static bool WaveLoad(SDL_IOStream *src, WaveFile *file) {
    int result;
    Uint32 chunkcount = 0;
    ...
    if (!WaveNextChunk(src, &RIFFchunk)) {
    ...
    if (RIFFchunk.fourcc == RIFF) {
        ...
        if (SDL_ReadIO(src, &formtype, sizeof(Uint32)) != sizeof(Uint32)) {
            return SDL_SetError("Could not read RIFF form type");
        } else if (SDL_Swap32LE(formtype) != WAVE) {
        ...
        if (chunk->fourcc == FMT) {
            ...
            if (file->fact.status == 0) {
                if (chunk->length < 4) {
                    ...
                    Uint32 samplelength;
                    if (position == chunk->position && SDL_ReadIO(src, &samplelength, sizeof(Uint32)) == sizeof(Uint32)) {
                        file->fact.status = 1;
                        file->fact.samplelength = SDL_Swap32LE(samplelength);
                    } else {
                        file->fact.status = -1;
                    }
                }
    ...
    if (file->trunchint == TruncVeryStrict) {
        ...
        if (chunk->fourcc != DATA && chunk->length > 0) {
            ...
            } else if (SDL_ReadIO(src, &tmp, 1) != 1) {
    ...
    *chunk = fmtchunk;
    ...
    if (chunk->length < 14) {
        ...
        return -1;
    } else if (!WaveCheckFormat(file, (size_t)file->data.length)) {
        return -1;
    }
    ...
    case MULAW_CODE:
        ...
        file->samplefmt = SDL_AUDIO_S16;
        break;
    case IEEE_FLOAT_CODE:
        file->samplefmt = SDL_AUDIO_F32LE;
        break;
    case PCM_CODE:
        switch (format->bitspersample) {
        case 8:
            file->samplefmt = SDL_AUDIO_U8;
            break;
        case 16:
            file->samplefmt = SDL_AUDIO_S16LE;
            break;
        case 24: /* Has been shifted to 32 bits. */
        case 32:
            file->samplefmt = SDL_AUDIO_S32LE;
            break;
        default:
 ...
 * @return true on success
 */
static bool WaveBegin(WaveFile *file) {
    WaveFormat *format = &file->format;
        ...
        case PCM_CODE:
            switch (format->bitspersample) {
                ...
                case 16:
                    ...
                    if (swap) {
                        for (ii = samples; ii > 0; ii--) {
                            out[ii-1] = (float)(SDL_Swap16LE(in16[ii-1])/factor);
                        }
                    } else {
                        for (ii = samples; ii > 0; ii--) {
                            out[ii-1] = (float)(in16[ii-1]/factor);
                ...
                case 32:
                    ...
                    if (swap) {
                        for (ii = samples; ii > 0; ii--) {
                            out[ii-1] = (float)(SDL_Swap32LE(in32[ii-1])/factor);
                        }
                    } else {
                        for (ii = samples; ii > 0; ii--) {
 ...
 * @return true on success
 */
static bool WaveEnd(WaveFile *file) {
    if (file->ownsource) {
        SDL_CloseIO(file->source);
    } else {
        SDL_SeekIO(file->source, file->chunk.position, SDL_IO_SEEK_SET);
    }
    ...
    case PCM_CODE:
    case IEEE_FLOAT_CODE:
        success = PCM_EndDecode(file);
        break;
    case ALAW_CODE:
    case MULAW_CODE:
        success = LAW_EndDecode(file);
        break;
    case MS_ADPCM_CODE:
        success = MS_ADPCM_EndDecode(file);
        break;
    case IMA_ADPCM_CODE:
        success = IMA_ADPCM_EndDecode(file);
        break;
    default:
        ATK_SetError("Unrecognized WAV encoding");
        return false;
    ...
    if (success && file->info.fourcc == LIST) {
 ...
 * @return true on success
 */
static bool WaveComments(WaveFile *file, ATK_AudioMetadata* metadata) {
    ...
    while (pos+4 < chunk->size) {
        ...
        memcpy(tag, val, 4);
        memcpy(&len, val+4, sizeof(Uint32));
        len = SDL_Swap32LE(len);
...
static void WavePushState(WaveFile *file, WaveState* state) {
    WaveFormat *format = &file->format;
    WaveBlock* block   = &(file->data);
    SDL_IOStream* stream  = file->source;
...
ATK_AudioSource* ATK_LoadWAV(const char* filename) {
    ...
    if (pool) {
        stream = ATK_IOFromFilePool(filename, "rb", pool);
    } else {
    ...
    return ATK_LoadWAV_IO(stream,true);
}
 ...
 */
ATK_AudioSource* ATK_LoadWAV_IO(SDL_IOStream* source, bool ownsrc) {
    if (source == NULL) {
 ...
 */
bool ATK_SourceIsWAV(SDL_IOStream* source) {
    if (source == NULL) {
        return false;
    }
    ...
    ATK_ClearError();
    SDL_SeekIO(source, pos, SDL_IO_SEEK_SET);
    return result;
}
 ...
 */
bool ATK_WAV_UnloadSource(ATK_AudioSource* source) {
...
Sint32 ATK_WAV_SeekSourcePage(ATK_AudioSource* source, Uint32 page) {
    ...
    WaveBlock* block  = &(file->data);
    SDL_IOStream* stream = file->source;
    Sint64 offset = block->start+block->pagesize*page;
 ...
 */
bool ATK_WAV_IsSourceEOF(ATK_AudioSource* source) {
 ...
 * @return a new ATK_AudioSource from an WAV readable/seekable IOStream
 */
ATK_AudioSource* ATK_LoadWAV_IO(SDL_IOStream* source, int ownsrc) {
    ATK_SetError("Codec WAV is not supported");
    return NULL;
}
 ...
 * @returns true if this is WAV data.
 */
bool ATK_SourceIsWAV(SDL_IOStream* source) { return false; }
 ...
 * @return true if the source was successfully closed
 */
bool ATK_WAV_UnloadSource(ATK_AudioSource* source) { return false; }
 ...
 * @return true if the audio source is at the end of the stream
 */
bool ATK_WAV_IsSourceEOF(ATK_AudioSource* source) { return false; }
 ...
 * @return true if WAV supports the given comment tag.
 */
bool ATK_WAV_SupportsCommentTag(const char* tag) { return false; }
...
ATK_AudioEncoding* ATK_EncodeWAV(const char* filename,
    ...
    SDL_IOStream* stream = NULL;
    if (pool) {
        stream = ATK_IOFromFilePool(filename, "wb", pool);
    } else {
        stream = SDL_IOFromFile(filename, "wb");
    }
    if (stream == NULL) {
        ATK_SetError("Could not open '%s'",filename);
        return NULL;
    }
    return ATK_EncodeWAV_IO(stream,true,metadata);
}
 ...
 */
ATK_AudioEncoding* ATK_EncodeWAV_IO(SDL_IOStream* source, bool ownsrc,
                                    const ATK_AudioMetadata* metadata) {
    if (!source) {
    ...
    output->begin = SDL_TellIO(source);
    output->written = 0;
    ...
    header.format_len = SDL_Swap32LE(0x10);
    header.fixed = SDL_Swap16LE(1);
    header.channels = SDL_Swap16LE(metadata->channels);
    header.sample_rate = SDL_Swap32LE(metadata->rate);
    header.bits_per_sample = SDL_Swap16LE(BPS);
    header.byte_per_sample = SDL_Swap16LE((BPS * metadata->channels) / 8);
    header.byte_rate = SDL_Swap32LE((metadata->rate * BPS * metadata->channels)/8);
    header.package_len = SDL_Swap32LE((Uint32)(data_len+output->data_off));

    // Write header
    if (SDL_WriteIO(source, &(header), sizeof(WaveHead)) < 1) {
        goto fail;
    }
    // Write metadata
    if (meta_len > 0) {
        Uint32 code = LIST;
        if (SDL_WriteIO(source, &code, sizeof(Uint32)) < 1) {
            goto fail;
        }
        code = SDL_Swap32LE(meta_len);
        if (SDL_WriteIO(source, &code, sizeof(Uint32)) < 1) {
            goto fail;
        }
        code = INFO;
        ...
        if (metadata->num_comments) {
            for(int ii = 0; ii < metadata->num_comments; ii++) {
                ...
                if (info) {
                    if (SDL_WriteIO(source, info, sizeof(char)*4) < 1) {
                        goto fail;
                    }
                    const char* val = metadata->comments[ii].value;
                    Uint32 size = SDL_Swap32LE((Uint32)strlen(val)+1);
                    if (SDL_WriteIO(source, &size, sizeof(Uint32)) < 1) {
                        goto fail;
                    }
                    if (SDL_WriteIO(source, val, sizeof(char)*(strlen(val)+1)) < 1) {
                        goto fail;
                    }
                }
            }
        ...
        if (meta_len % 2 == 1) {
            char nullc = 0;
            if (SDL_WriteIO(source, &nullc, sizeof(char)) < 1) {
                goto fail;
            }
        }
    }
...
Sint64 ATK_WAV_WriteEncoding(ATK_AudioEncoding* encoding, float* buffer, size_t frames) {
    ...
    if (swap) {
        for (size_t ii = 0; ii < samples; ii++) {
            out[ii] = SDL_Swap16LE((Sint16)(limit*buffer[ii]));
        }
    } else {
        for (size_t ii = 0; ii < samples; ii++) {
    ...
    size_t amt = SDL_WriteIO(output->source, out, sizeof(Sint16)*samples);
    size_t written = amt/(sizeof(Sint16)*encoding->metadata.channels);
 ...
 */
bool ATK_WAV_FinishEncoding(ATK_AudioEncoding* encoding) {
    ...
    if (encoding->encoder != NULL) {
        ...
        if (output->written != encoding->metadata.frames) {
            ...
            Uint32 pckg = SDL_Swap32LE((Uint32)(data+output->data_off));
            data = SDL_Swap32LE(data);
            if (success && SDL_SeekIO(output->source, output->data_off+4, SDL_IO_SEEK_SET) < 0) {
                success = false;
            }
            if (success && SDL_WriteIO(output->source, &data, sizeof(Uint32)) < sizeof(Uint32)) {
                success = false;
            }
            if (success && SDL_SeekIO(output->source, 4, SDL_IO_SEEK_SET) < 4) {
                success = false;
            }
            if (success && SDL_WriteIO(output->source, &pckg, sizeof(Uint32)) < sizeof(Uint32)) {
                success = false;
            }
        }

        if (output->ownsource) {
            SDL_CloseIO(output->source);
        } else {
            SDL_SeekIO(output->source, output->begin, SDL_IO_SEEK_SET);
        }
        ATK_free(output);
