...
static ATK_AudioComment* ATK_FLAC_AllocComments(const FLAC__StreamMetadata_VorbisComment* comment,
    ...
    for(int ii = 0; ii < amt; ii++) {
        ...
        int pos = -1;
        for(int jj = 0; jj < length && pos == -1; jj++) {
            if (entry[jj] == '=') {
                pos = jj;
            }
        }
...
FLAC__StreamDecoderReadStatus flac_decoder_read(const FLAC__StreamDecoder *flac,
    ...
    if (SDL_TellIO(decoder->stream) == SDL_GetIOSize(decoder->stream)) {
        return FLAC__STREAM_DECODER_READ_STATUS_END_OF_STREAM;
    }
    *bytes = SDL_ReadIO(decoder->stream, buffer, *bytes);
    if (((int)*bytes) == -1) {
...
FLAC__StreamDecoderSeekStatus flac_decoder_seek(const FLAC__StreamDecoder *flac,
    ...
    ATK_AudioSource* source = ( ATK_AudioSource*)cdata;
    ATK_FLAC_Decoder* decoder = (ATK_FLAC_Decoder*)source->decoder;
    if (SDL_SeekIO(decoder->stream,offset, SDL_IO_SEEK_SET) == -1) {
...
FLAC__StreamDecoderTellStatus flac_decoder_tell(const FLAC__StreamDecoder *flac,
    ...
    ATK_AudioSource* source = ( ATK_AudioSource*)cdata;
    ATK_FLAC_Decoder* decoder = (ATK_FLAC_Decoder*)source->decoder;
    *offset = SDL_TellIO(decoder->stream);
    return (((int)*offset) == -1 ? FLAC__STREAM_DECODER_TELL_STATUS_ERROR : FLAC__STREAM_DECODER_TELL_STATUS_OK);
}
...
FLAC__StreamDecoderLengthStatus flac_decoder_size(const FLAC__StreamDecoder *flac,
    ...
    ATK_AudioSource* source = ( ATK_AudioSource*)cdata;
    ATK_FLAC_Decoder* decoder = (ATK_FLAC_Decoder*)source->decoder;
    *length = SDL_GetIOSize(decoder->stream);
    return (((int)*length) == -1 ? FLAC__STREAM_DECODER_LENGTH_STATUS_ERROR : FLAC__STREAM_DECODER_LENGTH_STATUS_OK);
}
...
FLAC__bool flac_decoder_eof(const FLAC__StreamDecoder *flac, void *cdata) {
    ...
    ATK_AudioSource* source = ( ATK_AudioSource*)cdata;
    ATK_FLAC_Decoder* decoder = (ATK_FLAC_Decoder*)source->decoder;
    return SDL_TellIO(decoder->stream) == SDL_GetIOSize(decoder->stream);
}
...
Sint32 flac_read_page(ATK_AudioSource* source, ATK_FLAC_Decoder* decoder, float* buffer) {
    ...
    while (read < (Sint32)limit) {
        ...
        Sint32 check = (Sint32)(decoder->pagesize - read);
        avail = (check < avail ? check : avail);
...
ATK_AudioSource* ATK_LoadFLAC(const char* filename) {
    ...
    return ATK_LoadFLAC_IO(stream,1);
}
 ...
 */
ATK_AudioSource* ATK_LoadFLAC_IO(SDL_IOStream* source, bool ownsrc) {
    if (source == NULL) {
    ...
    decoder = (ATK_FLAC_Decoder*)ATK_malloc(sizeof(ATK_FLAC_Decoder));
    if (!decoder) {
        ATK_OutOfMemory();
        goto fail;
    }

    result = ( ATK_AudioSource*)malloc(sizeof( ATK_AudioSource));
    if (!result) {
        ATK_OutOfMemory();
        goto fail;
    }
 ...
 */
bool ATK_SourceIsFLAC(SDL_IOStream* source) {
    if (source == NULL) {
        return false;
    }
 ...
 */
bool ATK_FLAC_UnloadSource(ATK_AudioSource* source) {
 ...
 */
bool ATK_FLAC_IsSourceEOF(ATK_AudioSource* source) {
 ...
 * @return a new ATK_AudioSource from an Xiph FLAC readable/seekable IOstream
 */
ATK_AudioSource* ATK_LoadFLAC_IO(SDL_RWops* source, bool ownsrc) {
    ATK_SetError("Codec FLAC is not supported");
    return NULL;
}
 ...
 * @returns true if this is Xiph FLAC data.
 */
bool ATK_SourceIsFLAC(SDL_IOStream* source) { return false; }
 ...
 * @return true if the source was successfully closed
 */
bool ATK_FLAC_UnloadSource(ATK_AudioSource* source) { return false; }
 ...
 * @return true if the audio source is at the end of the stream
 */
bool ATK_FLAC_IsEOF(ATK_AudioSource* source) { return false; }
...
FLAC__StreamEncoderWriteStatus flac_encoder_write(const FLAC__StreamEncoder *encoder,
                                                  ...
                                                  void *cdata) {
    ATK_FLAC_Encoder* output = (ATK_FLAC_Encoder*)cdata;
    SDL_IOStream* stream = output->stream;
    if (SDL_WriteIO(stream,buffer, bytes) != bytes) {
...
FLAC__StreamEncoderSeekStatus flac_encoder_seek(const FLAC__StreamEncoder *encoder,
    ...
    ATK_FLAC_Encoder* output = (ATK_FLAC_Encoder*)cdata;
    SDL_IOStream* stream = output->stream;
    if(!SDL_SeekIO(stream, offset, SDL_IO_SEEK_SET)) {
...
FLAC__StreamEncoderTellStatus flac_encoder_tell(const FLAC__StreamEncoder *encoder,
    ...
    SDL_IOStream* stream = output->stream;
    Sint64 pos;
    if((pos = SDL_TellIO(stream)) < 0) {
        return FLAC__STREAM_ENCODER_TELL_STATUS_ERROR;
    }

    *offset = (FLAC__uint64)pos;
...
ATK_AudioEncoding* ATK_EncodeFLAC(const char* filename,
    ...
    return ATK_EncodeFLAC_IO(stream,1,metadata);
}
 ...
 */
ATK_AudioEncoding* ATK_EncodeFLAC_IO(SDL_IOStream* source, bool ownsrc,
                                     const ATK_AudioMetadata* metadata) {
    FLAC__bool ok = true;
    ...
    for(size_t ii = 0; ii < metadata->num_comments; ii++) {
        char* key = ATK_malloc(sizeof(char)*(strlen(src->key)+1));
        strcpy(key, src->key);
        char* value = ATK_malloc(sizeof(char)*(strlen(src->value)+1));
...
Sint64 ATK_FLAC_WriteEncoding(ATK_AudioEncoding* encoding, float* buffer, size_t frames) {
    ...
    Uint64 factor = (Uint64)((1L << (flac->bitdepth-1))-1);
    float* input = buffer;
    while (frames) {
 ...
 */
bool ATK_FLAC_FinishEncoding(ATK_AudioEncoding* encoding) {
    if (encoding == NULL) {
        return false;
    }
