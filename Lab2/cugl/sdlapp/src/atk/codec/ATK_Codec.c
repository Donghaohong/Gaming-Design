...
#define ATK_DECODER_PROPERTY "edu.cornell.gdiac.atk.decoder"
#define ATK_ENCODER_PROPERTY "edu.cornell.gdiac.atk.encoder"
 ...
 */
bool ATK_SupportsComments(ATK_CodecType type) {
    switch(type) {
    case ATK_CODEC_WAV:
    case ATK_CODEC_VORBIS:
    case ATK_CODEC_FLAC:
 ...
 */
bool ATK_SupportsCommentTag(ATK_CodecType type, const char* tag) {
    switch(type) {
    case ATK_CODEC_WAV:
 ...
 */
void ATK_FreeMetadata(ATK_AudioMetadata* metadata, bool deep) {
    if (metadata == NULL) {
        return;
    }
 ...
 */
ATK_AudioMetadata* ATK_CopyMetadata(ATK_AudioMetadata* metadata, bool deep) {
    if (metadata == NULL) {
        return NULL;
    }
...
static struct {
    ...
    ATK_AudioSource *(SDLCALL *load)(SDL_IOStream *src, bool ownsrc);
} supported[] = {
    /* keep popular formats first */
    { "wav",  ATK_SourceIsWAV,    ATK_LoadWAV_IO },
    { "ogg",  ATK_SourceIsVorbis, ATK_LoadVorbis_IO },
    { "flac", ATK_SourceIsFLAC,   ATK_LoadFLAC_IO },
    { "mp3",  ATK_SourceIsMP3,    ATK_LoadMP3_IO },
};
...
ATK_AudioSource* ATK_LoadSource(const char* filename) {
    ...
    return ATK_LoadTypedSource_IO(src, 1, ext);
}
 ...
 */
ATK_AudioSource* ATK_LoadSource_IO(SDL_IOStream* source, bool ownsrc) {
    return ATK_LoadTypedSource_IO(source, ownsrc, NULL);
}
 ...
 * @return a new ATK_AudioSource from the given readable/seekable IOStream
 */
ATK_AudioSource* ATK_LoadTypedSource_IO(SDL_IOStream* source, bool ownsrc, const char* type) {
    ...
    if ( SDL_SeekIO(source, 0, SDL_IO_SEEK_CUR) < 0 ) {
 ...
 */
bool ATK_UnloadSource(ATK_AudioSource* source) {
    if (source == NULL) {
 ...
 */
bool ATK_IsSourceEOF(ATK_AudioSource* source) {
    if (source == NULL) {
        ATK_SetError("Attempt to access NULL codec source");
        return false;
    }
...
#pragma mark -
#pragma mark IOStream Decoding
/** Data String for decoding */
typedef struct {
    ATK_AudioSource* source;
    Uint32 current;
    size_t capacity;
    size_t pagesize;
    size_t firstpage;
 ...
 *
 * @param context   a pointer to an ATK_DecoderState structure
 *
 * @return the total size of the file in bytes or -1 on error
 */
static Sint64 ATK_DecodeSize(void *context) {
    if (context == NULL) {
        ATK_SetError("Attempted to query a null context");
        return -1;
    }
    
    ATK_AudioSource* source = ((ATK_DecoderState*)context)->source;
 ...
 */
static Sint64 ATK_DecodeSeek(void* context, Sint64 offset, SDL_IOWhence whence) {
    if (context == NULL) {
    ...
    ATK_DecoderState* state = (ATK_DecoderState*)context;
    ATK_AudioSource* source = state->source;
 ...
 */
static size_t ATK_DecodeRead(void *context, void *ptr, size_t size, SDL_IOStatus *status) {
    if (context == NULL) {
    ...
    size_t total  = size/chunk; // Number of floats to read
    ...
    if (rem > total) {
        memcpy(ptr,state->buffer+state->offset,total*chunk);
        state->offset += total;
    } else {
        size_t left = total;
        while (rem && left >= rem) {
            ...
            char* cptr = (char*)ptr;
            memcpy(cptr+off,state->buffer+state->offset,rem*chunk);
            left -= rem;
            state->available = ATK_ReadSourcePage(source, state->buffer);
            state->available *= source->metadata.channels;
            state->current++;
        ...
        if (rem && left) {
            size_t off = (total-left)*chunk;
            char* cptr = (char*)ptr;
            memcpy(cptr+off,state->buffer,left*chunk);
            state->offset = left;
        }
    }
 ...
 */
static size_t ATK_DecodeWrite(void *context, const void *ptr, size_t size, SDL_IOStatus *status) {
 ...
 */
static bool ATK_DecodeFlush(void *context, SDL_IOStatus *status) {
    ATK_SetError("Audio sources are read-only");
    *status = SDL_IO_STATUS_READONLY;
    return false;
}
 ...
 * @return true on success
 */
static bool ATK_DecodeClose(void *context) {
    if (context == NULL) {
 ...
 * will fail. The purpose of this object is to provide an interace for a smooth,
 * buffered stream on top of the paging interface provided by {@link ATK_AudioSource}.
 *
 ...
 * to free this data.
 *
 * If ownsrc is true, this SDL_IOStream will close the wrapped SDL_IOStream when done.
 *
 ...
 */
SDL_IOStream* ATK_IOFromAudioSourceIO(SDL_IOStream* stream, bool ownsrc, const char* type,
                                      ATK_AudioMetadata** metadata) {
    SDL_IOStream* result = NULL;
    ATK_AudioSource* source = NULL;
    ATK_DecoderState* state = NULL;
    float* buffer = NULL;
    
    source = ATK_LoadTypedSource_IO(stream, ownsrc, type);
    if (source == NULL) {
        goto fail;
    }
    ...
    iface.size = ATK_DecodeSize;
    iface.seek = ATK_DecodeSeek;
    iface.read = ATK_DecodeRead;
    iface.write = ATK_DecodeWrite;
    iface.flush = ATK_DecodeFlush;
    iface.close = ATK_DecodeClose;
    ...
    SDL_SetPointerProperty(props, ATK_DECODER_PROPERTY, state);
 ...
 */
SDL_IOStream* ATK_IOFromTypedAudioSource(const char* filename, const char* type,
                                         ATK_AudioMetadata** metadata) {
    ...
    return ATK_IOFromAudioSourceIO(stream, 1, type, metadata);
}

/**
 * Returns an SDL_IOStream with the audio frames of the given file
 *
 * The SDL_RWops object returned is read-only. Any attempts to write to it will
 * fail. The purpose of this object is to provide an interace for a smooth,
 * buffered stream on top of the paging interface provided by {@link ATK_AudioSource}.
 *
 ...
 * @return an SDL_IOStream with the audio frames of the given file
 */
SDL_IOStream* ATK_IOFromAudioSource(const char* filename,
                                    ATK_AudioMetadata** metadata) {
    ATK_FilePool* pool = ATK_DefaultFilePool();
    SDL_IOStream* stream = NULL;
    if (pool) {
    ...
    return ATK_IOFromAudioSourceIO(stream, 1, ext, metadata);
}
 ...
 */
ATK_AudioSource* ATK_GetIOAudioSource(SDL_IOStream* stream) {
    if (stream == NULL) {
        return NULL;
    }
    
    SDL_PropertiesID props = SDL_GetIOProperties(stream);
    void* value = SDL_GetPointerProperty(props, ATK_DECODER_PROPERTY, NULL);
    if (value == NULL) {
        return NULL;
    }
    ATK_DecoderState* state = (ATK_DecoderState*)value;
    return state->source;
}
...
ATK_AudioEncoding* ATK_EncodeAudio(const char* filename, const char* type,
    ...
    SDL_IOStream *dst = SDL_IOFromFile(filename, "wb");
    if (!dst) {
        /* The error message has been set in SDL_IOFromFile */
        return NULL;
    }
    return ATK_EncodeAudio_IO(dst, 1, type, metadata);
 ...
 */
ATK_AudioEncoding* ATK_EncodeAudio_IO(SDL_IOStream* stream, bool ownsrc, const char* type,
                                      const ATK_AudioMetadata* metadata) {
    if (stream == NULL) {
    ...
    if (ATK_string_equals(type, "wav")) {
        return ATK_EncodeWAV_IO(stream, ownsrc, metadata);
    } else if (ATK_string_equals(type, "ogg")) {
        return ATK_EncodeVorbis_IO(stream, ownsrc, metadata);
    } else if (ATK_string_equals(type, "flac")) {
        return ATK_EncodeFLAC_IO(stream, ownsrc, metadata);
 ...
 * @param encoding  The encoding stream
 *
 * @return true if the enconding was successfully completed, false otherwise.
 */
bool ATK_FinishEncoding(ATK_AudioEncoding* encoding) {
    if (encoding == NULL) {
        ATK_SetError("Attempt to access NULL codec encoding");
        return false;
    }
...
#pragma mark -
#pragma mark IOStream Encoding
/**
 * Internal implementation of SDL_SizeIO
 *
 * This function is not implemented for encoding.
 *
 * @param context a pointer to an ATK_AudioEncoding structure
 *
 * @return the total size of the file in bytes or -1 on error
 */
static Sint64 ATK_EncodeSize(void* context) {
 ...
 * @return the final offset in the file after the seek or -1 on error
 */
static Sint64 ATK_EncodeSeek(void* context, Sint64 offset, SDL_IOWhence whence) {
 ...
 */
static size_t ATK_EncodeRead(void *context, void *ptr, size_t size, SDL_IOStatus* status) {
 ...
 * @param status    the status on an error result
 *
 * @return the number of bytes written, which is less than num on error
 */
static size_t ATK_EncodeWrite(void *context, const void *ptr, size_t size, SDL_IOStatus* status) {
    if (context == NULL) {
    ...
    ATK_AudioEncoding* out = (ATK_AudioEncoding*)context;
    ...
    Sint64 amt = ATK_WriteEncoding(out, (float*)ptr, chunk);
    if (amt < 0) {
        *status = SDL_IO_STATUS_NOT_READY;
        return 0;
    }
    return amt*(sizeof(float)*out->metadata.channels);
}

/**
 * Internal implementation of SDL_FlushIO
 *
 * This is a trivial implementation that always succeeds
 *
 * @param context   a pointer to an ATK_AudioEncoding structure
 * @param status    the status on an error result
 *
 * @return the number of objects written, which is less than num on error
 */
static bool ATK_EncodeFlush(void *context, SDL_IOStatus* status) {
    return true;
}
 ...
 *
 * @param context   a pointer to an ATK_AudioEncoding structure to close
 *
 * @return true on success
 */
static bool ATK_EncodeClose(void *context) {
    if (context == NULL) {
        ATK_SetError("Attempted to close a null context");
        return false;
    }
 ...
 * (or even seek while writing) will fail. The purpose of this object is to
 ...
 */
SDL_IOStream* ATK_IOToAudioEncodingIO(SDL_IOStream* stream, int ownsrc,
                                      const char* type,
                                      const ATK_AudioMetadata* metadata) {

    SDL_IOStream* result = NULL;
    ATK_AudioEncoding* out = ATK_EncodeAudio_IO(stream, ownsrc, type, metadata);
    if (out == NULL) {
        return NULL;
    }
    ...
    SDL_IOStreamInterface iface;
    SDL_INIT_INTERFACE(&iface);
    iface.size = ATK_EncodeSize;
    iface.seek = ATK_EncodeSeek;
    iface.read = ATK_EncodeRead;
    iface.write = ATK_EncodeWrite;
    iface.flush = ATK_EncodeFlush;
    iface.close = ATK_EncodeClose;

    result = SDL_OpenIO(&iface, out);

    if (result) {
        SDL_PropertiesID props = SDL_GetIOProperties(result);
        SDL_SetPointerProperty(props, ATK_ENCODER_PROPERTY, out);
    }
 ...
 */
SDL_IOStream* ATK_IOToTypedAudioEncoding(const char* filename, const char* type,
                                         const ATK_AudioMetadata* metadata) {
    ATK_FilePool* pool = ATK_DefaultFilePool();
    SDL_IOStream* stream = NULL;
    if (pool) {
    ...
    return ATK_IOToAudioEncodingIO(stream, 1, type, metadata);
}
 ...
 */
SDL_IOStream* ATK_IOToAudioEncoding(const char* filename,
                                    const ATK_AudioMetadata* metadata) {
    ATK_FilePool* pool = ATK_DefaultFilePool();
    SDL_IOStream* stream = NULL;
    if (pool) {
    ...
    return ATK_IOToAudioEncodingIO(stream, 1, ext, metadata);
}
 ...
 */
ATK_AudioEncoding* ATK_GetIOAudioEncoding(SDL_IOStream* stream) {
    if (stream == NULL) {
        return NULL;
    }
    
    SDL_PropertiesID props = SDL_GetIOProperties(stream);
    void* value = SDL_GetPointerProperty(props, ATK_ENCODER_PROPERTY, NULL);
    return (ATK_AudioEncoding*)value;
}
